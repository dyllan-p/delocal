//! Seeds the simulator found and the engine was fixed for (DESIGN.md
//! §14.4: every bug gets a simulator step that reproduces it before it is
//! fixed). Each test pins the seed, the knobs and the minimal step list the
//! shrinker produced, so the fix is guarded by exactly what found the bug.

use crate::steps::Step;
use crate::{Knobs, run_twice};

fn passes(seed: u64, steps: &[Step]) {
    passes_with(seed, &Knobs::default(), steps);
}

/// For seeds found with other knobs than the defaults.
fn passes_with(seed: u64, knobs: &Knobs, steps: &[Step]) {
    if let Err(f) = run_twice(seed, knobs, steps) {
        panic!("{f}");
    }
}

/// The knobs of the nightly's corruption slice (§14.1): one fetch in twenty
/// arrives with the wrong hash.
fn corrupting() -> Knobs {
    Knobs {
        corruption: 0.05,
        ..Knobs::default()
    }
}

/// The same content edited on every node at once: the identical-content
/// merges tie on every field and were counted as rule-5 conflict decisions.
#[test]
fn identical_content_everywhere_is_not_a_winner_fallback() {
    passes(
        0,
        &[Step::Everywhere {
            path: 6,
            contents: vec![Some(2), Some(2)],
        }],
    );
}

/// The same directory created on two nodes: the same tie, on directories.
#[test]
fn the_same_directory_created_twice_is_not_a_winner_fallback() {
    passes(
        1000,
        &[
            Step::Mkdir { node: 5, dir: 2 },
            Step::Tier {
                a: 0,
                b: 2,
                tier: 2,
            },
            Step::Create {
                node: 1,
                path: 11,
                content: 4,
            },
            Step::Mkdir { node: 3, dir: 2 },
        ],
    );
}

/// A version already accepted and being fetched arrived again relayed in a
/// batch that tripped the brake, was quarantined, and its own fetch then
/// adopted it from quarantine.
#[test]
fn a_relayed_copy_of_a_wanted_version_is_not_quarantined() {
    passes(
        5000,
        &[
            Step::Modify {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Settle { secs: 36 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(2), None, Some(3)],
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(3), None],
            },
            Step::Offline { node: 5 },
            Step::MassModify {
                node: 3,
                fraction: 59,
                content: 1,
            },
            Step::Tier {
                a: 2,
                b: 4,
                tier: 0,
            },
            Step::Chmod { node: 6, path: 4 },
        ],
    );
}

/// A want created from the deferred set (an unfreeze, a superseded entry, a
/// re-classification under an observable want) skipped quarantine matching
/// and the brake, and could adopt a quarantined version.
#[test]
fn entries_readmitted_from_the_deferred_set_respect_the_quarantine() {
    passes(
        9000,
        &[
            Step::Online { node: 2 },
            Step::Delete { node: 5, path: 11 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, None, None, Some(5), Some(2)],
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 57,
            },
            Step::Partition { a: 6, b: 0 },
            Step::Heal { a: 0, b: 6 },
            Step::Modify {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 6,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Partition { a: 5, b: 1 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 11,
            },
            Step::Partition { a: 3, b: 5 },
            Step::MassModify {
                node: 7,
                fraction: 68,
                content: 3,
            },
        ],
    );
}

/// `revert` put the announced records back and removed never-announced adds
/// without reporting either, so the persisted index kept the pending
/// records and a restart would have re-announced the reverted changes.
#[test]
fn a_revert_reports_every_index_write() {
    passes(
        0,
        &[
            Step::Settle { secs: 34 },
            Step::Modify {
                node: 1,
                path: 10,
                content: 5,
            },
            Step::Chmod { node: 4, path: 5 },
            Step::Offline { node: 7 },
            Step::Touch { node: 2, path: 2 },
            Step::Settle { secs: 3 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, None, Some(3), Some(4), Some(5), Some(3)],
            },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 2,
            },
            Step::Partition { a: 7, b: 4 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 4,
            },
            Step::Offline { node: 1 },
            Step::Delete { node: 7, path: 11 },
            Step::Create {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::MassDelete {
                node: 2,
                fraction: 95,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 5,
            },
        ],
    );
}

/// A conflict's merged version `M` is announced first by the winner's
/// holder, which does not have `M` until it has seen the loser too; it
/// answered `NotAvailable`, was excluded for good, and its later
/// announcement of `M` did not make it a source again, so the want had no
/// source forever.
#[test]
fn a_source_that_announces_the_wanted_version_again_is_asked_again() {
    passes(
        0,
        &[
            Step::Tier {
                a: 2,
                b: 1,
                tier: 2,
            },
            Step::Chmod { node: 7, path: 5 },
            Step::Settle { secs: 12 },
            Step::Touch { node: 6, path: 7 },
            Step::Tier {
                a: 1,
                b: 4,
                tier: 2,
            },
            Step::Modify {
                node: 7,
                path: 1,
                content: 5,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![None, None],
            },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 0,
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::MassDelete {
                node: 2,
                fraction: 95,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(2), Some(2)],
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 1,
                content: 2,
            },
            Step::Settle { secs: 27 },
            Step::Offline { node: 3 },
            Step::Create {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 5,
            },
            Step::Online { node: 0 },
            Step::Create {
                node: 6,
                path: 5,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 11,
                content: 2,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 24,
            },
            Step::Touch { node: 2, path: 8 },
            Step::Tier {
                a: 4,
                b: 1,
                tier: 2,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 6,
                },
                delay_secs: 10,
            },
            Step::Delete { node: 0, path: 5 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 45,
            },
            Step::Delete { node: 7, path: 3 },
            Step::Online { node: 1 },
            Step::Rmdir { node: 0, dir: 2 },
            Step::Chmod { node: 7, path: 1 },
            Step::Crash {
                node: 4,
                gap_secs: 35,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Heal { a: 0, b: 2 },
            Step::Settle { secs: 9 },
            Step::Modify {
                node: 2,
                path: 6,
                content: 4,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 41,
            },
            Step::Create {
                node: 2,
                path: 2,
                content: 3,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Settle { secs: 22 },
            Step::Touch { node: 5, path: 10 },
            Step::Modify {
                node: 0,
                path: 10,
                content: 2,
            },
            Step::Online { node: 2 },
            Step::Rename {
                node: 1,
                from: 2,
                to: 9,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 2,
                path: 6,
                content: 5,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(4), Some(5), Some(1), Some(5), Some(2), None, Some(1)],
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(2), Some(3), Some(3), None, None],
            },
            Step::Create {
                node: 0,
                path: 7,
                content: 3,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 49,
            },
            Step::Delete { node: 6, path: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 39,
            },
            Step::Rename {
                node: 3,
                from: 4,
                to: 9,
            },
            Step::Offline { node: 0 },
            Step::Create {
                node: 1,
                path: 7,
                content: 3,
            },
            Step::Symlink {
                node: 4,
                path: 10,
                target: 1,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(5), Some(2), Some(1), Some(5), Some(3)],
            },
            Step::Crash {
                node: 3,
                gap_secs: 40,
            },
            Step::Symlink {
                node: 6,
                path: 11,
                target: 7,
            },
            Step::Partition { a: 6, b: 0 },
            Step::MassModify {
                node: 7,
                fraction: 82,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 6,
                content: 3,
            },
            Step::Online { node: 4 },
            Step::Delete { node: 7, path: 1 },
            Step::Online { node: 3 },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(5), None, None],
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 43,
            },
            Step::Chmod { node: 1, path: 11 },
            Step::Online { node: 4 },
            Step::Symlink {
                node: 7,
                path: 9,
                target: 3,
            },
            Step::Delete { node: 4, path: 8 },
            Step::Mkdir { node: 6, dir: 0 },
            Step::Crash {
                node: 0,
                gap_secs: 22,
            },
            Step::Rename {
                node: 5,
                from: 10,
                to: 5,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 39,
            },
            Step::MassDelete {
                node: 6,
                fraction: 91,
            },
            Step::Delete { node: 5, path: 3 },
            Step::Delete { node: 1, path: 7 },
            Step::Tier {
                a: 6,
                b: 6,
                tier: 1,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 1,
            },
        ],
    );
}

/// A scan of a frozen path thawed its entry while the folder was still
/// paused, and the entry, from a batch the brake had held for its other
/// paths, went through the brake alone, passed, and was committed while
/// its batch was still under review.
#[test]
fn an_entry_of_a_held_batch_is_not_committed_before_approve() {
    passes(
        5,
        &[
            Step::MassDelete {
                node: 2,
                fraction: 65,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 20 },
            Step::Create {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Symlink {
                node: 6,
                path: 6,
                target: 11,
            },
            Step::MassDelete {
                node: 1,
                fraction: 62,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 2,
            },
            Step::Online { node: 5 },
            Step::Crash {
                node: 7,
                gap_secs: 96,
            },
            Step::Rename {
                node: 4,
                from: 6,
                to: 7,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), None, Some(5), Some(4), Some(3)],
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 1,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(4), Some(3), None, None, Some(2), None],
            },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 4,
            },
            Step::Partition { a: 6, b: 4 },
            Step::Delete { node: 7, path: 10 },
            Step::Offline { node: 0 },
            Step::MassModify {
                node: 4,
                fraction: 56,
                content: 3,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Symlink {
                node: 5,
                path: 6,
                target: 9,
            },
            Step::Crash {
                node: 5,
                gap_secs: 56,
            },
            Step::Online { node: 4 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::Touch { node: 6, path: 9 },
            Step::Delete { node: 6, path: 7 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Chmod { node: 2, path: 4 },
            Step::Crash {
                node: 1,
                gap_secs: 66,
            },
            Step::Delete { node: 4, path: 5 },
            Step::Crash {
                node: 0,
                gap_secs: 72,
            },
            Step::Chmod { node: 6, path: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::Partition { a: 2, b: 6 },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 1,
                },
                delay_secs: 35,
            },
            Step::Partition { a: 1, b: 3 },
            Step::Chmod { node: 5, path: 8 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(4), Some(1), None, Some(2)],
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 18,
                },
                delay_secs: 35,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 14,
            },
            Step::Rename {
                node: 0,
                from: 11,
                to: 10,
            },
            Step::Delete { node: 0, path: 7 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 0,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 3,
            },
            Step::Heal { a: 4, b: 0 },
            Step::MassModify {
                node: 1,
                fraction: 82,
                content: 1,
            },
            Step::Delete { node: 4, path: 11 },
            Step::Partition { a: 1, b: 1 },
            Step::Create {
                node: 4,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 8,
                content: 1,
            },
        ],
    );
}

/// A crash between a fetch and its commit kept the want's fetched flag
/// while the temp file died with the process; the restarted node asked the
/// host to commit content it did not have, the commit failed, and the entry
/// was deferred at a path with no record and no file, which no scan ever
/// reports.
#[test]
fn a_want_fetched_before_a_crash_is_fetched_again() {
    passes(
        5,
        &[
            Step::Chmod { node: 6, path: 7 },
            Step::Offline { node: 0 },
            Step::MassDelete {
                node: 2,
                fraction: 65,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 20 },
            Step::Delete { node: 6, path: 3 },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Modify {
                node: 7,
                path: 1,
                content: 5,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 58,
            },
            Step::Rmdir { node: 1, dir: 0 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(4), Some(4), Some(2), None, Some(4), None],
            },
            Step::Delete { node: 3, path: 7 },
            Step::Settle { secs: 36 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 56,
            },
            Step::Modify {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(4), None, None, Some(5), Some(1), None],
            },
            Step::Tier {
                a: 4,
                b: 7,
                tier: 2,
            },
            Step::Chmod { node: 1, path: 7 },
            Step::Modify {
                node: 5,
                path: 1,
                content: 3,
            },
            Step::Crash {
                node: 3,
                gap_secs: 7,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Settle { secs: 28 },
            Step::Online { node: 5 },
            Step::Symlink {
                node: 2,
                path: 6,
                target: 1,
            },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 2,
            },
            Step::Heal { a: 5, b: 1 },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 1,
            },
        ],
    );
}

/// Requests named a version, and a conflict's merged version `M` exists
/// nowhere until someone merges: the winner's holders answered
/// `NotAvailable` for `M` although they held its content, and a path whose
/// concurrent holders never all met stayed different across the mesh.
/// Fetches are by hash now (§7.5 steps 2 and 3).
#[test]
fn content_is_fetched_by_hash_from_whoever_holds_it() {
    passes(
        108,
        &[
            Step::Mkdir { node: 6, dir: 1 },
            Step::Crash {
                node: 7,
                gap_secs: 33,
            },
            Step::Offline { node: 3 },
            Step::Create {
                node: 6,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 6,
                path: 6,
                content: 1,
            },
            Step::Offline { node: 5 },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 11,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), None],
            },
            Step::Rename {
                node: 3,
                from: 5,
                to: 7,
            },
            Step::Tier {
                a: 6,
                b: 2,
                tier: 1,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Heal { a: 3, b: 0 },
            Step::Delete { node: 7, path: 4 },
            Step::Tier {
                a: 6,
                b: 3,
                tier: 0,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![None, Some(3)],
            },
            Step::Modify {
                node: 0,
                path: 11,
                content: 4,
            },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Rename {
                node: 3,
                from: 1,
                to: 5,
            },
            Step::Symlink {
                node: 4,
                path: 6,
                target: 1,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Delete { node: 6, path: 1 },
            Step::Create {
                node: 2,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 9,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 2,
            },
        ],
    );
}

/// A batch lost to a crash or a dropped link was never caught up: the
/// receiver took the next batch's `seq_high` as its watermark although
/// it had never seen the sequence numbers below it, so `have_up_to` asked
/// for nothing and a directory's tombstone never reached one node, which
/// kept the directory alive (I3). Batches chain through `seq_low` now and
/// acknowledgements are the contiguous watermark (§7.4).
#[test]
fn a_batch_lost_in_flight_is_caught_up() {
    passes(
        29,
        &[
            Step::Partition { a: 4, b: 6 },
            Step::Create {
                node: 2,
                path: 1,
                content: 4,
            },
            Step::Symlink {
                node: 7,
                path: 8,
                target: 8,
            },
            Step::Touch { node: 0, path: 9 },
            Step::Heal { a: 0, b: 1 },
            Step::Create {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::MassModify {
                node: 4,
                fraction: 91,
                content: 2,
            },
            Step::Delete { node: 7, path: 6 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(1), Some(2), None, None, Some(2)],
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(5), None],
            },
            Step::Tier {
                a: 0,
                b: 6,
                tier: 0,
            },
            Step::Settle { secs: 35 },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 1,
            },
            Step::Crash {
                node: 5,
                gap_secs: 27,
            },
            Step::MassModify {
                node: 3,
                fraction: 86,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 8,
                content: 4,
            },
            Step::Symlink {
                node: 7,
                path: 10,
                target: 8,
            },
            Step::Touch { node: 4, path: 11 },
            Step::Modify {
                node: 5,
                path: 7,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Chmod { node: 5, path: 2 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 57,
            },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Heal { a: 6, b: 7 },
            Step::Symlink {
                node: 2,
                path: 6,
                target: 3,
            },
            Step::Settle { secs: 13 },
            Step::Tier {
                a: 0,
                b: 6,
                tier: 0,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 77,
            },
            Step::Partition { a: 4, b: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 2,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![
                    Some(4),
                    Some(4),
                    Some(4),
                    Some(4),
                    Some(5),
                    Some(2),
                    Some(5),
                ],
            },
            Step::Create {
                node: 1,
                path: 4,
                content: 1,
            },
            Step::Heal { a: 4, b: 0 },
            Step::Delete { node: 5, path: 3 },
            Step::Heal { a: 5, b: 2 },
            Step::MassDelete {
                node: 1,
                fraction: 75,
            },
            Step::Delete { node: 1, path: 7 },
            Step::Create {
                node: 0,
                path: 6,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 2,
            },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Create {
                node: 0,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(1), Some(5)],
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), None],
            },
            Step::MassModify {
                node: 5,
                fraction: 59,
                content: 1,
            },
            Step::Symlink {
                node: 4,
                path: 2,
                target: 4,
            },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(3)],
            },
            Step::Heal { a: 5, b: 4 },
            Step::Settle { secs: 30 },
            Step::Create {
                node: 1,
                path: 6,
                content: 3,
            },
            Step::Settle { secs: 26 },
            Step::Delete { node: 7, path: 2 },
            Step::Create {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Rmdir { node: 1, dir: 0 },
            Step::Online { node: 6 },
            Step::Offline { node: 1 },
            Step::Delete { node: 3, path: 2 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 5,
            },
            Step::Offline { node: 7 },
            Step::Modify {
                node: 2,
                path: 2,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(3), Some(1), Some(2), Some(2), Some(1)],
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 2,
            },
            Step::Touch { node: 7, path: 6 },
            Step::Modify {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Rename {
                node: 3,
                from: 1,
                to: 10,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Crash {
                node: 0,
                gap_secs: 38,
            },
            Step::Online { node: 0 },
            Step::Heal { a: 0, b: 2 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 59,
            },
            Step::Create {
                node: 3,
                path: 5,
                content: 3,
            },
            Step::Modify {
                node: 4,
                path: 3,
                content: 4,
            },
        ],
    );
}

/// A chmod whose watcher event was dropped was invisible to every later
/// scan: the fast path compared size and mtime, and chmod changes neither,
/// so the index disagreed with the disk about the exec bit forever. The
/// fast path compares the exec bit now (§7.3).
#[test]
fn a_chmod_the_watcher_missed_is_found_by_the_next_scan() {
    passes(
        83,
        &[
            Step::Modify {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Crash {
                node: 2,
                gap_secs: 50,
            },
            Step::Chmod { node: 2, path: 3 },
        ],
    );
}

/// Same class as above.
#[test]
fn a_missed_chmod_of_a_conflict_copy_is_found_by_the_next_scan() {
    passes(
        37,
        &[
            Step::Partition { a: 1, b: 0 },
            Step::Heal { a: 2, b: 6 },
            Step::Rename {
                node: 6,
                from: 0,
                to: 2,
            },
            Step::Modify {
                node: 2,
                path: 9,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 5,
            },
            Step::Settle { secs: 10 },
            Step::Crash {
                node: 2,
                gap_secs: 112,
            },
            Step::Heal { a: 3, b: 4 },
            Step::Settle { secs: 37 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), Some(3), Some(3)],
            },
            Step::Online { node: 2 },
            Step::Rename {
                node: 3,
                from: 2,
                to: 4,
            },
            Step::Delete { node: 3, path: 4 },
            Step::Create {
                node: 3,
                path: 4,
                content: 2,
            },
            Step::Partition { a: 4, b: 6 },
            Step::Chmod { node: 0, path: 8 },
            Step::Delete { node: 3, path: 3 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 5,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 14,
                },
                delay_secs: 29,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(4), Some(4), Some(3), Some(1)],
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 3,
            },
            Step::Partition { a: 6, b: 7 },
            Step::Heal { a: 1, b: 1 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 7,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 3,
            },
            Step::Settle { secs: 12 },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(1), Some(2), Some(2), Some(3), Some(1)],
            },
            Step::Chmod { node: 6, path: 8 },
            Step::Settle { secs: 2 },
            Step::Tier {
                a: 5,
                b: 6,
                tier: 2,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(2), Some(2), None, None, Some(5), Some(4)],
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 5,
                path: 1,
                content: 2,
            },
            Step::Chmod { node: 7, path: 4 },
            Step::Offline { node: 3 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(1), Some(3)],
            },
            Step::Offline { node: 5 },
            Step::Touch { node: 0, path: 8 },
            Step::Create {
                node: 7,
                path: 10,
                content: 5,
            },
            Step::Heal { a: 6, b: 1 },
            Step::Delete { node: 4, path: 4 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(1), Some(4), Some(2)],
            },
            Step::Heal { a: 1, b: 4 },
            Step::Create {
                node: 5,
                path: 11,
                content: 4,
            },
            Step::Chmod { node: 3, path: 0 },
            Step::Create {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::MassModify {
                node: 2,
                fraction: 60,
                content: 1,
            },
            Step::Mkdir { node: 3, dir: 0 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 58,
            },
            Step::Settle { secs: 1 },
            Step::Delete { node: 4, path: 8 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(1), Some(1), None],
            },
            Step::Offline { node: 3 },
            Step::Heal { a: 0, b: 7 },
            Step::Settle { secs: 8 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 1,
            },
            Step::Partition { a: 5, b: 1 },
            Step::Chmod { node: 7, path: 11 },
            Step::Create {
                node: 2,
                path: 8,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 9,
                content: 4,
            },
            Step::Chmod { node: 6, path: 1 },
            Step::Modify {
                node: 6,
                path: 3,
                content: 3,
            },
            Step::Rename {
                node: 2,
                from: 11,
                to: 0,
            },
            Step::Create {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Partition { a: 7, b: 5 },
            Step::Offline { node: 0 },
            Step::Create {
                node: 7,
                path: 3,
                content: 5,
            },
            Step::MassModify {
                node: 0,
                fraction: 55,
                content: 1,
            },
        ],
    );
}

/// Same class as above.
#[test]
fn a_missed_chmod_is_found_after_a_long_run() {
    passes(
        76,
        &[
            Step::Settle { secs: 32 },
            Step::Rename {
                node: 5,
                from: 11,
                to: 9,
            },
            Step::Rename {
                node: 2,
                from: 9,
                to: 4,
            },
            Step::Tier {
                a: 0,
                b: 1,
                tier: 2,
            },
            Step::Create {
                node: 6,
                path: 4,
                content: 2,
            },
            Step::MassModify {
                node: 4,
                fraction: 94,
                content: 4,
            },
            Step::Delete { node: 7, path: 8 },
            Step::Offline { node: 0 },
            Step::Settle { secs: 33 },
            Step::Offline { node: 2 },
            Step::Create {
                node: 0,
                path: 7,
                content: 2,
            },
            Step::Symlink {
                node: 3,
                path: 5,
                target: 8,
            },
            Step::Touch { node: 7, path: 4 },
            Step::Chmod { node: 2, path: 11 },
            Step::Rename {
                node: 3,
                from: 1,
                to: 4,
            },
            Step::Mkdir { node: 6, dir: 2 },
            Step::Online { node: 4 },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 1,
            },
            Step::Touch { node: 6, path: 1 },
            Step::MassModify {
                node: 5,
                fraction: 95,
                content: 3,
            },
            Step::Crash {
                node: 4,
                gap_secs: 94,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(2), None, None, Some(2), Some(5), Some(3)],
            },
            Step::Heal { a: 1, b: 5 },
            Step::Create {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Create {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 0,
                content: 2,
            },
            Step::Rename {
                node: 1,
                from: 0,
                to: 1,
            },
            Step::Touch { node: 3, path: 4 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 2,
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 5,
            },
            Step::Heal { a: 6, b: 1 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Delete { node: 4, path: 1 },
            Step::Crash {
                node: 2,
                gap_secs: 21,
            },
            Step::Modify {
                node: 3,
                path: 7,
                content: 3,
            },
            Step::MassDelete {
                node: 5,
                fraction: 93,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(3), Some(3), Some(4), Some(4)],
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Offline { node: 1 },
            Step::Modify {
                node: 4,
                path: 3,
                content: 2,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 25,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 2,
            },
            Step::Symlink {
                node: 1,
                path: 5,
                target: 8,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Online { node: 2 },
            Step::Create {
                node: 2,
                path: 11,
                content: 5,
            },
            Step::Partition { a: 4, b: 7 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 55,
            },
            Step::Create {
                node: 2,
                path: 10,
                content: 2,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(4), None, Some(5), Some(3), Some(2)],
            },
            Step::Chmod { node: 1, path: 10 },
            Step::Offline { node: 5 },
            Step::Create {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Delete { node: 2, path: 2 },
            Step::Chmod { node: 2, path: 4 },
            Step::MassModify {
                node: 1,
                fraction: 93,
                content: 3,
            },
            Step::Delete { node: 5, path: 6 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), None, Some(4), Some(3), Some(3), Some(4)],
            },
            Step::Create {
                node: 7,
                path: 4,
                content: 4,
            },
            Step::Crash {
                node: 7,
                gap_secs: 91,
            },
            Step::Partition { a: 1, b: 1 },
            Step::Rename {
                node: 2,
                from: 6,
                to: 10,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 5,
                    hold_pct: 14,
                },
                delay_secs: 36,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 5,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), None, Some(2), Some(1)],
            },
            Step::Chmod { node: 0, path: 7 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(2), Some(1), Some(4), Some(5), None],
            },
        ],
    );
}

/// Three concurrent versions of one path merged pairwise in different
/// orders gave two nodes the same vector with different content (a file on
/// one, a symlink on the other): a symlink that replaced a file ranked
/// below the file it replaced, and one node met the newer increment alone.
/// The stamp is strictly increasing along every chain and is the first key
/// of the winner rule (§7.1, §7.6), so equal vectors mean equal content (I7).
#[test]
fn equal_vectors_mean_equal_content_when_a_symlink_replaces_a_file() {
    passes(
        2,
        &[
            Step::Delete { node: 4, path: 11 },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 2,
            },
            Step::Create {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 3,
            },
            Step::Heal { a: 4, b: 4 },
            Step::Partition { a: 0, b: 7 },
            Step::Touch { node: 5, path: 1 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Delete { node: 4, path: 2 },
            Step::Partition { a: 5, b: 3 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 5,
            },
            Step::Online { node: 4 },
            Step::Offline { node: 2 },
            Step::Create {
                node: 6,
                path: 5,
                content: 1,
            },
            Step::Symlink {
                node: 1,
                path: 11,
                target: 3,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 1,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 38,
            },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Settle { secs: 11 },
            Step::Delete { node: 0, path: 4 },
            Step::Tier {
                a: 0,
                b: 5,
                tier: 1,
            },
            Step::Tier {
                a: 5,
                b: 7,
                tier: 1,
            },
            Step::Delete { node: 4, path: 1 },
            Step::Modify {
                node: 7,
                path: 2,
                content: 3,
            },
            Step::Rmdir { node: 7, dir: 2 },
            Step::Delete { node: 1, path: 11 },
            Step::Modify {
                node: 1,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::Partition { a: 5, b: 6 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Rename {
                node: 5,
                from: 3,
                to: 4,
            },
            Step::Create {
                node: 6,
                path: 3,
                content: 3,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 3,
            },
            Step::Settle { secs: 18 },
            Step::Tier {
                a: 4,
                b: 0,
                tier: 2,
            },
            Step::Delete { node: 7, path: 6 },
            Step::Create {
                node: 4,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 3 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Delete { node: 7, path: 8 },
            Step::Partition { a: 6, b: 2 },
            Step::Modify {
                node: 0,
                path: 11,
                content: 2,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Touch { node: 0, path: 5 },
            Step::Offline { node: 1 },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Create {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 7,
                to: 5,
            },
            Step::Offline { node: 1 },
            Step::Delete { node: 1, path: 7 },
            Step::Settle { secs: 22 },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Partition { a: 1, b: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 14,
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 35,
                },
                delay_secs: 23,
            },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Chmod { node: 1, path: 1 },
            Step::Chmod { node: 7, path: 11 },
            Step::Heal { a: 0, b: 3 },
            Step::Delete { node: 5, path: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 26,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 1,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(1), Some(2), Some(3)],
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Chmod { node: 2, path: 4 },
            Step::Offline { node: 0 },
            Step::Create {
                node: 4,
                path: 6,
                content: 3,
            },
            Step::MassModify {
                node: 0,
                fraction: 90,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 1,
            },
            Step::Create {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Heal { a: 6, b: 2 },
            Step::Symlink {
                node: 3,
                path: 9,
                target: 11,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(5), None, Some(3), None, None],
            },
            Step::MassModify {
                node: 1,
                fraction: 57,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 11,
                content: 4,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(2), Some(1), Some(1), Some(5)],
            },
            Step::Modify {
                node: 1,
                path: 6,
                content: 2,
            },
            Step::Heal { a: 2, b: 4 },
            Step::Everywhere {
                path: 4,
                contents: vec![None, Some(1), None],
            },
            Step::Partition { a: 4, b: 6 },
            Step::Settle { secs: 12 },
            Step::Partition { a: 6, b: 7 },
            Step::Partition { a: 1, b: 6 },
            Step::Heal { a: 2, b: 2 },
            Step::Online { node: 0 },
            Step::Offline { node: 7 },
            Step::Online { node: 7 },
            Step::Create {
                node: 6,
                path: 1,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 10,
                content: 1,
            },
            Step::Settle { secs: 39 },
            Step::Delete { node: 6, path: 3 },
            Step::Create {
                node: 5,
                path: 11,
                content: 4,
            },
            Step::Heal { a: 4, b: 2 },
            Step::Modify {
                node: 4,
                path: 8,
                content: 1,
            },
            Step::Touch { node: 0, path: 2 },
            Step::Modify {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::Offline { node: 0 },
            Step::Create {
                node: 3,
                path: 6,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Partition { a: 1, b: 7 },
            Step::Create {
                node: 2,
                path: 7,
                content: 2,
            },
            Step::MassDelete {
                node: 5,
                fraction: 89,
            },
            Step::Settle { secs: 26 },
            Step::Delete { node: 3, path: 4 },
            Step::Touch { node: 1, path: 0 },
            Step::Delete { node: 3, path: 10 },
            Step::MassDelete {
                node: 7,
                fraction: 84,
            },
            Step::Online { node: 4 },
            Step::Partition { a: 3, b: 7 },
            Step::Partition { a: 6, b: 7 },
            Step::Create {
                node: 1,
                path: 9,
                content: 3,
            },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Symlink {
                node: 0,
                path: 11,
                target: 4,
            },
            Step::Settle { secs: 24 },
            Step::Tier {
                a: 6,
                b: 0,
                tier: 0,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 39,
            },
            Step::Delete { node: 0, path: 2 },
            Step::Create {
                node: 0,
                path: 9,
                content: 5,
            },
            Step::MassModify {
                node: 6,
                fraction: 69,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Crash {
                node: 7,
                gap_secs: 117,
            },
            Step::Heal { a: 3, b: 6 },
            Step::Create {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 3,
            },
            Step::Offline { node: 4 },
            Step::Symlink {
                node: 4,
                path: 11,
                target: 3,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Partition { a: 4, b: 6 },
        ],
    );
}

/// Same class: two tombstones merged on one node, a tombstone and a live
/// edit then the other tombstone on another; one vector, one deleted and one
/// live, each dropping the other as Equal forever.
#[test]
fn equal_vectors_mean_equal_content_when_deletes_race_an_edit() {
    passes(
        111,
        &[
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 19,
            },
            Step::Symlink {
                node: 2,
                path: 9,
                target: 7,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::MassModify {
                node: 5,
                fraction: 51,
                content: 5,
            },
            Step::Delete { node: 5, path: 9 },
            Step::MassDelete {
                node: 2,
                fraction: 66,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::Online { node: 5 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(2), Some(2), Some(5)],
            },
            Step::Chmod { node: 4, path: 9 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Delete { node: 2, path: 0 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(1), Some(3), Some(1), Some(1), Some(3), Some(1)],
            },
            Step::Modify {
                node: 5,
                path: 11,
                content: 5,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 12,
            },
            Step::Heal { a: 4, b: 5 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 1,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(4), None, Some(4)],
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Heal { a: 6, b: 6 },
            Step::Heal { a: 0, b: 4 },
            Step::Crash {
                node: 4,
                gap_secs: 117,
            },
            Step::Crash {
                node: 2,
                gap_secs: 118,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 5,
            },
            Step::Crash {
                node: 5,
                gap_secs: 4,
            },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 2,
            },
            Step::Modify {
                node: 7,
                path: 11,
                content: 1,
            },
            Step::Tier {
                a: 2,
                b: 5,
                tier: 0,
            },
            Step::Partition { a: 4, b: 2 },
            Step::Delete { node: 4, path: 9 },
            Step::Online { node: 0 },
            Step::Modify {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Tier {
                a: 0,
                b: 4,
                tier: 0,
            },
            Step::Partition { a: 3, b: 3 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 12,
            },
            Step::Delete { node: 0, path: 10 },
            Step::Touch { node: 1, path: 3 },
            Step::Create {
                node: 6,
                path: 4,
                content: 2,
            },
            Step::Heal { a: 0, b: 1 },
            Step::Partition { a: 4, b: 1 },
            Step::Rename {
                node: 3,
                from: 6,
                to: 8,
            },
            Step::Modify {
                node: 3,
                path: 11,
                content: 4,
            },
            Step::Delete { node: 1, path: 8 },
            Step::Rename {
                node: 7,
                from: 8,
                to: 2,
            },
            Step::Create {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 13,
                },
                delay_secs: 4,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 6,
                content: 2,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(2), None, Some(1), Some(2), Some(4)],
            },
            Step::Touch { node: 7, path: 0 },
            Step::Settle { secs: 29 },
            Step::Create {
                node: 6,
                path: 9,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 4,
            },
            Step::Settle { secs: 38 },
            Step::Partition { a: 4, b: 0 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Online { node: 1 },
            Step::Settle { secs: 34 },
            Step::Symlink {
                node: 4,
                path: 0,
                target: 8,
            },
            Step::Tier {
                a: 6,
                b: 7,
                tier: 1,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 5,
            },
            Step::Settle { secs: 38 },
            Step::Delete { node: 3, path: 2 },
            Step::Rename {
                node: 6,
                from: 3,
                to: 4,
            },
            Step::Modify {
                node: 6,
                path: 6,
                content: 4,
            },
            Step::Delete { node: 1, path: 8 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Tier {
                a: 6,
                b: 7,
                tier: 1,
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 5,
            },
            Step::Touch { node: 3, path: 8 },
            Step::Create {
                node: 7,
                path: 10,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 10,
                content: 4,
            },
            Step::Settle { secs: 14 },
            Step::Modify {
                node: 0,
                path: 4,
                content: 2,
            },
            Step::Offline { node: 0 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(2), Some(5), None, None],
            },
            Step::Modify {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Partition { a: 4, b: 2 },
            Step::Create {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Online { node: 6 },
            Step::Heal { a: 6, b: 6 },
            Step::Delete { node: 6, path: 9 },
            Step::Modify {
                node: 7,
                path: 5,
                content: 3,
            },
            Step::Settle { secs: 12 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 4,
            },
            Step::Crash {
                node: 4,
                gap_secs: 1,
            },
            Step::Tier {
                a: 4,
                b: 1,
                tier: 0,
            },
            Step::Symlink {
                node: 5,
                path: 9,
                target: 1,
            },
            Step::Partition { a: 3, b: 5 },
            Step::Delete { node: 3, path: 4 },
            Step::Settle { secs: 9 },
            Step::MassDelete {
                node: 2,
                fraction: 77,
            },
            Step::MassDelete {
                node: 3,
                fraction: 59,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 2,
            },
        ],
    );
}

/// A `deny` with no local record made a tombstone stamped 1 that dominated
/// a version stamped by its mtime; a node that met the tombstone alone
/// ranked its own edit above it while a node holding the dominated version
/// ranked that above everything, and the same vector carried two contents
/// (I7). A deny's bump now stamps one past every version it dominates.
#[test]
fn a_deny_ranks_above_every_version_it_dominates() {
    passes(
        0,
        &[
            Step::Tier {
                a: 7,
                b: 7,
                tier: 0,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 8,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 4,
                content: 3,
            },
            Step::Heal { a: 4, b: 0 },
            Step::Create {
                node: 6,
                path: 5,
                content: 2,
            },
            Step::Offline { node: 4 },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Create {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 7,
                content: 4,
            },
            Step::Create {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 8,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(3), Some(5), Some(1)],
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 4,
            },
            Step::Partition { a: 3, b: 3 },
            Step::Settle { secs: 30 },
            Step::Create {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::Crash {
                node: 1,
                gap_secs: 16,
            },
            Step::MassModify {
                node: 7,
                fraction: 98,
                content: 1,
            },
            Step::Partition { a: 7, b: 4 },
            Step::Heal { a: 3, b: 5 },
            Step::Delete { node: 0, path: 11 },
            Step::Online { node: 0 },
            Step::Create {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Delete { node: 2, path: 6 },
            Step::Online { node: 2 },
            Step::Rename {
                node: 1,
                from: 2,
                to: 9,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Offline { node: 3 },
            Step::Offline { node: 2 },
            Step::Settle { secs: 20 },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Create {
                node: 6,
                path: 10,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 3,
            },
            Step::Settle { secs: 22 },
            Step::Online { node: 6 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(4), Some(3)],
            },
            Step::Crash {
                node: 7,
                gap_secs: 19,
            },
            Step::Create {
                node: 1,
                path: 9,
                content: 3,
            },
            Step::Tier {
                a: 6,
                b: 7,
                tier: 1,
            },
            Step::Modify {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(2), Some(5)],
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(4), None, Some(4)],
            },
            Step::Delete { node: 7, path: 1 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 3,
                content: 5,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 43,
            },
            Step::Chmod { node: 1, path: 11 },
            Step::Online { node: 4 },
            Step::Rmdir { node: 0, dir: 0 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 1,
            },
            Step::Rename {
                node: 1,
                from: 7,
                to: 7,
            },
            Step::Settle { secs: 15 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 29,
                },
                delay_secs: 9,
            },
        ],
    );
}

/// Two holders of the same losing version each displaced it to the same
/// conflict-copy path. One wanted the other's copy and was fetching it when
/// its own commit wrote its copy record at that path; the fetch then landed
/// over that record with a version that did not dominate it (I8). Every
/// index write now re-classifies the wants at its path, so the want meets
/// the node's own copy as an identical-content merge.
#[test]
fn a_want_for_a_peers_conflict_copy_meets_our_own_copy_as_a_merge() {
    passes(
        17,
        &[
            Step::Rename {
                node: 5,
                from: 3,
                to: 6,
            },
            Step::Settle { secs: 13 },
            Step::Create {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::MassDelete {
                node: 1,
                fraction: 52,
            },
            Step::Settle { secs: 18 },
            Step::Touch { node: 7, path: 1 },
            Step::Crash {
                node: 3,
                gap_secs: 13,
            },
            Step::Touch { node: 1, path: 9 },
            Step::Heal { a: 4, b: 1 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(5), Some(4), Some(2), Some(2), Some(2)],
            },
        ],
    );
}

/// A node denied a batch, then reverted: its deny bump was discarded before
/// anyone saw it. Another node later reached the same vector by a merge
/// with the node's re-issued counter, and I7 compared it with the discarded
/// record. Records a revert discarded are no evidence of anything.
#[test]
fn a_vector_a_revert_discarded_may_be_reached_again_by_a_merge() {
    passes(
        164,
        &[
            Step::MassDelete {
                node: 3,
                fraction: 68,
            },
            Step::Create {
                node: 7,
                path: 1,
                content: 4,
            },
            Step::Crash {
                node: 6,
                gap_secs: 26,
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 1,
            },
            Step::Touch { node: 4, path: 10 },
            Step::Settle { secs: 33 },
            Step::Settle { secs: 12 },
            Step::Offline { node: 4 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 40,
            },
            Step::Online { node: 5 },
            Step::Rmdir { node: 5, dir: 0 },
            Step::Delete { node: 2, path: 4 },
            Step::Heal { a: 0, b: 6 },
            Step::Rename {
                node: 0,
                from: 6,
                to: 5,
            },
            Step::Partition { a: 5, b: 0 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 3,
            },
            Step::Online { node: 4 },
            Step::Chmod { node: 6, path: 5 },
            Step::Online { node: 2 },
            Step::Delete { node: 1, path: 11 },
            Step::Touch { node: 3, path: 9 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 2,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![
                    Some(3),
                    Some(3),
                    Some(1),
                    Some(5),
                    Some(2),
                    Some(4),
                    Some(1),
                ],
            },
            Step::Create {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Chmod { node: 7, path: 2 },
            Step::Modify {
                node: 0,
                path: 2,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 1,
            },
            Step::Chmod { node: 2, path: 6 },
            Step::MassDelete {
                node: 2,
                fraction: 54,
            },
            Step::Modify {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 14,
            },
            Step::Online { node: 4 },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::MassDelete {
                node: 7,
                fraction: 59,
            },
            Step::Partition { a: 5, b: 3 },
            Step::Online { node: 2 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 29,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 34,
            },
            Step::Crash {
                node: 1,
                gap_secs: 78,
            },
            Step::Rename {
                node: 3,
                from: 9,
                to: 7,
            },
        ],
    );
}

/// A node removed a directory, paused on the deletes, and reverted: the
/// tombstones were discarded unannounced, and I3 took one for a deletion the
/// mesh had agreed on.
#[test]
fn a_tombstone_a_revert_discarded_is_not_a_deletion() {
    passes(
        176,
        &[
            Step::Modify {
                node: 1,
                path: 11,
                content: 3,
            },
            Step::Rename {
                node: 1,
                from: 10,
                to: 9,
            },
            Step::Chmod { node: 4, path: 3 },
            Step::Tier {
                a: 7,
                b: 5,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 4,
                content: 3,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Heal { a: 2, b: 2 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 3,
            },
            Step::MassDelete {
                node: 4,
                fraction: 75,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 4,
            },
            Step::Chmod { node: 7, path: 6 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 5,
            },
            Step::Tier {
                a: 5,
                b: 0,
                tier: 0,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(3), None, Some(5), Some(3), Some(3), None, None],
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 50,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Heal { a: 2, b: 1 },
            Step::Heal { a: 0, b: 6 },
            Step::Settle { secs: 17 },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 30,
                },
                delay_secs: 49,
            },
            Step::Chmod { node: 2, path: 5 },
            Step::Settle { secs: 31 },
            Step::Offline { node: 4 },
            Step::Crash {
                node: 3,
                gap_secs: 93,
            },
            Step::Settle { secs: 29 },
            Step::Create {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 97,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::DenyAll,
                delay_secs: 50,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Rename {
                node: 1,
                from: 11,
                to: 10,
            },
            Step::Delete { node: 1, path: 1 },
            Step::Rmdir { node: 5, dir: 2 },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Modify {
                node: 4,
                path: 1,
                content: 2,
            },
            Step::Settle { secs: 5 },
            Step::Touch { node: 3, path: 5 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::Create {
                node: 3,
                path: 11,
                content: 4,
            },
        ],
    );
}

/// A node's first version of a path was discarded by its revert; its next
/// change re-issued the same vector with the same content but a later
/// mtime. The version table kept the discarded record (same content, no
/// I7 clash), so the conflict copy the node later made of the newer record
/// was named after an mtime the table did not know.
#[test]
fn a_reissued_vector_replaces_the_discarded_record_whatever_its_content() {
    passes(
        26,
        &[
            Step::Touch { node: 6, path: 3 },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 0,
            },
            Step::Create {
                node: 5,
                path: 6,
                content: 5,
            },
            Step::Online { node: 3 },
            Step::Chmod { node: 3, path: 10 },
            Step::Create {
                node: 6,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 1,
            },
            Step::Tier {
                a: 6,
                b: 5,
                tier: 0,
            },
            Step::Online { node: 1 },
            Step::Create {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::MassModify {
                node: 3,
                fraction: 88,
                content: 3,
            },
            Step::Delete { node: 0, path: 10 },
            Step::Chmod { node: 1, path: 11 },
            Step::Create {
                node: 6,
                path: 1,
                content: 3,
            },
            Step::Online { node: 5 },
            Step::Rename {
                node: 0,
                from: 0,
                to: 8,
            },
            Step::Tier {
                a: 5,
                b: 4,
                tier: 2,
            },
            Step::Touch { node: 4, path: 11 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 26,
            },
            Step::Create {
                node: 5,
                path: 4,
                content: 2,
            },
            Step::Delete { node: 1, path: 3 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(4), None, Some(5), None],
            },
            Step::Create {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Crash {
                node: 3,
                gap_secs: 41,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), Some(1), Some(3), None, Some(3)],
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 5,
            },
            Step::MassDelete {
                node: 7,
                fraction: 83,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::MassModify {
                node: 4,
                fraction: 80,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 7,
                content: 5,
            },
            Step::Rename {
                node: 1,
                from: 4,
                to: 3,
            },
            Step::Modify {
                node: 2,
                path: 3,
                content: 2,
            },
            Step::Touch { node: 3, path: 9 },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Touch { node: 7, path: 9 },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 38,
            },
            Step::Delete { node: 6, path: 11 },
            Step::Partition { a: 4, b: 3 },
            Step::Tier {
                a: 1,
                b: 4,
                tier: 1,
            },
            Step::Offline { node: 2 },
            Step::Create {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Settle { secs: 21 },
            Step::Create {
                node: 6,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 1,
            },
            Step::Offline { node: 3 },
            Step::MassModify {
                node: 1,
                fraction: 87,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 0,
                content: 1,
            },
            Step::Partition { a: 5, b: 7 },
            Step::Modify {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 14,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Online { node: 2 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(1)],
            },
        ],
    );
}

/// A symlink retargeted under a dropped watcher event was invisible to every
/// later scan: the fast path matched symlinks by kind alone. The source kept
/// announcing the old target's hash and served the new target's bytes, so
/// receivers mismatched twice and gave up. The fast path compares the
/// target now (§7.3).
#[test]
fn a_retargeted_symlink_the_watcher_missed_is_found_by_the_next_scan() {
    passes(
        90,
        &[
            Step::Create {
                node: 3,
                path: 9,
                content: 2,
            },
            Step::MassModify {
                node: 5,
                fraction: 73,
                content: 5,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Partition { a: 1, b: 6 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(5), None],
            },
            Step::Crash {
                node: 3,
                gap_secs: 46,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Modify {
                node: 2,
                path: 7,
                content: 3,
            },
            Step::Crash {
                node: 1,
                gap_secs: 81,
            },
            Step::Crash {
                node: 6,
                gap_secs: 82,
            },
            Step::Offline { node: 2 },
            Step::Crash {
                node: 1,
                gap_secs: 29,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 4,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Delete { node: 0, path: 6 },
            Step::Delete { node: 1, path: 7 },
            Step::Settle { secs: 17 },
            Step::Create {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Touch { node: 5, path: 3 },
            Step::Settle { secs: 14 },
            Step::Tier {
                a: 7,
                b: 1,
                tier: 2,
            },
            Step::Mkdir { node: 6, dir: 2 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 10,
            },
            Step::Symlink {
                node: 2,
                path: 1,
                target: 2,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 3,
            },
            Step::Symlink {
                node: 2,
                path: 1,
                target: 11,
            },
        ],
    );
}

/// A file announced, fetched by nobody, deleted by its user and then
/// reverted: the restored record described content that existed nowhere,
/// the restoring want never found a source, and the index said live while
/// the disk said absent forever. Once every member has answered
/// NotAvailable the deletion stands (§8.3 step 4).
#[test]
fn a_reverted_deletion_nobody_can_serve_stands() {
    passes(
        106,
        &[
            Step::Delete { node: 1, path: 5 },
            Step::Modify {
                node: 2,
                path: 2,
                content: 4,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Crash {
                node: 2,
                gap_secs: 61,
            },
            Step::Delete { node: 0, path: 8 },
            Step::Partition { a: 0, b: 1 },
            Step::Modify {
                node: 5,
                path: 10,
                content: 3,
            },
            Step::MassDelete {
                node: 2,
                fraction: 57,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Delete { node: 0, path: 8 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(1), Some(2), Some(5), Some(5), None, Some(5)],
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 4,
            },
            Step::Partition { a: 4, b: 0 },
            Step::Symlink {
                node: 3,
                path: 3,
                target: 5,
            },
            Step::Offline { node: 5 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Heal { a: 5, b: 0 },
            Step::Settle { secs: 30 },
            Step::Partition { a: 0, b: 0 },
            Step::Delete { node: 7, path: 11 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 40,
            },
            Step::Crash {
                node: 4,
                gap_secs: 99,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 2,
                content: 3,
            },
            Step::Chmod { node: 0, path: 6 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 3,
            },
            Step::MassDelete {
                node: 4,
                fraction: 85,
            },
            Step::Delete { node: 5, path: 2 },
            Step::Delete { node: 6, path: 3 },
            Step::Create {
                node: 1,
                path: 7,
                content: 1,
            },
            Step::Offline { node: 0 },
            Step::Offline { node: 2 },
            Step::Partition { a: 5, b: 2 },
            Step::Heal { a: 2, b: 0 },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(1), None],
            },
            Step::Online { node: 7 },
            Step::Touch { node: 6, path: 8 },
            Step::Partition { a: 1, b: 5 },
            Step::Everywhere {
                path: 3,
                contents: vec![None, None, Some(5)],
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(2), None, Some(3), Some(5), Some(2), Some(2)],
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Settle { secs: 28 },
            Step::Rename {
                node: 0,
                from: 0,
                to: 11,
            },
            Step::Delete { node: 0, path: 4 },
            Step::Delete { node: 0, path: 1 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Rename {
                node: 7,
                from: 1,
                to: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 15,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 42,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![None, None, Some(1), None, Some(4), Some(2), None],
            },
        ],
    );
}

/// A paused folder answered a peer's catch-up with a record it had adopted
/// but never announced (the pending set kept it as what a later local
/// change replaced). The batch's seq_high moved the peer's watermark past
/// the withheld pending records, and when the pause was approved and they
/// went out, that peer never asked for them: a conflict copy live on one
/// node and absent on another.
#[test]
fn catch_up_never_sends_what_this_machine_has_not_announced() {
    passes(
        33,
        &[
            Step::Delete { node: 7, path: 10 },
            Step::Settle { secs: 2 },
            Step::Delete { node: 5, path: 11 },
            Step::Delete { node: 7, path: 3 },
            Step::Create {
                node: 5,
                path: 11,
                content: 4,
            },
            Step::Heal { a: 1, b: 1 },
            Step::Settle { secs: 12 },
            Step::Touch { node: 6, path: 4 },
            Step::Settle { secs: 16 },
            Step::Everywhere {
                path: 3,
                contents: vec![None, None, Some(4), Some(2), Some(1), Some(5), Some(3)],
            },
            Step::Heal { a: 1, b: 0 },
            Step::Heal { a: 7, b: 4 },
            Step::Touch { node: 4, path: 11 },
            Step::Heal { a: 4, b: 7 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::DenyAll,
                delay_secs: 39,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Online { node: 5 },
            Step::MassModify {
                node: 3,
                fraction: 97,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 1,
            },
            Step::Chmod { node: 4, path: 2 },
            Step::Partition { a: 4, b: 1 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 54,
            },
            Step::Delete { node: 7, path: 6 },
            Step::Create {
                node: 4,
                path: 5,
                content: 4,
            },
            Step::Tier {
                a: 4,
                b: 3,
                tier: 1,
            },
            Step::Chmod { node: 4, path: 7 },
            Step::Modify {
                node: 7,
                path: 10,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 1,
            },
            Step::Partition { a: 1, b: 6 },
            Step::Symlink {
                node: 3,
                path: 10,
                target: 4,
            },
            Step::Delete { node: 6, path: 9 },
            Step::Settle { secs: 6 },
            Step::Online { node: 5 },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Create {
                node: 0,
                path: 7,
                content: 2,
            },
            Step::Settle { secs: 31 },
            Step::Partition { a: 7, b: 3 },
            Step::Delete { node: 1, path: 2 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Online { node: 0 },
            Step::MassDelete {
                node: 7,
                fraction: 85,
            },
            Step::Rename {
                node: 3,
                from: 0,
                to: 2,
            },
            Step::Offline { node: 3 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 10,
            },
        ],
    );
}

/// A revert restored a node's announced record and the checker took that
/// record, too, for one the revert had discarded, so a deletion it
/// dominated looked uncontradicted. Only the node's versions above the
/// restored one were pending and discarded.
#[test]
fn a_revert_discards_only_the_versions_above_the_restored_record() {
    passes(
        203,
        &[
            Step::Touch { node: 4, path: 4 },
            Step::Partition { a: 1, b: 5 },
            Step::Delete { node: 4, path: 6 },
            Step::Settle { secs: 29 },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Rmdir { node: 3, dir: 0 },
            Step::Create {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Touch { node: 4, path: 10 },
            Step::Create {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::Delete { node: 0, path: 6 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(1), Some(2), None, Some(1), Some(4)],
            },
            Step::Chmod { node: 6, path: 6 },
            Step::Delete { node: 4, path: 8 },
            Step::Settle { secs: 23 },
            Step::Chmod { node: 4, path: 8 },
            Step::Settle { secs: 14 },
            Step::Tier {
                a: 2,
                b: 6,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Settle { secs: 9 },
            Step::Offline { node: 6 },
            Step::MassDelete {
                node: 1,
                fraction: 93,
            },
            Step::Delete { node: 0, path: 5 },
            Step::Heal { a: 7, b: 0 },
            Step::Crash {
                node: 0,
                gap_secs: 35,
            },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Offline { node: 4 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(4), Some(2), Some(5), Some(2)],
            },
            Step::Symlink {
                node: 5,
                path: 8,
                target: 0,
            },
            Step::Create {
                node: 6,
                path: 5,
                content: 1,
            },
            Step::Heal { a: 5, b: 6 },
            Step::Create {
                node: 0,
                path: 6,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Online { node: 2 },
            Step::Delete { node: 5, path: 3 },
            Step::Delete { node: 3, path: 8 },
            Step::Symlink {
                node: 1,
                path: 0,
                target: 11,
            },
            Step::Offline { node: 5 },
            Step::Online { node: 6 },
            Step::Online { node: 5 },
            Step::Heal { a: 6, b: 4 },
            Step::Delete { node: 2, path: 3 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Delete { node: 2, path: 6 },
            Step::Rename {
                node: 3,
                from: 0,
                to: 3,
            },
            Step::Partition { a: 2, b: 6 },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 1,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Heal { a: 6, b: 2 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Touch { node: 1, path: 6 },
            Step::Partition { a: 3, b: 5 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 4,
            },
            Step::Crash {
                node: 7,
                gap_secs: 105,
            },
            Step::Heal { a: 4, b: 1 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 36,
            },
            Step::Touch { node: 6, path: 8 },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(5), Some(2)],
            },
            Step::Symlink {
                node: 0,
                path: 8,
                target: 0,
            },
            Step::Create {
                node: 6,
                path: 5,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 4,
            },
            Step::Touch { node: 3, path: 4 },
            Step::Settle { secs: 7 },
            Step::Delete { node: 0, path: 4 },
            Step::Partition { a: 2, b: 7 },
            Step::Create {
                node: 4,
                path: 2,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 2,
            },
            Step::Heal { a: 1, b: 5 },
            Step::Settle { secs: 14 },
            Step::Delete { node: 3, path: 9 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), Some(1)],
            },
            Step::Online { node: 1 },
            Step::Create {
                node: 2,
                path: 8,
                content: 3,
            },
            Step::Delete { node: 3, path: 1 },
            Step::Offline { node: 5 },
            Step::MassDelete {
                node: 1,
                fraction: 95,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 45,
                },
                delay_secs: 3,
            },
            Step::Offline { node: 5 },
            Step::Online { node: 5 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 2,
            },
            Step::Crash {
                node: 3,
                gap_secs: 44,
            },
            Step::Tier {
                a: 5,
                b: 4,
                tier: 0,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 57,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 59,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 3,
            },
            Step::Settle { secs: 3 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(5), Some(4), None, Some(4), Some(3), Some(4)],
            },
        ],
    );
}

/// The commit guard compared size and mtime for files and kind alone for
/// symlinks, so a chmod or a retarget between a losing version's record
/// and the commit that displaced its file went through, and the conflict
/// copy carried the user's later state under the loser's name (I4). The
/// guard is the scan fast path's predicate now (§7.5 step 6).
#[test]
fn a_chmod_under_a_pending_commit_is_changed_underneath() {
    passes(
        83,
        &[
            Step::Touch { node: 0, path: 2 },
            Step::Modify {
                node: 6,
                path: 1,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 7,
                content: 1,
            },
            Step::Heal { a: 3, b: 1 },
            Step::Offline { node: 1 },
            Step::Create {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::Heal { a: 2, b: 7 },
            Step::MassModify {
                node: 6,
                fraction: 72,
                content: 2,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(5), Some(1)],
            },
            Step::Chmod { node: 4, path: 4 },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Settle { secs: 24 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(5), None, Some(5), Some(1), Some(5)],
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(5), None, None, Some(3), Some(1), Some(4), Some(3)],
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(2), None, Some(5), Some(1)],
            },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Mkdir { node: 6, dir: 0 },
            Step::Chmod { node: 4, path: 3 },
            Step::Create {
                node: 2,
                path: 2,
                content: 5,
            },
            Step::MassModify {
                node: 2,
                fraction: 52,
                content: 2,
            },
            Step::Online { node: 6 },
            Step::Create {
                node: 7,
                path: 1,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Chmod { node: 5, path: 5 },
            Step::Partition { a: 0, b: 6 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(5), Some(1), None, Some(4), Some(3), Some(1), Some(2)],
            },
            Step::Heal { a: 6, b: 5 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 4,
            },
            Step::Partition { a: 1, b: 4 },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 0,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Touch { node: 5, path: 6 },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(1), Some(2), Some(2), Some(2), Some(5), Some(5)],
            },
            Step::Partition { a: 2, b: 2 },
            Step::Chmod { node: 2, path: 1 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), Some(3), Some(5), None],
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 32,
            },
            Step::Partition { a: 2, b: 5 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 59,
            },
            Step::Settle { secs: 34 },
            Step::Crash {
                node: 5,
                gap_secs: 28,
            },
            Step::Crash {
                node: 6,
                gap_secs: 75,
            },
            Step::Tier {
                a: 6,
                b: 1,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 2,
            },
            Step::Heal { a: 0, b: 7 },
            Step::MassModify {
                node: 2,
                fraction: 99,
                content: 1,
            },
            Step::Rmdir { node: 6, dir: 2 },
            Step::Modify {
                node: 1,
                path: 3,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 3,
            },
            Step::Partition { a: 0, b: 6 },
            Step::Tier {
                a: 5,
                b: 6,
                tier: 1,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 6,
                content: 1,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(5), None, Some(5), Some(4)],
            },
            Step::Tier {
                a: 4,
                b: 1,
                tier: 0,
            },
            Step::Modify {
                node: 0,
                path: 7,
                content: 3,
            },
            Step::Rmdir { node: 3, dir: 0 },
            Step::Online { node: 5 },
            Step::Rename {
                node: 3,
                from: 7,
                to: 0,
            },
            Step::Delete { node: 7, path: 7 },
            Step::Create {
                node: 6,
                path: 2,
                content: 1,
            },
            Step::Settle { secs: 8 },
            Step::Crash {
                node: 0,
                gap_secs: 7,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 4,
            },
            Step::Heal { a: 2, b: 7 },
            Step::Create {
                node: 6,
                path: 10,
                content: 2,
            },
            Step::Heal { a: 1, b: 5 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 6,
            },
            Step::Settle { secs: 30 },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Crash {
                node: 1,
                gap_secs: 88,
            },
            Step::Crash {
                node: 0,
                gap_secs: 111,
            },
            Step::Touch { node: 0, path: 0 },
            Step::Delete { node: 5, path: 6 },
            Step::Partition { a: 7, b: 3 },
            Step::Modify {
                node: 2,
                path: 2,
                content: 1,
            },
            Step::Rename {
                node: 1,
                from: 8,
                to: 11,
            },
            Step::Crash {
                node: 2,
                gap_secs: 50,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 3,
            },
            Step::Chmod { node: 2, path: 3 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 5,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Rmdir { node: 0, dir: 1 },
            Step::Offline { node: 4 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![None, Some(5), Some(5), Some(4)],
            },
            Step::Touch { node: 1, path: 3 },
            Step::Touch { node: 0, path: 10 },
            Step::Create {
                node: 6,
                path: 2,
                content: 1,
            },
            Step::Partition { a: 6, b: 6 },
            Step::Create {
                node: 1,
                path: 6,
                content: 4,
            },
            Step::Offline { node: 5 },
            Step::Modify {
                node: 6,
                path: 10,
                content: 2,
            },
            Step::Tier {
                a: 2,
                b: 5,
                tier: 1,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(2), Some(4), Some(4), None, Some(2), None],
            },
            Step::Chmod { node: 6, path: 2 },
            Step::Partition { a: 3, b: 4 },
            Step::Rename {
                node: 3,
                from: 4,
                to: 10,
            },
            Step::Partition { a: 3, b: 2 },
        ],
    );
}

/// Same class.
#[test]
fn a_chmod_under_a_pending_commit_is_changed_underneath_in_a_shorter_run() {
    passes(
        126,
        &[
            Step::Delete { node: 1, path: 5 },
            Step::Create {
                node: 0,
                path: 6,
                content: 1,
            },
            Step::Online { node: 4 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(1)],
            },
            Step::Crash {
                node: 0,
                gap_secs: 72,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 3,
            },
            Step::Heal { a: 0, b: 7 },
            Step::Create {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::Rename {
                node: 5,
                from: 4,
                to: 1,
            },
            Step::Rmdir { node: 2, dir: 2 },
            Step::Heal { a: 1, b: 4 },
            Step::Create {
                node: 0,
                path: 4,
                content: 1,
            },
            Step::Chmod { node: 4, path: 2 },
            Step::Create {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 3,
            },
            Step::Settle { secs: 13 },
            Step::Modify {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Rename {
                node: 1,
                from: 11,
                to: 11,
            },
            Step::Rename {
                node: 1,
                from: 6,
                to: 0,
            },
            Step::Offline { node: 0 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 5,
                path: 8,
                content: 2,
            },
            Step::Partition { a: 5, b: 6 },
            Step::MassModify {
                node: 1,
                fraction: 98,
                content: 1,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 42,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 2,
            },
            Step::Partition { a: 1, b: 4 },
            Step::Delete { node: 3, path: 8 },
            Step::Chmod { node: 0, path: 7 },
            Step::Touch { node: 2, path: 5 },
            Step::Modify {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(4), None],
            },
            Step::Chmod { node: 5, path: 7 },
            Step::Touch { node: 4, path: 2 },
            Step::Chmod { node: 7, path: 0 },
            Step::Rename {
                node: 1,
                from: 10,
                to: 4,
            },
            Step::Online { node: 4 },
            Step::Offline { node: 2 },
            Step::Settle { secs: 32 },
            Step::Crash {
                node: 7,
                gap_secs: 106,
            },
            Step::Delete { node: 3, path: 4 },
            Step::Heal { a: 6, b: 3 },
            Step::Chmod { node: 7, path: 0 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(1), Some(3), Some(1)],
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
        ],
    );
}

/// Same class.
#[test]
fn a_retarget_under_a_pending_commit_is_changed_underneath() {
    passes(
        314,
        &[
            Step::Modify {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 9,
            },
            Step::Chmod { node: 2, path: 11 },
            Step::Crash {
                node: 0,
                gap_secs: 87,
            },
            Step::Symlink {
                node: 1,
                path: 11,
                target: 9,
            },
            Step::Rename {
                node: 1,
                from: 0,
                to: 8,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Partition { a: 2, b: 1 },
            Step::Modify {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 4,
            },
            Step::Partition { a: 2, b: 1 },
            Step::Symlink {
                node: 4,
                path: 2,
                target: 1,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 54,
            },
            Step::Chmod { node: 6, path: 10 },
            Step::Online { node: 4 },
            Step::Delete { node: 5, path: 6 },
            Step::MassDelete {
                node: 6,
                fraction: 95,
            },
            Step::Crash {
                node: 0,
                gap_secs: 29,
            },
            Step::Rmdir { node: 0, dir: 0 },
            Step::Modify {
                node: 4,
                path: 9,
                content: 3,
            },
            Step::Touch { node: 2, path: 9 },
            Step::Heal { a: 0, b: 5 },
            Step::Touch { node: 5, path: 0 },
            Step::Delete { node: 6, path: 6 },
            Step::Settle { secs: 37 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 5,
            },
            Step::Rename {
                node: 5,
                from: 5,
                to: 0,
            },
            Step::Partition { a: 6, b: 5 },
            Step::Everywhere {
                path: 10,
                contents: vec![None, Some(1)],
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), None, Some(1), Some(1), None],
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 27 },
            Step::Create {
                node: 5,
                path: 10,
                content: 4,
            },
            Step::Heal { a: 5, b: 6 },
            Step::Mkdir { node: 6, dir: 0 },
            Step::Delete { node: 4, path: 10 },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Create {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 7,
                content: 3,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(2), Some(1)],
            },
            Step::Heal { a: 1, b: 5 },
            Step::Create {
                node: 4,
                path: 5,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 5,
                to: 10,
            },
            Step::Create {
                node: 1,
                path: 3,
                content: 5,
            },
            Step::Settle { secs: 29 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::MassModify {
                node: 4,
                fraction: 60,
                content: 3,
            },
            Step::Settle { secs: 16 },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 0,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(2), None, Some(3), Some(3), Some(1), Some(1)],
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 2,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 1,
            },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Everywhere {
                path: 4,
                contents: vec![None, Some(4), None, Some(1)],
            },
            Step::Offline { node: 5 },
            Step::Delete { node: 3, path: 5 },
            Step::Create {
                node: 1,
                path: 10,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 1,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 24,
            },
            Step::Chmod { node: 4, path: 2 },
            Step::Create {
                node: 1,
                path: 1,
                content: 5,
            },
            Step::Chmod { node: 1, path: 5 },
            Step::MassModify {
                node: 4,
                fraction: 68,
                content: 5,
            },
            Step::Symlink {
                node: 5,
                path: 5,
                target: 1,
            },
            Step::Create {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Partition { a: 2, b: 4 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 11,
                content: 3,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 44,
            },
            Step::Create {
                node: 6,
                path: 3,
                content: 1,
            },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 5,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 1,
            },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Partition { a: 5, b: 3 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 2,
                content: 4,
            },
            Step::Delete { node: 1, path: 7 },
            Step::Heal { a: 5, b: 2 },
            Step::Tier {
                a: 0,
                b: 3,
                tier: 1,
            },
            Step::Touch { node: 7, path: 8 },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Heal { a: 4, b: 5 },
            Step::Online { node: 0 },
            Step::Modify {
                node: 7,
                path: 7,
                content: 3,
            },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 0,
            },
            Step::Offline { node: 0 },
            Step::Rmdir { node: 7, dir: 2 },
            Step::Crash {
                node: 5,
                gap_secs: 81,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 2,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Rename {
                node: 1,
                from: 1,
                to: 8,
            },
            Step::Offline { node: 7 },
            Step::Partition { a: 1, b: 0 },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Heal { a: 7, b: 3 },
            Step::MassDelete {
                node: 6,
                fraction: 58,
            },
            Step::Crash {
                node: 5,
                gap_secs: 64,
            },
            Step::Rename {
                node: 6,
                from: 2,
                to: 8,
            },
            Step::Symlink {
                node: 0,
                path: 7,
                target: 0,
            },
        ],
    );
}

/// Same class.
#[test]
fn a_retarget_under_a_pending_commit_is_changed_underneath_in_another_run() {
    passes(
        491,
        &[
            Step::Touch { node: 2, path: 11 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Delete { node: 7, path: 3 },
            Step::Create {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(5), None, Some(5), Some(5)],
            },
            Step::Tier {
                a: 6,
                b: 6,
                tier: 1,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Settle { secs: 9 },
            Step::MassDelete {
                node: 2,
                fraction: 73,
            },
            Step::Rename {
                node: 2,
                from: 10,
                to: 11,
            },
            Step::Delete { node: 1, path: 2 },
            Step::Partition { a: 7, b: 6 },
            Step::Touch { node: 7, path: 11 },
            Step::Modify {
                node: 0,
                path: 4,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 5,
                content: 4,
            },
            Step::Offline { node: 6 },
            Step::Online { node: 3 },
            Step::Touch { node: 5, path: 5 },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 8,
                },
                delay_secs: 12,
            },
            Step::Delete { node: 7, path: 8 },
            Step::Delete { node: 6, path: 1 },
            Step::Crash {
                node: 3,
                gap_secs: 7,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 1,
            },
            Step::Offline { node: 4 },
            Step::Rmdir { node: 7, dir: 2 },
            Step::Heal { a: 3, b: 5 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 52,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, Some(1), None, Some(5)],
            },
            Step::Delete { node: 5, path: 11 },
            Step::Rename {
                node: 6,
                from: 1,
                to: 3,
            },
            Step::Offline { node: 6 },
            Step::MassModify {
                node: 0,
                fraction: 75,
                content: 2,
            },
            Step::Online { node: 0 },
            Step::Create {
                node: 3,
                path: 5,
                content: 5,
            },
            Step::Offline { node: 6 },
            Step::Delete { node: 0, path: 11 },
            Step::Offline { node: 7 },
            Step::Partition { a: 0, b: 0 },
            Step::Settle { secs: 22 },
            Step::Crash {
                node: 4,
                gap_secs: 40,
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 0, b: 4 },
            Step::Mkdir { node: 3, dir: 0 },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 42,
            },
            Step::Online { node: 4 },
            Step::Create {
                node: 3,
                path: 7,
                content: 1,
            },
            Step::Touch { node: 3, path: 8 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 3,
            },
            Step::Offline { node: 6 },
            Step::Delete { node: 6, path: 10 },
            Step::Crash {
                node: 2,
                gap_secs: 104,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Create {
                node: 7,
                path: 6,
                content: 3,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Rename {
                node: 5,
                from: 6,
                to: 11,
            },
            Step::Online { node: 5 },
            Step::Online { node: 5 },
            Step::Heal { a: 5, b: 6 },
            Step::Rename {
                node: 4,
                from: 1,
                to: 2,
            },
            Step::Settle { secs: 24 },
            Step::Rename {
                node: 5,
                from: 10,
                to: 0,
            },
            Step::Crash {
                node: 1,
                gap_secs: 93,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 21,
            },
            Step::Heal { a: 2, b: 1 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 11,
                content: 2,
            },
            Step::Heal { a: 7, b: 4 },
            Step::Create {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Online { node: 2 },
            Step::Create {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 18 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Touch { node: 1, path: 11 },
            Step::Delete { node: 7, path: 8 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Crash {
                node: 6,
                gap_secs: 69,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Chmod { node: 2, path: 2 },
            Step::Partition { a: 2, b: 2 },
            Step::Tier {
                a: 2,
                b: 0,
                tier: 1,
            },
            Step::Rename {
                node: 6,
                from: 4,
                to: 10,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 5,
                content: 2,
            },
            Step::Chmod { node: 3, path: 1 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(5), Some(4), Some(3), Some(5), Some(2), None, None],
            },
            Step::Delete { node: 0, path: 0 },
            Step::Crash {
                node: 4,
                gap_secs: 1,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![None, Some(4), Some(3), Some(1), Some(1)],
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 11,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 4,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(1), Some(2), Some(3), Some(4), Some(1)],
            },
            Step::Chmod { node: 1, path: 6 },
            Step::Modify {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::MassDelete {
                node: 6,
                fraction: 77,
            },
            Step::Partition { a: 1, b: 7 },
            Step::Offline { node: 3 },
            Step::Create {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::Chmod { node: 6, path: 2 },
            Step::Delete { node: 5, path: 8 },
            Step::Heal { a: 6, b: 2 },
            Step::Create {
                node: 0,
                path: 0,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 0,
                },
                delay_secs: 0,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 4,
            },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 7,
            },
            Step::Tier {
                a: 0,
                b: 5,
                tier: 1,
            },
            Step::Settle { secs: 7 },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 1,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 26,
                },
                delay_secs: 11,
            },
            Step::Partition { a: 5, b: 4 },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 10,
            },
            Step::Modify {
                node: 5,
                path: 8,
                content: 5,
            },
        ],
    );
}

/// A concurrent entry joining a held item had its version quarantined but
/// not its entry, so `deny`'s bump folded the version into its vector while
/// the stamp it started from ignored it: the bump dominated a metadata-only
/// version and ranked below it. Its tombstone then lost to that content on
/// one node and won on another, and one vector carried both (I7). Every
/// quarantined version now carries its stamp, and the bump starts above
/// the largest of them.
#[test]
fn a_joined_versions_stamp_counts_for_the_deny_bump() {
    passes(
        938,
        &[
            Step::Partition { a: 5, b: 1 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 4,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![None, None, Some(3), None],
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 5,
            },
            Step::Touch { node: 2, path: 5 },
            Step::Create {
                node: 6,
                path: 2,
                content: 5,
            },
            Step::Symlink {
                node: 1,
                path: 4,
                target: 6,
            },
            Step::Touch { node: 4, path: 1 },
            Step::Modify {
                node: 6,
                path: 4,
                content: 2,
            },
            Step::Tier {
                a: 7,
                b: 4,
                tier: 0,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Tier {
                a: 0,
                b: 7,
                tier: 0,
            },
            Step::Settle { secs: 9 },
            Step::Modify {
                node: 1,
                path: 3,
                content: 2,
            },
            Step::Heal { a: 1, b: 4 },
            Step::Heal { a: 2, b: 6 },
            Step::Create {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Create {
                node: 0,
                path: 1,
                content: 1,
            },
            Step::Delete { node: 0, path: 8 },
            Step::Online { node: 4 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 36,
            },
            Step::Modify {
                node: 7,
                path: 5,
                content: 4,
            },
            Step::Rmdir { node: 4, dir: 2 },
            Step::Delete { node: 3, path: 3 },
            Step::Offline { node: 5 },
            Step::Modify {
                node: 4,
                path: 8,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 17 },
            Step::Heal { a: 7, b: 6 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 40,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 5,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(2), Some(5), Some(4), None, Some(3), None],
            },
            Step::Settle { secs: 12 },
            Step::MassDelete {
                node: 2,
                fraction: 59,
            },
            Step::Delete { node: 5, path: 0 },
            Step::Heal { a: 1, b: 1 },
            Step::Modify {
                node: 1,
                path: 6,
                content: 5,
            },
            Step::Heal { a: 4, b: 0 },
            Step::Create {
                node: 6,
                path: 4,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Create {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Offline { node: 4 },
            Step::Create {
                node: 5,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 2,
            },
            Step::Delete { node: 4, path: 8 },
            Step::Delete { node: 3, path: 0 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 7,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 27,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 1,
            },
            Step::Create {
                node: 6,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 5, b: 5 },
            Step::Touch { node: 5, path: 8 },
            Step::MassDelete {
                node: 7,
                fraction: 98,
            },
            Step::Delete { node: 6, path: 0 },
            Step::MassModify {
                node: 2,
                fraction: 61,
                content: 2,
            },
            Step::Modify {
                node: 4,
                path: 9,
                content: 4,
            },
            Step::Delete { node: 3, path: 1 },
            Step::Partition { a: 3, b: 4 },
            Step::MassModify {
                node: 2,
                fraction: 99,
                content: 4,
            },
            Step::Settle { secs: 18 },
            Step::Create {
                node: 4,
                path: 1,
                content: 5,
            },
            Step::Rmdir { node: 0, dir: 1 },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Heal { a: 6, b: 6 },
            Step::Create {
                node: 7,
                path: 7,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 1,
                content: 2,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Heal { a: 3, b: 4 },
            Step::Modify {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::Offline { node: 5 },
            Step::Settle { secs: 39 },
            Step::Crash {
                node: 2,
                gap_secs: 113,
            },
            Step::Modify {
                node: 7,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 6,
                content: 1,
            },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Modify {
                node: 6,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 0, b: 1 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Modify {
                node: 2,
                path: 5,
                content: 3,
            },
            Step::Delete { node: 3, path: 7 },
            Step::Create {
                node: 6,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 5,
            },
            Step::Delete { node: 4, path: 7 },
            Step::MassDelete {
                node: 5,
                fraction: 58,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 0,
            },
            Step::Rename {
                node: 1,
                from: 10,
                to: 9,
            },
            Step::Rename {
                node: 1,
                from: 11,
                to: 5,
            },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Partition { a: 4, b: 3 },
            Step::Create {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), Some(3)],
            },
            Step::Settle { secs: 31 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 23,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(2), Some(3), Some(4), Some(1)],
            },
            Step::Chmod { node: 0, path: 5 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 11,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![None, Some(1), None, Some(3), Some(4)],
            },
            Step::Delete { node: 5, path: 1 },
            Step::Modify {
                node: 4,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Mkdir { node: 4, dir: 0 },
            Step::Create {
                node: 0,
                path: 6,
                content: 3,
            },
            Step::Delete { node: 0, path: 8 },
            Step::Modify {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::MassDelete {
                node: 3,
                fraction: 72,
            },
            Step::Symlink {
                node: 2,
                path: 8,
                target: 6,
            },
            Step::Heal { a: 3, b: 5 },
            Step::Heal { a: 0, b: 2 },
            Step::Tier {
                a: 3,
                b: 7,
                tier: 2,
            },
            Step::Delete { node: 0, path: 5 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(1), None],
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 7,
                content: 3,
            },
            Step::Offline { node: 0 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Delete { node: 6, path: 10 },
            Step::Create {
                node: 5,
                path: 1,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(3), Some(3), Some(2), None, Some(5)],
            },
            Step::Modify {
                node: 1,
                path: 10,
                content: 4,
            },
            Step::Partition { a: 2, b: 5 },
            Step::Modify {
                node: 1,
                path: 10,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Touch { node: 4, path: 1 },
            Step::Modify {
                node: 6,
                path: 5,
                content: 5,
            },
            Step::Chmod { node: 4, path: 4 },
            Step::Delete { node: 0, path: 8 },
            Step::Settle { secs: 38 },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 9,
            },
            Step::MassDelete {
                node: 2,
                fraction: 58,
            },
            Step::Chmod { node: 1, path: 2 },
            Step::Partition { a: 0, b: 0 },
        ],
    );
}

/// `revert` moved the file to trash and restored the announced record, and
/// the refetch's `Write` then expected that record's shape on disk; the
/// path was absent, so the guard failed, the want was deferred, the next
/// bracket end tombstoned the path, and everything that later arrived
/// there stayed deferred (quiescence). A restoring want's commit now
/// expects the path to be absent (§8.3 step 2).
#[test]
fn a_reverts_refetch_expects_the_path_it_trashed_to_be_absent() {
    passes(
        650,
        &[
            Step::Heal { a: 2, b: 5 },
            Step::Settle { secs: 19 },
            Step::Heal { a: 2, b: 4 },
            Step::MassModify {
                node: 6,
                fraction: 74,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Crash {
                node: 0,
                gap_secs: 101,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 3,
            },
            Step::Rename {
                node: 1,
                from: 8,
                to: 9,
            },
            Step::Delete { node: 7, path: 4 },
            Step::Create {
                node: 0,
                path: 4,
                content: 3,
            },
            Step::Delete { node: 1, path: 8 },
            Step::Symlink {
                node: 7,
                path: 2,
                target: 6,
            },
            Step::Online { node: 3 },
            Step::Create {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Offline { node: 0 },
            Step::Settle { secs: 2 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Chmod { node: 5, path: 9 },
            Step::Delete { node: 0, path: 4 },
            Step::Create {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Online { node: 4 },
            Step::Settle { secs: 21 },
            Step::Rename {
                node: 1,
                from: 0,
                to: 4,
            },
            Step::Partition { a: 7, b: 5 },
            Step::Rename {
                node: 5,
                from: 8,
                to: 1,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Delete { node: 0, path: 6 },
            Step::Offline { node: 2 },
            Step::Offline { node: 7 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 1,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(4), Some(5), Some(2), Some(1), Some(5)],
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 55,
            },
            Step::Crash {
                node: 2,
                gap_secs: 69,
            },
            Step::MassDelete {
                node: 1,
                fraction: 85,
            },
            Step::Modify {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Heal { a: 0, b: 3 },
            Step::Delete { node: 2, path: 11 },
            Step::Partition { a: 4, b: 2 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(4)],
            },
            Step::Settle { secs: 15 },
            Step::Create {
                node: 6,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Heal { a: 5, b: 2 },
            Step::Partition { a: 5, b: 7 },
            Step::Delete { node: 0, path: 3 },
            Step::Offline { node: 4 },
            Step::Create {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 2,
            },
            Step::Heal { a: 1, b: 3 },
            Step::Create {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 9,
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Settle { secs: 32 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 32,
            },
            Step::Chmod { node: 6, path: 1 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Partition { a: 4, b: 4 },
            Step::Modify {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 17,
            },
            Step::Settle { secs: 6 },
            Step::Offline { node: 5 },
            Step::Partition { a: 7, b: 5 },
            Step::Modify {
                node: 5,
                path: 9,
                content: 3,
            },
            Step::Create {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 9,
            },
            Step::Create {
                node: 4,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Delete { node: 6, path: 6 },
            Step::Create {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Heal { a: 3, b: 0 },
            Step::Symlink {
                node: 6,
                path: 9,
                target: 0,
            },
            Step::Settle { secs: 1 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 3,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Settle { secs: 33 },
            Step::Delete { node: 7, path: 6 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Crash {
                node: 5,
                gap_secs: 119,
            },
            Step::Touch { node: 6, path: 1 },
            Step::Delete { node: 6, path: 9 },
            Step::Heal { a: 4, b: 1 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Partition { a: 2, b: 6 },
            Step::Partition { a: 1, b: 4 },
            Step::Modify {
                node: 7,
                path: 7,
                content: 3,
            },
            Step::Delete { node: 6, path: 1 },
            Step::Online { node: 1 },
            Step::Rmdir { node: 3, dir: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 24,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Touch { node: 6, path: 0 },
            Step::Delete { node: 1, path: 9 },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Online { node: 0 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 35,
            },
            Step::Create {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Partition { a: 3, b: 5 },
            Step::Create {
                node: 6,
                path: 11,
                content: 4,
            },
            Step::Delete { node: 1, path: 1 },
            Step::Create {
                node: 5,
                path: 10,
                content: 2,
            },
            Step::Rmdir { node: 1, dir: 0 },
            Step::Partition { a: 2, b: 7 },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 1,
                path: 6,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 2,
                content: 4,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 1,
            },
            Step::Settle { secs: 21 },
            Step::Create {
                node: 5,
                path: 1,
                content: 5,
            },
            Step::Settle { secs: 9 },
            Step::Modify {
                node: 6,
                path: 1,
                content: 5,
            },
            Step::Offline { node: 1 },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Delete { node: 0, path: 3 },
            Step::Modify {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Offline { node: 2 },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 0,
            },
            Step::Modify {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![None, None, Some(3), None, Some(3), Some(3)],
            },
            Step::Delete { node: 6, path: 2 },
            Step::Create {
                node: 6,
                path: 6,
                content: 4,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 49,
            },
            Step::Create {
                node: 3,
                path: 7,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 0,
                content: 5,
            },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Crash {
                node: 6,
                gap_secs: 43,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 2,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 24,
            },
            Step::Modify {
                node: 3,
                path: 6,
                content: 3,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Heal { a: 2, b: 3 },
            Step::Rename {
                node: 5,
                from: 0,
                to: 1,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 3,
            },
            Step::Delete { node: 2, path: 2 },
            Step::Delete { node: 7, path: 9 },
            Step::Heal { a: 4, b: 0 },
            Step::Rename {
                node: 6,
                from: 7,
                to: 3,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), Some(3), Some(1), Some(2), None, Some(3)],
            },
            Step::Heal { a: 1, b: 1 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(4), Some(3), Some(5), Some(4), Some(5)],
            },
            Step::MassModify {
                node: 4,
                fraction: 87,
                content: 1,
            },
            Step::Delete { node: 5, path: 0 },
            Step::Chmod { node: 0, path: 7 },
            Step::Delete { node: 0, path: 1 },
            Step::Partition { a: 3, b: 4 },
            Step::Heal { a: 6, b: 6 },
            Step::Create {
                node: 6,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 4,
            },
            Step::Touch { node: 5, path: 5 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 5 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Delete { node: 6, path: 3 },
            Step::Online { node: 5 },
            Step::Crash {
                node: 3,
                gap_secs: 109,
            },
            Step::Settle { secs: 32 },
            Step::Settle { secs: 16 },
            Step::Heal { a: 4, b: 0 },
            Step::Rmdir { node: 5, dir: 0 },
            Step::Chmod { node: 7, path: 10 },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 44,
            },
            Step::Heal { a: 3, b: 6 },
            Step::Modify {
                node: 6,
                path: 6,
                content: 1,
            },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Create {
                node: 6,
                path: 10,
                content: 4,
            },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 1,
            },
            Step::Chmod { node: 4, path: 2 },
            Step::MassModify {
                node: 3,
                fraction: 62,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 4,
            },
            Step::Symlink {
                node: 5,
                path: 2,
                target: 2,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 1,
            },
            Step::Tier {
                a: 2,
                b: 7,
                tier: 0,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 2,
            },
            Step::Settle { secs: 4 },
            Step::Delete { node: 7, path: 3 },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 38,
            },
            Step::Create {
                node: 6,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, Some(1), None, Some(5), Some(3)],
            },
            Step::Partition { a: 1, b: 4 },
            Step::Online { node: 4 },
            Step::Delete { node: 7, path: 9 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 41,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(4), Some(5), Some(2), Some(4)],
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 51,
            },
            Step::Heal { a: 1, b: 6 },
            Step::Create {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 3,
            },
            Step::MassModify {
                node: 7,
                fraction: 74,
                content: 4,
            },
            Step::Rename {
                node: 5,
                from: 11,
                to: 3,
            },
            Step::Partition { a: 6, b: 1 },
            Step::Tier {
                a: 2,
                b: 0,
                tier: 0,
            },
            Step::Delete { node: 2, path: 0 },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 9,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Rename {
                node: 0,
                from: 0,
                to: 1,
            },
            Step::Partition { a: 1, b: 1 },
            Step::Create {
                node: 1,
                path: 1,
                content: 3,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(3), None],
            },
            Step::Heal { a: 6, b: 7 },
            Step::Settle { secs: 30 },
            Step::Heal { a: 0, b: 4 },
            Step::Settle { secs: 38 },
            Step::Online { node: 3 },
            Step::Partition { a: 4, b: 1 },
            Step::Settle { secs: 6 },
            Step::Delete { node: 7, path: 6 },
            Step::Create {
                node: 0,
                path: 7,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 57,
            },
            Step::Settle { secs: 30 },
            Step::MassDelete {
                node: 2,
                fraction: 87,
            },
        ],
    );
}

/// A crash after a rename left a fetched file on disk the index never
/// learnt of; the scan made it a local add, a remote version became a
/// conflict want `M` folding that add in, a later remote version was
/// deferred as concurrent with `M`, and `revert` then discarded the add.
/// `M` stayed, describing a merge that never happened, with content no
/// peer held; every bracket end re-deferred the later version behind it
/// (quiescence). Revert now re-derives the want from the entry as
/// received, and the deferred version dominates and replaces it.
#[test]
fn a_want_at_a_reverted_path_is_rederived_from_the_received_entry() {
    passes(
        650,
        &[
            Step::Heal { a: 2, b: 5 },
            Step::Settle { secs: 19 },
            Step::Heal { a: 2, b: 4 },
            Step::MassModify {
                node: 6,
                fraction: 74,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Crash {
                node: 0,
                gap_secs: 101,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 3,
            },
            Step::Rename {
                node: 1,
                from: 8,
                to: 9,
            },
            Step::Delete { node: 7, path: 4 },
            Step::Online { node: 3 },
            Step::Create {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Offline { node: 0 },
            Step::Settle { secs: 2 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Chmod { node: 5, path: 9 },
            Step::Delete { node: 0, path: 4 },
            Step::Create {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Online { node: 4 },
            Step::Settle { secs: 21 },
            Step::Rename {
                node: 1,
                from: 0,
                to: 4,
            },
            Step::Partition { a: 7, b: 5 },
            Step::Rename {
                node: 5,
                from: 8,
                to: 1,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Delete { node: 0, path: 6 },
            Step::Offline { node: 2 },
            Step::Offline { node: 7 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Crash {
                node: 2,
                gap_secs: 69,
            },
            Step::MassDelete {
                node: 1,
                fraction: 85,
            },
            Step::Modify {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Heal { a: 0, b: 3 },
            Step::Delete { node: 2, path: 11 },
            Step::Partition { a: 4, b: 2 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(4)],
            },
            Step::Settle { secs: 15 },
            Step::Create {
                node: 6,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Heal { a: 5, b: 2 },
            Step::Partition { a: 5, b: 7 },
            Step::Delete { node: 0, path: 3 },
            Step::Offline { node: 4 },
            Step::Create {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 2,
            },
            Step::Heal { a: 1, b: 3 },
            Step::Create {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 9,
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Settle { secs: 32 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 32,
            },
            Step::Chmod { node: 6, path: 1 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Partition { a: 4, b: 4 },
            Step::Modify {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 17,
            },
            Step::Settle { secs: 6 },
            Step::Offline { node: 5 },
            Step::Partition { a: 7, b: 5 },
            Step::Modify {
                node: 5,
                path: 9,
                content: 3,
            },
            Step::Create {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 9,
            },
            Step::Create {
                node: 4,
                path: 6,
                content: 2,
            },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Modify {
                node: 6,
                path: 6,
                content: 5,
            },
            Step::Offline { node: 7 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(2), Some(3), None, None, Some(5), Some(1)],
            },
            Step::Heal { a: 1, b: 6 },
            Step::Partition { a: 5, b: 4 },
            Step::Tier {
                a: 5,
                b: 7,
                tier: 0,
            },
            Step::Create {
                node: 6,
                path: 7,
                content: 3,
            },
            Step::MassDelete {
                node: 5,
                fraction: 99,
            },
            Step::Create {
                node: 0,
                path: 11,
                content: 4,
            },
            Step::Symlink {
                node: 1,
                path: 5,
                target: 1,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Delete { node: 6, path: 6 },
            Step::Create {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Heal { a: 3, b: 0 },
            Step::Symlink {
                node: 6,
                path: 9,
                target: 0,
            },
            Step::Settle { secs: 1 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 3,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Settle { secs: 33 },
            Step::Delete { node: 7, path: 6 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Crash {
                node: 5,
                gap_secs: 119,
            },
            Step::Touch { node: 6, path: 1 },
            Step::Delete { node: 6, path: 9 },
            Step::Heal { a: 4, b: 1 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Partition { a: 2, b: 6 },
            Step::Partition { a: 1, b: 4 },
            Step::Modify {
                node: 7,
                path: 7,
                content: 3,
            },
            Step::Delete { node: 6, path: 1 },
            Step::Online { node: 1 },
            Step::Rmdir { node: 3, dir: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 24,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Touch { node: 6, path: 0 },
            Step::Delete { node: 1, path: 9 },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Online { node: 0 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 35,
            },
            Step::Create {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, Some(1), None, Some(5), Some(3)],
            },
            Step::MassDelete {
                node: 7,
                fraction: 69,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 21,
            },
            Step::Heal { a: 2, b: 2 },
        ],
    );
}

/// `revert` restored a conflict copy's record and refetched its content;
/// two peers that wanted that content asked while the file was still in
/// the trash, were refused, and went without source. The landing kept the
/// restored record's old `seq`, so it was never announced and the peers
/// never asked again (I1). The landing is now adopted with a new `seq`
/// like any other commit and announced at the next tick (§8.3 step 2).
#[test]
fn a_restores_landing_is_announced_to_peers_it_refused() {
    passes(
        5,
        &[
            Step::Rmdir { node: 4, dir: 1 },
            Step::Delete { node: 2, path: 1 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 52,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(2), Some(1), Some(2)],
            },
            Step::Offline { node: 1 },
            Step::Touch { node: 4, path: 4 },
            Step::Create {
                node: 3,
                path: 8,
                content: 1,
            },
            Step::Settle { secs: 1 },
            Step::Modify {
                node: 4,
                path: 9,
                content: 2,
            },
            Step::Symlink {
                node: 3,
                path: 3,
                target: 6,
            },
            Step::Tier {
                a: 1,
                b: 2,
                tier: 2,
            },
            Step::Rmdir { node: 4, dir: 0 },
            Step::Delete { node: 6, path: 3 },
            Step::Settle { secs: 30 },
            Step::Partition { a: 7, b: 2 },
            Step::Heal { a: 5, b: 2 },
            Step::Chmod { node: 5, path: 7 },
            Step::Create {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Crash {
                node: 3,
                gap_secs: 8,
            },
            Step::Crash {
                node: 5,
                gap_secs: 104,
            },
            Step::Touch { node: 2, path: 2 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 2,
            },
            Step::Online { node: 1 },
            Step::Heal { a: 0, b: 4 },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 1,
            },
            Step::Delete { node: 5, path: 11 },
            Step::Settle { secs: 26 },
            Step::Create {
                node: 5,
                path: 6,
                content: 5,
            },
            Step::Chmod { node: 0, path: 1 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 26,
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 2,
            },
            Step::Heal { a: 1, b: 2 },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Settle { secs: 30 },
            Step::Online { node: 5 },
            Step::MassDelete {
                node: 3,
                fraction: 51,
            },
            Step::MassDelete {
                node: 5,
                fraction: 80,
            },
            Step::Delete { node: 5, path: 1 },
            Step::Create {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Modify {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Chmod { node: 3, path: 2 },
            Step::Heal { a: 1, b: 0 },
            Step::Create {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Tier {
                a: 5,
                b: 5,
                tier: 2,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 1,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(5), Some(4), Some(1), Some(2), Some(3)],
            },
            Step::Modify {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Touch { node: 2, path: 5 },
            Step::Online { node: 2 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), Some(5), Some(3), None],
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Chmod { node: 6, path: 7 },
            Step::Offline { node: 0 },
            Step::MassDelete {
                node: 2,
                fraction: 65,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 20 },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Offline { node: 5 },
            Step::Settle { secs: 5 },
            Step::Delete { node: 6, path: 6 },
            Step::Chmod { node: 3, path: 5 },
            Step::Tier {
                a: 2,
                b: 7,
                tier: 1,
            },
            Step::Delete { node: 6, path: 3 },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 1,
            },
            Step::MassModify {
                node: 5,
                fraction: 84,
                content: 3,
            },
            Step::Heal { a: 3, b: 0 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 5,
                content: 3,
            },
            Step::Offline { node: 5 },
            Step::Offline { node: 4 },
            Step::Partition { a: 6, b: 3 },
            Step::MassDelete {
                node: 4,
                fraction: 77,
            },
            Step::Settle { secs: 30 },
            Step::Tier {
                a: 6,
                b: 0,
                tier: 2,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(5), Some(4), Some(4), Some(3), Some(4)],
            },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(1), Some(2)],
            },
            Step::Rename {
                node: 0,
                from: 11,
                to: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(4), None, None, Some(5), Some(1), None],
            },
            Step::Create {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Offline { node: 6 },
            Step::Online { node: 7 },
            Step::Partition { a: 0, b: 3 },
            Step::Modify {
                node: 5,
                path: 10,
                content: 2,
            },
            Step::Delete { node: 1, path: 3 },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 2,
            },
            Step::Modify {
                node: 0,
                path: 11,
                content: 1,
            },
            Step::Tier {
                a: 1,
                b: 5,
                tier: 1,
            },
            Step::Online { node: 5 },
            Step::Tier {
                a: 3,
                b: 7,
                tier: 1,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 3,
            },
            Step::Crash {
                node: 7,
                gap_secs: 96,
            },
            Step::Partition { a: 0, b: 3 },
            Step::Offline { node: 1 },
            Step::Rename {
                node: 4,
                from: 6,
                to: 7,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), None, Some(5), Some(4), Some(3)],
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 1,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(4), Some(3), None, None, Some(2), None],
            },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 4,
            },
            Step::Partition { a: 6, b: 4 },
            Step::Delete { node: 7, path: 10 },
            Step::Everywhere {
                path: 11,
                contents: vec![None, None, Some(1), Some(2), Some(2)],
            },
            Step::Rename {
                node: 6,
                from: 4,
                to: 4,
            },
            Step::Chmod { node: 1, path: 5 },
            Step::Heal { a: 0, b: 4 },
            Step::Rename {
                node: 3,
                from: 4,
                to: 9,
            },
            Step::Partition { a: 0, b: 5 },
            Step::Settle { secs: 3 },
            Step::Delete { node: 1, path: 4 },
            Step::Modify {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 3,
            },
            Step::Touch { node: 5, path: 4 },
            Step::Create {
                node: 5,
                path: 10,
                content: 3,
            },
            Step::Symlink {
                node: 7,
                path: 9,
                target: 6,
            },
            Step::Offline { node: 1 },
            Step::Modify {
                node: 2,
                path: 7,
                content: 5,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Symlink {
                node: 5,
                path: 6,
                target: 9,
            },
            Step::Crash {
                node: 5,
                gap_secs: 56,
            },
            Step::Online { node: 4 },
            Step::Tier {
                a: 4,
                b: 3,
                tier: 2,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Chmod { node: 2, path: 4 },
            Step::Crash {
                node: 1,
                gap_secs: 66,
            },
            Step::Delete { node: 4, path: 5 },
            Step::Crash {
                node: 0,
                gap_secs: 72,
            },
            Step::Create {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Chmod { node: 6, path: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::Partition { a: 2, b: 6 },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 1,
                },
                delay_secs: 35,
            },
            Step::Partition { a: 1, b: 3 },
            Step::Chmod { node: 5, path: 8 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(4), Some(1), None, Some(2)],
            },
            Step::Chmod { node: 5, path: 1 },
            Step::Chmod { node: 2, path: 6 },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Chmod { node: 7, path: 8 },
            Step::Heal { a: 0, b: 6 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(5), Some(1), None, Some(5)],
            },
            Step::Partition { a: 4, b: 7 },
            Step::Create {
                node: 0,
                path: 1,
                content: 2,
            },
            Step::Delete { node: 4, path: 7 },
            Step::Delete { node: 6, path: 4 },
            Step::Rename {
                node: 2,
                from: 7,
                to: 0,
            },
            Step::Delete { node: 0, path: 3 },
        ],
    );
}

/// A user file appeared at a restored path while the refetch was in
/// flight, so the commit failed and was deferred carrying the mark; the
/// user deleted the file before any scan saw it. Absence at a marked path
/// is ignored, so nothing ever wanted the content again (quiescence). A
/// bracket end that does not see a marked path now wants its deferred
/// carriers again as they stand (§8.3). Shrunk against a build without
/// that rule; it fails there and passes with it.
#[test]
fn a_deferred_carrier_at_a_path_that_stays_absent_is_wanted_again() {
    passes(
        991,
        &[
            Step::Delete { node: 3, path: 3 },
            Step::Create {
                node: 4,
                path: 3,
                content: 2,
            },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 9,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 11,
            },
            Step::Settle { secs: 27 },
            Step::Partition { a: 4, b: 1 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 47,
            },
            Step::Symlink {
                node: 2,
                path: 3,
                target: 6,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 1,
            },
            Step::MassModify {
                node: 2,
                fraction: 84,
                content: 3,
            },
            Step::Delete { node: 1, path: 4 },
            Step::Heal { a: 6, b: 7 },
            Step::Create {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::Offline { node: 1 },
            Step::Create {
                node: 6,
                path: 8,
                content: 1,
            },
            Step::Crash {
                node: 3,
                gap_secs: 77,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Symlink {
                node: 4,
                path: 2,
                target: 3,
            },
            Step::Delete { node: 7, path: 4 },
            Step::MassModify {
                node: 7,
                fraction: 83,
                content: 3,
            },
            Step::Crash {
                node: 1,
                gap_secs: 85,
            },
            Step::Online { node: 5 },
            Step::Delete { node: 7, path: 0 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(4), Some(4)],
            },
            Step::Modify {
                node: 6,
                path: 1,
                content: 3,
            },
            Step::Rmdir { node: 4, dir: 0 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 8,
                content: 4,
            },
            Step::Settle { secs: 20 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Touch { node: 2, path: 0 },
            Step::Heal { a: 5, b: 6 },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(5), None, Some(4), None, Some(5), Some(5)],
            },
            Step::Modify {
                node: 4,
                path: 7,
                content: 1,
            },
            Step::Crash {
                node: 0,
                gap_secs: 37,
            },
            Step::Offline { node: 7 },
            Step::MassDelete {
                node: 0,
                fraction: 85,
            },
            Step::Create {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Touch { node: 2, path: 6 },
            Step::Partition { a: 5, b: 6 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 0,
            },
            Step::Crash {
                node: 4,
                gap_secs: 81,
            },
            Step::Heal { a: 0, b: 3 },
            Step::Crash {
                node: 3,
                gap_secs: 80,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 3,
            },
            Step::Heal { a: 3, b: 2 },
            Step::MassModify {
                node: 6,
                fraction: 97,
                content: 2,
            },
            Step::Touch { node: 3, path: 0 },
            Step::Settle { secs: 37 },
            Step::Crash {
                node: 6,
                gap_secs: 100,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 16,
            },
            Step::Heal { a: 4, b: 0 },
            Step::Settle { secs: 15 },
            Step::Symlink {
                node: 1,
                path: 0,
                target: 11,
            },
            Step::Offline { node: 3 },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(2), None],
            },
            Step::Heal { a: 1, b: 3 },
            Step::Rename {
                node: 0,
                from: 8,
                to: 0,
            },
            Step::Rename {
                node: 7,
                from: 9,
                to: 3,
            },
            Step::MassModify {
                node: 7,
                fraction: 93,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 3,
                content: 4,
            },
            Step::Offline { node: 6 },
            Step::Touch { node: 0, path: 1 },
            Step::Create {
                node: 4,
                path: 8,
                content: 2,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 16,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 2,
            },
            Step::MassModify {
                node: 2,
                fraction: 60,
                content: 5,
            },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 0,
            },
            Step::Offline { node: 5 },
            Step::Create {
                node: 7,
                path: 10,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 5,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 6,
                content: 3,
            },
            Step::Delete { node: 2, path: 10 },
            Step::Touch { node: 2, path: 8 },
            Step::Crash {
                node: 2,
                gap_secs: 11,
            },
            Step::Create {
                node: 3,
                path: 8,
                content: 4,
            },
            Step::Crash {
                node: 2,
                gap_secs: 43,
            },
            Step::Symlink {
                node: 2,
                path: 4,
                target: 6,
            },
            Step::Symlink {
                node: 7,
                path: 10,
                target: 3,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 2,
            },
            Step::Settle { secs: 32 },
            Step::Delete { node: 5, path: 7 },
            Step::Partition { a: 0, b: 5 },
            Step::Heal { a: 2, b: 4 },
            Step::Offline { node: 7 },
            Step::Online { node: 3 },
            Step::Chmod { node: 5, path: 2 },
            Step::Tier {
                a: 3,
                b: 7,
                tier: 0,
            },
            Step::Rmdir { node: 0, dir: 1 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 9,
            },
            Step::Partition { a: 2, b: 7 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 38,
            },
            Step::Settle { secs: 21 },
            Step::MassModify {
                node: 7,
                fraction: 66,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 3,
            },
            Step::Online { node: 1 },
            Step::Settle { secs: 25 },
            Step::Create {
                node: 4,
                path: 6,
                content: 4,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Create {
                node: 6,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 8,
                content: 2,
            },
            Step::Settle { secs: 35 },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 2,
            },
            Step::Offline { node: 5 },
            Step::Online { node: 1 },
            Step::Tier {
                a: 6,
                b: 6,
                tier: 1,
            },
            Step::Online { node: 1 },
            Step::Partition { a: 0, b: 2 },
            Step::Rename {
                node: 2,
                from: 4,
                to: 9,
            },
            Step::Settle { secs: 23 },
            Step::Create {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Create {
                node: 6,
                path: 1,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Chmod { node: 1, path: 5 },
            Step::Modify {
                node: 4,
                path: 9,
                content: 5,
            },
            Step::Settle { secs: 4 },
            Step::Modify {
                node: 6,
                path: 4,
                content: 3,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(2), Some(2), None],
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Tier {
                a: 4,
                b: 2,
                tier: 1,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 20,
            },
            Step::Partition { a: 7, b: 5 },
            Step::Online { node: 5 },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 23,
            },
            Step::Create {
                node: 0,
                path: 6,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Settle { secs: 2 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 5,
            },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 1,
            },
            Step::Create {
                node: 3,
                path: 7,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 4,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Settle { secs: 34 },
            Step::Settle { secs: 19 },
            Step::Settle { secs: 23 },
            Step::Modify {
                node: 2,
                path: 8,
                content: 3,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 17,
            },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 1,
            },
            Step::MassModify {
                node: 0,
                fraction: 55,
                content: 1,
            },
            Step::Heal { a: 6, b: 1 },
            Step::Mkdir { node: 1, dir: 0 },
            Step::Partition { a: 7, b: 5 },
            Step::Create {
                node: 0,
                path: 2,
                content: 2,
            },
            Step::Chmod { node: 3, path: 6 },
            Step::Heal { a: 7, b: 6 },
            Step::Settle { secs: 37 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Heal { a: 3, b: 2 },
            Step::Rename {
                node: 0,
                from: 8,
                to: 10,
            },
            Step::Delete { node: 4, path: 10 },
            Step::Symlink {
                node: 7,
                path: 1,
                target: 10,
            },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Tier {
                a: 1,
                b: 7,
                tier: 2,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 1,
            },
            Step::Rename {
                node: 6,
                from: 11,
                to: 7,
            },
            Step::Partition { a: 4, b: 5 },
            Step::Partition { a: 2, b: 5 },
            Step::Delete { node: 2, path: 3 },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 2,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 2,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 20,
            },
            Step::MassDelete {
                node: 7,
                fraction: 81,
            },
            Step::Offline { node: 1 },
            Step::Partition { a: 2, b: 5 },
            Step::Settle { secs: 7 },
            Step::MassDelete {
                node: 4,
                fraction: 64,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 3,
            },
            Step::MassDelete {
                node: 4,
                fraction: 63,
            },
            Step::Modify {
                node: 3,
                path: 6,
                content: 1,
            },
            Step::Tier {
                a: 0,
                b: 2,
                tier: 1,
            },
            Step::Delete { node: 2, path: 6 },
            Step::Create {
                node: 6,
                path: 5,
                content: 1,
            },
            Step::Rmdir { node: 2, dir: 2 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 4,
            },
            Step::Touch { node: 0, path: 6 },
            Step::Heal { a: 4, b: 4 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::Rename {
                node: 7,
                from: 10,
                to: 9,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 32,
                },
                delay_secs: 16,
            },
            Step::Online { node: 5 },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 40,
            },
            Step::Settle { secs: 37 },
            Step::Settle { secs: 10 },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::Offline { node: 7 },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Create {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 13,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Create {
                node: 6,
                path: 1,
                content: 5,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), None, Some(2), Some(3), None],
            },
            Step::Delete { node: 0, path: 6 },
            Step::Tier {
                a: 7,
                b: 3,
                tier: 0,
            },
            Step::Offline { node: 0 },
            Step::Create {
                node: 7,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 1,
            },
            Step::MassModify {
                node: 0,
                fraction: 77,
                content: 5,
            },
            Step::Chmod { node: 3, path: 11 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 0,
            },
            Step::Chmod { node: 6, path: 8 },
            Step::Create {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Symlink {
                node: 7,
                path: 10,
                target: 9,
            },
            Step::Heal { a: 3, b: 2 },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Delete { node: 4, path: 11 },
            Step::Symlink {
                node: 4,
                path: 0,
                target: 3,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, Some(4), Some(4), Some(3), Some(5), Some(1)],
            },
            Step::Touch { node: 1, path: 8 },
            Step::Touch { node: 5, path: 11 },
            Step::Rename {
                node: 4,
                from: 9,
                to: 7,
            },
            Step::Offline { node: 4 },
            Step::Delete { node: 0, path: 0 },
            Step::Create {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Tier {
                a: 0,
                b: 6,
                tier: 0,
            },
            Step::Modify {
                node: 7,
                path: 7,
                content: 5,
            },
            Step::Delete { node: 0, path: 4 },
            Step::Tier {
                a: 5,
                b: 0,
                tier: 1,
            },
            Step::Settle { secs: 16 },
            Step::Tier {
                a: 3,
                b: 6,
                tier: 0,
            },
            Step::Touch { node: 2, path: 6 },
            Step::Online { node: 2 },
            Step::Create {
                node: 6,
                path: 10,
                content: 3,
            },
            Step::Symlink {
                node: 5,
                path: 2,
                target: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 8,
            },
            Step::Modify {
                node: 1,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 4,
            },
            Step::Symlink {
                node: 2,
                path: 10,
                target: 8,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 4,
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::Online { node: 2 },
            Step::Modify {
                node: 2,
                path: 11,
                content: 2,
            },
            Step::Crash {
                node: 3,
                gap_secs: 116,
            },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 0,
            },
            Step::Settle { secs: 39 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), None, Some(4), Some(4), None, Some(4), Some(5)],
            },
            Step::Partition { a: 5, b: 4 },
            Step::Rename {
                node: 5,
                from: 6,
                to: 3,
            },
            Step::Chmod { node: 7, path: 4 },
            Step::Delete { node: 1, path: 7 },
            Step::Create {
                node: 2,
                path: 10,
                content: 5,
            },
            Step::Partition { a: 1, b: 6 },
            Step::Chmod { node: 3, path: 9 },
            Step::Partition { a: 7, b: 1 },
            Step::Modify {
                node: 0,
                path: 2,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 3,
            },
            Step::Rename {
                node: 1,
                from: 10,
                to: 4,
            },
            Step::Heal { a: 5, b: 1 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 48,
            },
            Step::Rename {
                node: 6,
                from: 3,
                to: 6,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 8,
                content: 5,
            },
            Step::Offline { node: 3 },
            Step::Delete { node: 1, path: 2 },
            Step::Online { node: 6 },
            Step::Symlink {
                node: 6,
                path: 1,
                target: 0,
            },
            Step::Delete { node: 7, path: 3 },
            Step::Create {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Settle { secs: 23 },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Delete { node: 4, path: 5 },
            Step::Partition { a: 0, b: 5 },
            Step::Delete { node: 1, path: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 6,
                },
                delay_secs: 19,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 3,
            },
            Step::Settle { secs: 11 },
            Step::Partition { a: 2, b: 4 },
            Step::Partition { a: 3, b: 7 },
            Step::Create {
                node: 0,
                path: 5,
                content: 4,
            },
            Step::Heal { a: 2, b: 3 },
            Step::Rename {
                node: 0,
                from: 5,
                to: 8,
            },
            Step::Create {
                node: 3,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Online { node: 5 },
            Step::Crash {
                node: 1,
                gap_secs: 63,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 5,
            },
            Step::Delete { node: 6, path: 4 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Delete { node: 7, path: 9 },
            Step::MassDelete {
                node: 0,
                fraction: 90,
            },
        ],
    );
}

/// A re-derived conflict merge at a restored path carried the mark, and
/// its winning content was the restored record's own, which the user had
/// deleted: a local delete skips the trash, so the content existed
/// nowhere. The want asked only the batch's source and author, went
/// without source, and every peer wanting the same content waited on it
/// (I1). Every want at a marked path now asks every member, and step 4
/// settles it once all refuse (§8.3). Shrunk against a build without that
/// rule; it fails there and passes with it.
#[test]
fn a_marked_want_whose_content_exists_nowhere_settles_unrecoverable() {
    passes(
        414,
        &[
            Step::Modify {
                node: 1,
                path: 5,
                content: 4,
            },
            Step::Chmod { node: 2, path: 8 },
            Step::Delete { node: 0, path: 5 },
            Step::Partition { a: 2, b: 1 },
            Step::Settle { secs: 25 },
            Step::Create {
                node: 4,
                path: 7,
                content: 3,
            },
            Step::Crash {
                node: 5,
                gap_secs: 1,
            },
            Step::Settle { secs: 6 },
            Step::Tier {
                a: 6,
                b: 3,
                tier: 2,
            },
            Step::Rename {
                node: 1,
                from: 10,
                to: 5,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Crash {
                node: 0,
                gap_secs: 7,
            },
            Step::Partition { a: 1, b: 2 },
            Step::Create {
                node: 0,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 7,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Chmod { node: 2, path: 2 },
            Step::Offline { node: 7 },
            Step::Rename {
                node: 1,
                from: 3,
                to: 2,
            },
            Step::Create {
                node: 1,
                path: 10,
                content: 4,
            },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Modify {
                node: 4,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 3, b: 1 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Create {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Online { node: 0 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(1), None, Some(2), Some(2), Some(2), None],
            },
            Step::Crash {
                node: 5,
                gap_secs: 23,
            },
            Step::Modify {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Chmod { node: 3, path: 5 },
            Step::Delete { node: 3, path: 5 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 41,
            },
            Step::MassDelete {
                node: 3,
                fraction: 93,
            },
            Step::Create {
                node: 5,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 5,
            },
            Step::Tier {
                a: 6,
                b: 3,
                tier: 2,
            },
            Step::Modify {
                node: 3,
                path: 5,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 5,
            },
            Step::Crash {
                node: 7,
                gap_secs: 75,
            },
            Step::Settle { secs: 18 },
            Step::Symlink {
                node: 5,
                path: 7,
                target: 1,
            },
            Step::Online { node: 1 },
            Step::Delete { node: 0, path: 11 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 51,
            },
            Step::Offline { node: 3 },
            Step::Heal { a: 3, b: 6 },
            Step::Rename {
                node: 2,
                from: 8,
                to: 1,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 21,
            },
            Step::Modify {
                node: 0,
                path: 0,
                content: 4,
            },
            Step::Partition { a: 3, b: 0 },
            Step::MassModify {
                node: 2,
                fraction: 90,
                content: 3,
            },
            Step::Online { node: 0 },
            Step::Modify {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Partition { a: 6, b: 4 },
            Step::Create {
                node: 2,
                path: 4,
                content: 5,
            },
            Step::Heal { a: 2, b: 1 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 56,
            },
            Step::Partition { a: 6, b: 2 },
            Step::Heal { a: 5, b: 0 },
            Step::Tier {
                a: 1,
                b: 1,
                tier: 2,
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 89,
            },
            Step::Modify {
                node: 7,
                path: 11,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 3,
            },
            Step::Rename {
                node: 0,
                from: 10,
                to: 7,
            },
            Step::Settle { secs: 12 },
            Step::Create {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Crash {
                node: 2,
                gap_secs: 105,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Offline { node: 7 },
            Step::Create {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 22,
            },
            Step::MassModify {
                node: 4,
                fraction: 50,
                content: 4,
            },
            Step::Offline { node: 4 },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 4,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 10,
                content: 1,
            },
            Step::Delete { node: 0, path: 8 },
            Step::User {
                node: 1,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 18,
                },
                delay_secs: 41,
            },
            Step::Delete { node: 7, path: 2 },
            Step::Rmdir { node: 4, dir: 0 },
            Step::Online { node: 1 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 0,
            },
        ],
    );
}

/// A winning tombstone moved a node's adopted symlink to a conflict copy
/// (§7.6), and the node's user then removed the copy's directory. I2 looked
/// for a user edit at the path the content was adopted at, not where sync
/// had put it, and called the user's own deletion a loss. The adoption now
/// follows the content to the copy (§14.1 I2). Fails without that.
#[test]
fn a_conflict_copy_its_user_removed_with_its_directory_is_not_lost() {
    passes(
        19236,
        &[
            Step::Offline { node: 0 },
            Step::Crash {
                node: 1,
                gap_secs: 47,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(3), Some(3), Some(4)],
            },
            Step::Settle { secs: 2 },
            Step::Modify {
                node: 6,
                path: 10,
                content: 5,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, None, Some(5), Some(5)],
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 4,
            },
            Step::Symlink {
                node: 3,
                path: 9,
                target: 6,
            },
            Step::Offline { node: 3 },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 51,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::MassModify {
                node: 0,
                fraction: 83,
                content: 3,
            },
            Step::Rmdir { node: 5, dir: 1 },
            Step::Delete { node: 1, path: 3 },
            Step::Partition { a: 5, b: 6 },
            Step::MassDelete {
                node: 7,
                fraction: 94,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 4,
            },
            Step::Create {
                node: 6,
                path: 0,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 1,
            },
            Step::Offline { node: 4 },
            Step::MassModify {
                node: 2,
                fraction: 95,
                content: 3,
            },
            Step::Settle { secs: 1 },
            Step::Modify {
                node: 4,
                path: 10,
                content: 3,
            },
            Step::Rename {
                node: 5,
                from: 9,
                to: 7,
            },
            Step::Symlink {
                node: 6,
                path: 5,
                target: 11,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Online { node: 7 },
            Step::Modify {
                node: 1,
                path: 8,
                content: 2,
            },
            Step::Rmdir { node: 6, dir: 0 },
        ],
    );
}

/// The same, with the copy removed by the user's mass delete.
#[test]
fn a_conflict_copy_its_user_mass_deleted_is_not_lost() {
    passes(
        1423,
        &[
            Step::Touch { node: 7, path: 8 },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Symlink {
                node: 1,
                path: 5,
                target: 3,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 1,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(4), Some(3), Some(5)],
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 4,
            },
            Step::Heal { a: 2, b: 2 },
            Step::Create {
                node: 2,
                path: 5,
                content: 4,
            },
            Step::Touch { node: 1, path: 10 },
            Step::Rename {
                node: 3,
                from: 0,
                to: 6,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 2,
            },
            Step::Crash {
                node: 5,
                gap_secs: 114,
            },
            Step::Rename {
                node: 4,
                from: 5,
                to: 6,
            },
            Step::Touch { node: 4, path: 4 },
            Step::Mkdir { node: 1, dir: 0 },
            Step::Modify {
                node: 6,
                path: 2,
                content: 1,
            },
            Step::MassModify {
                node: 7,
                fraction: 64,
                content: 4,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 32,
            },
            Step::MassModify {
                node: 6,
                fraction: 94,
                content: 3,
            },
            Step::Delete { node: 5, path: 5 },
            Step::Partition { a: 4, b: 5 },
            Step::Partition { a: 1, b: 6 },
            Step::Rename {
                node: 0,
                from: 1,
                to: 8,
            },
            Step::Delete { node: 2, path: 4 },
            Step::Crash {
                node: 0,
                gap_secs: 1,
            },
            Step::Crash {
                node: 7,
                gap_secs: 72,
            },
            Step::Crash {
                node: 1,
                gap_secs: 19,
            },
            Step::Create {
                node: 5,
                path: 1,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 34,
            },
            Step::Modify {
                node: 6,
                path: 4,
                content: 5,
            },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Offline { node: 6 },
            Step::Tier {
                a: 4,
                b: 1,
                tier: 0,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Settle { secs: 39 },
            Step::Tier {
                a: 5,
                b: 3,
                tier: 1,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 4,
            },
            Step::Heal { a: 7, b: 4 },
            Step::Create {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Delete { node: 3, path: 0 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 5,
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Create {
                node: 7,
                path: 7,
                content: 5,
            },
            Step::Heal { a: 7, b: 1 },
            Step::Online { node: 3 },
            Step::Modify {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Chmod { node: 3, path: 7 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 5,
            },
            Step::Rename {
                node: 2,
                from: 6,
                to: 1,
            },
            Step::Offline { node: 6 },
            Step::Delete { node: 2, path: 10 },
            Step::Heal { a: 5, b: 2 },
            Step::Crash {
                node: 4,
                gap_secs: 105,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 2,
            },
            Step::Chmod { node: 3, path: 4 },
            Step::Settle { secs: 29 },
            Step::Touch { node: 3, path: 7 },
            Step::Create {
                node: 1,
                path: 9,
                content: 5,
            },
            Step::Chmod { node: 4, path: 0 },
            Step::Create {
                node: 1,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 3,
            },
            Step::MassModify {
                node: 3,
                fraction: 94,
                content: 2,
            },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Partition { a: 7, b: 0 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 0,
                to: 5,
            },
            Step::Offline { node: 5 },
            Step::Modify {
                node: 6,
                path: 2,
                content: 3,
            },
            Step::Rmdir { node: 2, dir: 2 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 4,
            },
            Step::Tier {
                a: 7,
                b: 3,
                tier: 1,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 90,
            },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 2,
                path: 9,
                content: 5,
            },
            Step::MassDelete {
                node: 4,
                fraction: 70,
            },
            Step::Modify {
                node: 7,
                path: 4,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 48,
            },
            Step::Rename {
                node: 4,
                from: 4,
                to: 10,
            },
            Step::MassModify {
                node: 2,
                fraction: 94,
                content: 5,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 3,
            },
            Step::MassDelete {
                node: 4,
                fraction: 93,
            },
            Step::Delete { node: 2, path: 3 },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 0, b: 4 },
            Step::Rename {
                node: 0,
                from: 2,
                to: 5,
            },
            Step::Create {
                node: 7,
                path: 9,
                content: 4,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), Some(3), Some(5), None, Some(3), Some(3), Some(1)],
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 4,
            },
            Step::Tier {
                a: 6,
                b: 5,
                tier: 0,
            },
            Step::MassDelete {
                node: 1,
                fraction: 75,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 1,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![None, Some(5), Some(1), None, Some(4)],
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 10,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::Settle { secs: 11 },
            Step::Mkdir { node: 4, dir: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 26,
            },
            Step::Create {
                node: 2,
                path: 8,
                content: 3,
            },
            Step::Online { node: 3 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 22,
            },
            Step::Touch { node: 3, path: 3 },
            Step::Settle { secs: 34 },
            Step::Chmod { node: 6, path: 4 },
            Step::Partition { a: 2, b: 4 },
            Step::Crash {
                node: 1,
                gap_secs: 19,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(2), Some(2), Some(1), None],
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 11,
                content: 2,
            },
            Step::Online { node: 1 },
            Step::Tier {
                a: 5,
                b: 0,
                tier: 2,
            },
            Step::MassModify {
                node: 2,
                fraction: 64,
                content: 1,
            },
            Step::Online { node: 0 },
            Step::Delete { node: 4, path: 2 },
            Step::Touch { node: 5, path: 11 },
            Step::Tier {
                a: 4,
                b: 3,
                tier: 1,
            },
            Step::Touch { node: 4, path: 2 },
            Step::Chmod { node: 7, path: 11 },
            Step::Create {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Symlink {
                node: 0,
                path: 1,
                target: 11,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 21,
            },
            Step::Modify {
                node: 4,
                path: 5,
                content: 3,
            },
            Step::Heal { a: 4, b: 3 },
            Step::Partition { a: 0, b: 0 },
            Step::MassDelete {
                node: 0,
                fraction: 99,
            },
        ],
    );
}

/// The same, with the copy rewritten by the user's mass modify.
#[test]
fn a_conflict_copy_its_user_rewrote_is_not_lost() {
    passes(
        5665,
        &[
            Step::Heal { a: 3, b: 5 },
            Step::Touch { node: 7, path: 3 },
            Step::Delete { node: 0, path: 1 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 7,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 2,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Partition { a: 5, b: 6 },
            Step::Delete { node: 0, path: 10 },
            Step::Partition { a: 3, b: 6 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 2,
            },
            Step::Heal { a: 1, b: 6 },
            Step::Online { node: 1 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 1,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(2), None, Some(3)],
            },
            Step::Create {
                node: 5,
                path: 3,
                content: 1,
            },
            Step::MassDelete {
                node: 5,
                fraction: 93,
            },
            Step::Modify {
                node: 2,
                path: 3,
                content: 2,
            },
            Step::Delete { node: 3, path: 4 },
            Step::Everywhere {
                path: 0,
                contents: vec![None, Some(4), None],
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 5,
            },
            Step::Settle { secs: 1 },
            Step::Chmod { node: 4, path: 1 },
            Step::Heal { a: 1, b: 3 },
            Step::Mkdir { node: 2, dir: 0 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 33,
            },
            Step::Symlink {
                node: 1,
                path: 0,
                target: 2,
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 2,
            },
            Step::Delete { node: 7, path: 11 },
            Step::Settle { secs: 29 },
            Step::Create {
                node: 2,
                path: 7,
                content: 4,
            },
            Step::Rename {
                node: 1,
                from: 9,
                to: 8,
            },
            Step::Create {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Crash {
                node: 6,
                gap_secs: 16,
            },
            Step::Chmod { node: 2, path: 8 },
            Step::Settle { secs: 25 },
            Step::Touch { node: 2, path: 3 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(5), Some(1), Some(2), Some(2), Some(4), Some(3)],
            },
            Step::Touch { node: 2, path: 2 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 4,
            },
            Step::Crash {
                node: 5,
                gap_secs: 92,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 1,
            },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Modify {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Partition { a: 7, b: 4 },
            Step::Create {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::Settle { secs: 14 },
            Step::Tier {
                a: 1,
                b: 4,
                tier: 1,
            },
            Step::MassModify {
                node: 5,
                fraction: 98,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 1,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Partition { a: 7, b: 0 },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Create {
                node: 5,
                path: 1,
                content: 2,
            },
            Step::Heal { a: 5, b: 4 },
            Step::Modify {
                node: 1,
                path: 0,
                content: 3,
            },
            Step::Delete { node: 6, path: 11 },
            Step::Delete { node: 3, path: 2 },
            Step::Create {
                node: 1,
                path: 2,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 1,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 5,
            },
            Step::Symlink {
                node: 2,
                path: 9,
                target: 9,
            },
            Step::MassModify {
                node: 1,
                fraction: 75,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
        ],
    );
}

/// Class A. A paused node held a peer's tombstone; its user denied, then
/// reverted. The deny's bump joined the pending batch, the revert
/// discarded it with the quarantine it had consumed, and the tombstone
/// was lost (I1). A deny now waits while the folder is paused, and a
/// revert that discards a deny returns its held item (§8.3): the pin fails
/// with both off, and either one alone keeps it.
#[test]
fn a_deny_on_a_paused_folder_waits_instead_of_joining_the_pending_batch() {
    passes(
        12922,
        &[
            Step::Chmod { node: 1, path: 6 },
            Step::Create {
                node: 4,
                path: 8,
                content: 5,
            },
            Step::Delete { node: 7, path: 7 },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 14,
                },
                delay_secs: 17,
            },
            Step::Tier {
                a: 1,
                b: 2,
                tier: 0,
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 2,
            },
            Step::Partition { a: 2, b: 7 },
            Step::Partition { a: 2, b: 1 },
            Step::Offline { node: 1 },
            Step::Delete { node: 0, path: 8 },
            Step::Delete { node: 0, path: 7 },
            Step::Heal { a: 5, b: 1 },
            Step::Offline { node: 7 },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 33,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 1,
            },
            Step::Delete { node: 5, path: 2 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 46,
            },
            Step::Modify {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::Online { node: 4 },
            Step::Heal { a: 3, b: 2 },
            Step::Heal { a: 3, b: 2 },
            Step::Rename {
                node: 3,
                from: 4,
                to: 8,
            },
            Step::Online { node: 1 },
            Step::Settle { secs: 8 },
            Step::Delete { node: 3, path: 9 },
            Step::Heal { a: 5, b: 0 },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 49,
                },
                delay_secs: 16,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 2,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 16,
                },
                delay_secs: 8,
            },
            Step::Create {
                node: 5,
                path: 10,
                content: 5,
            },
            Step::Settle { secs: 39 },
            Step::MassDelete {
                node: 3,
                fraction: 85,
            },
            Step::Partition { a: 7, b: 3 },
            Step::Rename {
                node: 2,
                from: 6,
                to: 0,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 59,
            },
            Step::Create {
                node: 6,
                path: 2,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 11,
                content: 4,
            },
            Step::Settle { secs: 2 },
            Step::Delete { node: 2, path: 8 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 49,
            },
            Step::Modify {
                node: 2,
                path: 1,
                content: 4,
            },
            Step::Heal { a: 2, b: 2 },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 23,
            },
            Step::Heal { a: 4, b: 7 },
        ],
    );
}

/// Class B. After a revert a restoring commit found its path occupied and
/// left a carrier with the mark; the user's deny then bumped the record at
/// that marked path, and the carrier was wanted again as it stood and
/// committed under the deny's newer record (I8). A deny now waits while any
/// path of its held item carries the mark (§8.3). Shrunk on a build
/// without that wait; fails there.
#[test]
fn a_deny_at_a_marked_path_waits_for_the_mark_to_clear() {
    passes(
        19362,
        &[
            Step::Chmod { node: 1, path: 1 },
            Step::Crash {
                node: 1,
                gap_secs: 66,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 3,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(4), Some(4), None, None, Some(1)],
            },
            Step::Rmdir { node: 3, dir: 1 },
            Step::Create {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Delete { node: 6, path: 7 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 3,
            },
            Step::Touch { node: 3, path: 9 },
            Step::Online { node: 2 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 6,
            },
            Step::Partition { a: 3, b: 6 },
            Step::Heal { a: 5, b: 5 },
            Step::Delete { node: 6, path: 9 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(1), Some(1)],
            },
            Step::Settle { secs: 36 },
            Step::Symlink {
                node: 2,
                path: 0,
                target: 7,
            },
            Step::Delete { node: 7, path: 10 },
            Step::Symlink {
                node: 5,
                path: 3,
                target: 1,
            },
            Step::Delete { node: 7, path: 8 },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Heal { a: 6, b: 3 },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Heal { a: 7, b: 6 },
            Step::Modify {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Offline { node: 3 },
            Step::Partition { a: 6, b: 3 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 28,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 2,
            },
            Step::Crash {
                node: 7,
                gap_secs: 3,
            },
            Step::Online { node: 3 },
            Step::Delete { node: 7, path: 0 },
            Step::Tier {
                a: 1,
                b: 5,
                tier: 2,
            },
            Step::Modify {
                node: 1,
                path: 6,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 48,
            },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Rename {
                node: 2,
                from: 2,
                to: 6,
            },
            Step::Create {
                node: 3,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Settle { secs: 37 },
            Step::Create {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Create {
                node: 6,
                path: 8,
                content: 1,
            },
            Step::Create {
                node: 0,
                path: 7,
                content: 2,
            },
            Step::Offline { node: 0 },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 0,
            },
            Step::Chmod { node: 2, path: 9 },
            Step::Modify {
                node: 1,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(4), None, None, Some(2), Some(3), None],
            },
            Step::Delete { node: 3, path: 3 },
            Step::Heal { a: 0, b: 1 },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Modify {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Delete { node: 1, path: 2 },
            Step::Create {
                node: 4,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 1,
            },
            Step::Delete { node: 7, path: 0 },
            Step::Partition { a: 1, b: 4 },
            Step::Chmod { node: 3, path: 10 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(1), Some(4), None, None, Some(3), None, Some(4)],
            },
            Step::Create {
                node: 7,
                path: 5,
                content: 5,
            },
            Step::Create {
                node: 3,
                path: 11,
                content: 3,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 1,
            },
            Step::Touch { node: 7, path: 5 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 28,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 2,
            },
            Step::Symlink {
                node: 4,
                path: 1,
                target: 2,
            },
            Step::Rename {
                node: 0,
                from: 7,
                to: 2,
            },
            Step::Chmod { node: 3, path: 7 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(1), Some(1), Some(4)],
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(3), Some(2), Some(3), Some(3), Some(4), None],
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::Offline { node: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 57,
            },
            Step::Rmdir { node: 2, dir: 2 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Settle { secs: 25 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 4,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Create {
                node: 6,
                path: 1,
                content: 5,
            },
            Step::Rename {
                node: 2,
                from: 7,
                to: 8,
            },
            Step::Delete { node: 0, path: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 16,
                },
                delay_secs: 23,
            },
            Step::Touch { node: 7, path: 0 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(1), Some(1), Some(1), None, Some(1), Some(2)],
            },
            Step::Create {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Partition { a: 3, b: 3 },
            Step::Tier {
                a: 3,
                b: 1,
                tier: 1,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Delete { node: 2, path: 11 },
            Step::Crash {
                node: 6,
                gap_secs: 19,
            },
            Step::Tier {
                a: 5,
                b: 5,
                tier: 2,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 56,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::MassDelete {
                node: 2,
                fraction: 87,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 11,
                content: 3,
            },
            Step::Online { node: 5 },
            Step::Everywhere {
                path: 9,
                contents: vec![None, None],
            },
            Step::Rmdir { node: 0, dir: 2 },
            Step::Touch { node: 4, path: 1 },
            Step::Chmod { node: 7, path: 1 },
            Step::Create {
                node: 1,
                path: 0,
                content: 4,
            },
            Step::Crash {
                node: 7,
                gap_secs: 76,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(5), Some(1), Some(1)],
            },
            Step::Create {
                node: 1,
                path: 3,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(1), Some(3), Some(4)],
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 7,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 3,
            },
            Step::Online { node: 2 },
            Step::Heal { a: 7, b: 2 },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Delete { node: 0, path: 1 },
            Step::Chmod { node: 4, path: 7 },
            Step::Delete { node: 6, path: 3 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(3), Some(3), Some(5), Some(3), None, None],
            },
            Step::Delete { node: 1, path: 6 },
            Step::Create {
                node: 2,
                path: 7,
                content: 2,
            },
            Step::Touch { node: 2, path: 5 },
            Step::Delete { node: 0, path: 4 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 49,
            },
            Step::Heal { a: 7, b: 5 },
            Step::Offline { node: 1 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Chmod { node: 0, path: 9 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Settle { secs: 34 },
            Step::Create {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 38 },
            Step::Delete { node: 3, path: 8 },
            Step::Create {
                node: 2,
                path: 11,
                content: 3,
            },
            Step::Partition { a: 3, b: 0 },
            Step::Create {
                node: 7,
                path: 5,
                content: 5,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Modify {
                node: 3,
                path: 5,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Create {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 6,
                content: 3,
            },
            Step::Delete { node: 7, path: 9 },
            Step::Online { node: 7 },
            Step::Chmod { node: 0, path: 10 },
            Step::Modify {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 10,
                content: 2,
            },
            Step::Touch { node: 1, path: 10 },
            Step::Partition { a: 6, b: 6 },
            Step::Create {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Settle { secs: 2 },
            Step::Heal { a: 4, b: 2 },
            Step::Create {
                node: 6,
                path: 3,
                content: 1,
            },
            Step::Heal { a: 4, b: 0 },
            Step::MassDelete {
                node: 2,
                fraction: 66,
            },
            Step::Rename {
                node: 7,
                from: 7,
                to: 4,
            },
            Step::Create {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Symlink {
                node: 1,
                path: 1,
                target: 5,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 28,
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Partition { a: 1, b: 6 },
            Step::Settle { secs: 21 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, None, None, Some(3), Some(2)],
            },
            Step::Create {
                node: 1,
                path: 4,
                content: 3,
            },
            Step::Settle { secs: 19 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 42,
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 3,
            },
            Step::MassDelete {
                node: 6,
                fraction: 70,
            },
            Step::Settle { secs: 2 },
            Step::Online { node: 7 },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::Online { node: 0 },
            Step::Crash {
                node: 7,
                gap_secs: 8,
            },
            Step::Rename {
                node: 1,
                from: 11,
                to: 11,
            },
            Step::Rename {
                node: 5,
                from: 10,
                to: 2,
            },
            Step::MassModify {
                node: 5,
                fraction: 86,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Settle { secs: 37 },
            Step::Modify {
                node: 0,
                path: 10,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 54,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 1,
            },
            Step::Mkdir { node: 4, dir: 0 },
            Step::Create {
                node: 5,
                path: 7,
                content: 3,
            },
            Step::Settle { secs: 14 },
            Step::Symlink {
                node: 1,
                path: 10,
                target: 7,
            },
            Step::Delete { node: 3, path: 6 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 3,
            },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Create {
                node: 5,
                path: 6,
                content: 3,
            },
            Step::MassModify {
                node: 6,
                fraction: 93,
                content: 4,
            },
            Step::Offline { node: 1 },
            Step::Touch { node: 5, path: 1 },
            Step::Create {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 3,
            },
            Step::Delete { node: 4, path: 5 },
            Step::Delete { node: 5, path: 5 },
            Step::Partition { a: 4, b: 3 },
            Step::Chmod { node: 0, path: 6 },
            Step::Chmod { node: 4, path: 0 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Offline { node: 7 },
            Step::MassDelete {
                node: 1,
                fraction: 65,
            },
            Step::Delete { node: 7, path: 1 },
            Step::Tier {
                a: 6,
                b: 5,
                tier: 2,
            },
            Step::Create {
                node: 3,
                path: 2,
                content: 2,
            },
            Step::Crash {
                node: 3,
                gap_secs: 80,
            },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Modify {
                node: 0,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 0,
                path: 1,
                content: 3,
            },
            Step::Offline { node: 4 },
            Step::MassDelete {
                node: 5,
                fraction: 58,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 45,
            },
            Step::Heal { a: 6, b: 4 },
            Step::Delete { node: 4, path: 1 },
            Step::Rename {
                node: 7,
                from: 7,
                to: 2,
            },
            Step::Mkdir { node: 1, dir: 0 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 4,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(5), Some(5), Some(1), Some(2), None, Some(5)],
            },
            Step::Delete { node: 7, path: 0 },
            Step::Create {
                node: 5,
                path: 8,
                content: 4,
            },
            Step::Tier {
                a: 0,
                b: 0,
                tier: 0,
            },
            Step::Rename {
                node: 2,
                from: 0,
                to: 7,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(5), Some(3), Some(1), Some(2)],
            },
            Step::Heal { a: 1, b: 1 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 4,
            },
            Step::Tier {
                a: 4,
                b: 3,
                tier: 2,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Rename {
                node: 5,
                from: 8,
                to: 7,
            },
            Step::Crash {
                node: 7,
                gap_secs: 103,
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Heal { a: 2, b: 4 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(2), None, None, Some(3), Some(4), Some(2)],
            },
            Step::Heal { a: 1, b: 0 },
            Step::Online { node: 3 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Rmdir { node: 2, dir: 2 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::MassModify {
                node: 5,
                fraction: 71,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Heal { a: 0, b: 4 },
            Step::Offline { node: 1 },
            Step::Chmod { node: 2, path: 2 },
            Step::Modify {
                node: 3,
                path: 11,
                content: 5,
            },
            Step::Partition { a: 3, b: 1 },
            Step::Delete { node: 5, path: 2 },
            Step::Settle { secs: 19 },
            Step::Rename {
                node: 1,
                from: 4,
                to: 2,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(1), None],
            },
            Step::Delete { node: 2, path: 10 },
            Step::Create {
                node: 5,
                path: 10,
                content: 5,
            },
            Step::Online { node: 3 },
            Step::Touch { node: 3, path: 5 },
            Step::Partition { a: 2, b: 4 },
            Step::Settle { secs: 28 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(4), None, None, Some(5), Some(4), Some(1), Some(3)],
            },
            Step::Delete { node: 0, path: 5 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 6,
                content: 5,
            },
            Step::Partition { a: 5, b: 2 },
            Step::Symlink {
                node: 3,
                path: 10,
                target: 1,
            },
            Step::Modify {
                node: 3,
                path: 9,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, None, Some(5)],
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Crash {
                node: 2,
                gap_secs: 90,
            },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 5,
                path: 1,
                content: 3,
            },
            Step::Heal { a: 7, b: 7 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), Some(1), Some(1), None],
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 4,
            },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Offline { node: 6 },
            Step::Settle { secs: 11 },
            Step::Create {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Settle { secs: 1 },
            Step::Online { node: 7 },
            Step::Delete { node: 6, path: 1 },
            Step::Create {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 50,
            },
            Step::Touch { node: 7, path: 5 },
            Step::Create {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Offline { node: 1 },
            Step::Delete { node: 4, path: 1 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(5), None],
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 2,
            },
            Step::Delete { node: 1, path: 11 },
            Step::Online { node: 7 },
            Step::Delete { node: 5, path: 5 },
            Step::Settle { secs: 27 },
            Step::Tier {
                a: 0,
                b: 4,
                tier: 1,
            },
            Step::Create {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::Settle { secs: 34 },
            Step::Touch { node: 5, path: 3 },
            Step::Settle { secs: 18 },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Create {
                node: 5,
                path: 7,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 3,
            },
            Step::Rename {
                node: 0,
                from: 8,
                to: 10,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 1,
            },
            Step::Settle { secs: 2 },
            Step::Offline { node: 4 },
            Step::Heal { a: 4, b: 5 },
            Step::Modify {
                node: 0,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 2,
                content: 5,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 24,
            },
            Step::Chmod { node: 6, path: 10 },
            Step::Settle { secs: 24 },
            Step::Settle { secs: 20 },
            Step::Rename {
                node: 6,
                from: 3,
                to: 7,
            },
            Step::Settle { secs: 32 },
            Step::Delete { node: 0, path: 8 },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 2,
            },
            Step::Partition { a: 2, b: 0 },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 2,
            },
            Step::Rename {
                node: 2,
                from: 6,
                to: 0,
            },
            Step::Heal { a: 3, b: 6 },
            Step::Create {
                node: 6,
                path: 6,
                content: 3,
            },
            Step::Heal { a: 5, b: 5 },
            Step::Modify {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::Heal { a: 2, b: 4 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Rmdir { node: 7, dir: 1 },
            Step::Create {
                node: 5,
                path: 11,
                content: 5,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Modify {
                node: 7,
                path: 3,
                content: 3,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Partition { a: 0, b: 7 },
            Step::Heal { a: 7, b: 2 },
            Step::Partition { a: 6, b: 6 },
            Step::Heal { a: 2, b: 2 },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 30,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 16,
            },
            Step::Delete { node: 5, path: 7 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 3,
            },
            Step::Crash {
                node: 4,
                gap_secs: 40,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(1)],
            },
            Step::Create {
                node: 3,
                path: 7,
                content: 4,
            },
            Step::Mkdir { node: 1, dir: 0 },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 3,
            },
            Step::Delete { node: 3, path: 10 },
            Step::Delete { node: 5, path: 8 },
        ],
    );
}

/// Class C. A revert queued before a crash ran at the `ScanStarted` of the
/// restart's scan and removed records the host had already listed for the
/// bracket, which then reported `Unchanged` for a path with no record. A
/// revert now waits for the startup scan to finish, and no decision runs
/// inside an open bracket (§8.3): the pin fails with both off. Shrunk on a
/// build without the startup-scan wait.
#[test]
fn a_queued_revert_waits_for_the_startup_scan_to_finish() {
    passes(
        4925,
        &[
            Step::Tier {
                a: 7,
                b: 2,
                tier: 0,
            },
            Step::Crash {
                node: 3,
                gap_secs: 69,
            },
            Step::Heal { a: 5, b: 6 },
            Step::MassModify {
                node: 7,
                fraction: 85,
                content: 5,
            },
            Step::Chmod { node: 7, path: 3 },
            Step::Heal { a: 6, b: 0 },
            Step::Delete { node: 6, path: 0 },
            Step::MassDelete {
                node: 1,
                fraction: 89,
            },
            Step::Settle { secs: 16 },
            Step::Touch { node: 2, path: 2 },
            Step::Partition { a: 5, b: 4 },
            Step::User {
                node: 5,
                action: crate::UserAction::DenyAll,
                delay_secs: 14,
            },
            Step::Offline { node: 4 },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 13,
            },
            Step::Modify {
                node: 4,
                path: 6,
                content: 5,
            },
            Step::Settle { secs: 8 },
            Step::Create {
                node: 3,
                path: 2,
                content: 5,
            },
            Step::Settle { secs: 22 },
            Step::Create {
                node: 4,
                path: 8,
                content: 3,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(3)],
            },
            Step::Delete { node: 0, path: 8 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Create {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::Offline { node: 1 },
            Step::Settle { secs: 5 },
            Step::Modify {
                node: 0,
                path: 1,
                content: 3,
            },
            Step::Settle { secs: 4 },
            Step::Symlink {
                node: 5,
                path: 3,
                target: 11,
            },
            Step::Create {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Touch { node: 2, path: 6 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 5,
            },
            Step::Crash {
                node: 3,
                gap_secs: 7,
            },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Delete { node: 2, path: 11 },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Heal { a: 7, b: 5 },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::MassDelete {
                node: 5,
                fraction: 53,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(2), None, Some(5), Some(2)],
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 2,
            },
            Step::MassModify {
                node: 5,
                fraction: 93,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 11,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 6,
                content: 4,
            },
            Step::Online { node: 6 },
            Step::Modify {
                node: 4,
                path: 11,
                content: 1,
            },
            Step::Touch { node: 6, path: 11 },
            Step::Delete { node: 4, path: 1 },
            Step::Touch { node: 0, path: 3 },
            Step::Settle { secs: 12 },
            Step::Modify {
                node: 6,
                path: 11,
                content: 3,
            },
            Step::Offline { node: 4 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 4,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Create {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Settle { secs: 31 },
            Step::Create {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Online { node: 7 },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::Delete { node: 3, path: 2 },
            Step::Rmdir { node: 5, dir: 1 },
            Step::Chmod { node: 5, path: 0 },
            Step::Touch { node: 5, path: 4 },
            Step::Create {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Partition { a: 6, b: 1 },
            Step::Rmdir { node: 2, dir: 2 },
            Step::Offline { node: 2 },
            Step::Touch { node: 7, path: 4 },
            Step::Settle { secs: 30 },
            Step::Crash {
                node: 2,
                gap_secs: 75,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(5), Some(5), Some(3), Some(1)],
            },
            Step::Delete { node: 3, path: 9 },
            Step::Create {
                node: 5,
                path: 6,
                content: 2,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(5), Some(5), Some(1), None, Some(5), Some(2)],
            },
            Step::Chmod { node: 1, path: 7 },
            Step::Crash {
                node: 1,
                gap_secs: 49,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), Some(1), Some(2), Some(3)],
            },
            Step::Modify {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::Chmod { node: 0, path: 10 },
            Step::Offline { node: 1 },
            Step::Tier {
                a: 3,
                b: 7,
                tier: 2,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Delete { node: 6, path: 0 },
            Step::Touch { node: 5, path: 8 },
            Step::Create {
                node: 7,
                path: 1,
                content: 3,
            },
            Step::Chmod { node: 4, path: 6 },
            Step::Offline { node: 1 },
            Step::Create {
                node: 3,
                path: 0,
                content: 2,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![
                    Some(4),
                    Some(1),
                    Some(3),
                    Some(5),
                    Some(2),
                    Some(2),
                    Some(5),
                ],
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 4,
            },
            Step::Delete { node: 2, path: 2 },
            Step::Chmod { node: 1, path: 7 },
            Step::Delete { node: 7, path: 10 },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 0,
            },
            Step::Tier {
                a: 2,
                b: 7,
                tier: 2,
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 4,
            },
            Step::Settle { secs: 22 },
            Step::Chmod { node: 0, path: 11 },
            Step::Tier {
                a: 0,
                b: 3,
                tier: 0,
            },
            Step::Tier {
                a: 4,
                b: 4,
                tier: 2,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(3), Some(2)],
            },
            Step::Heal { a: 6, b: 5 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Settle { secs: 24 },
            Step::Create {
                node: 5,
                path: 8,
                content: 2,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Heal { a: 1, b: 2 },
            Step::MassModify {
                node: 4,
                fraction: 66,
                content: 3,
            },
            Step::Heal { a: 2, b: 0 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 4,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 4,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 49,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(5), Some(2), Some(3), None, Some(3), Some(2)],
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 4,
            },
            Step::Partition { a: 6, b: 6 },
            Step::Delete { node: 2, path: 1 },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
            Step::Rename {
                node: 0,
                from: 4,
                to: 5,
            },
            Step::Rename {
                node: 0,
                from: 10,
                to: 0,
            },
            Step::Modify {
                node: 7,
                path: 10,
                content: 1,
            },
            Step::Create {
                node: 3,
                path: 6,
                content: 3,
            },
            Step::Delete { node: 2, path: 4 },
            Step::Partition { a: 3, b: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 5,
                    hold_pct: 10,
                },
                delay_secs: 36,
            },
            Step::Modify {
                node: 0,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(3), Some(3), Some(5)],
            },
            Step::Partition { a: 0, b: 1 },
            Step::Rename {
                node: 7,
                from: 11,
                to: 6,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 24,
                },
                delay_secs: 2,
            },
            Step::MassModify {
                node: 0,
                fraction: 94,
                content: 3,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Rename {
                node: 0,
                from: 8,
                to: 2,
            },
            Step::Modify {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Delete { node: 3, path: 10 },
            Step::Online { node: 4 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 42,
            },
            Step::Online { node: 4 },
            Step::Chmod { node: 7, path: 11 },
            Step::MassModify {
                node: 2,
                fraction: 93,
                content: 1,
            },
            Step::MassDelete {
                node: 5,
                fraction: 87,
            },
            Step::Modify {
                node: 4,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 1,
            },
            Step::Touch { node: 6, path: 9 },
            Step::Online { node: 0 },
            Step::Delete { node: 6, path: 7 },
            Step::Delete { node: 6, path: 2 },
            Step::Create {
                node: 3,
                path: 5,
                content: 3,
            },
            Step::Touch { node: 7, path: 5 },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 6,
                content: 1,
            },
            Step::Heal { a: 7, b: 1 },
            Step::Create {
                node: 6,
                path: 1,
                content: 1,
            },
            Step::Offline { node: 7 },
            Step::Create {
                node: 7,
                path: 7,
                content: 4,
            },
            Step::Partition { a: 3, b: 7 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 0,
            },
            Step::Rename {
                node: 1,
                from: 6,
                to: 10,
            },
            Step::MassDelete {
                node: 5,
                fraction: 84,
            },
            Step::Touch { node: 1, path: 2 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Delete { node: 2, path: 5 },
            Step::Delete { node: 2, path: 7 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 5,
            },
            Step::MassModify {
                node: 0,
                fraction: 76,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 8,
                content: 5,
            },
            Step::Settle { secs: 21 },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(5), Some(4), None, None, None],
            },
            Step::Mkdir { node: 6, dir: 2 },
            Step::Modify {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 3,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Mkdir { node: 4, dir: 0 },
            Step::Create {
                node: 4,
                path: 9,
                content: 2,
            },
            Step::Offline { node: 2 },
            Step::Settle { secs: 20 },
            Step::Online { node: 7 },
            Step::Chmod { node: 2, path: 6 },
            Step::Heal { a: 3, b: 4 },
            Step::Everywhere {
                path: 0,
                contents: vec![None, Some(3)],
            },
            Step::Partition { a: 7, b: 2 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 19,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 4,
            },
            Step::Delete { node: 0, path: 4 },
            Step::MassModify {
                node: 0,
                fraction: 60,
                content: 2,
            },
            Step::Heal { a: 7, b: 3 },
            Step::Create {
                node: 5,
                path: 1,
                content: 4,
            },
            Step::Crash {
                node: 3,
                gap_secs: 32,
            },
            Step::Create {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), None],
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), None, None, Some(4), Some(1), None],
            },
            Step::Delete { node: 0, path: 4 },
            Step::Heal { a: 1, b: 0 },
            Step::Partition { a: 7, b: 2 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(3), Some(5)],
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Touch { node: 6, path: 11 },
            Step::Create {
                node: 7,
                path: 2,
                content: 5,
            },
            Step::Online { node: 1 },
            Step::Chmod { node: 3, path: 8 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 6,
            },
            Step::Create {
                node: 5,
                path: 7,
                content: 5,
            },
            Step::Chmod { node: 7, path: 10 },
            Step::Rename {
                node: 1,
                from: 9,
                to: 2,
            },
            Step::Offline { node: 6 },
            Step::Symlink {
                node: 3,
                path: 9,
                target: 2,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![None, Some(5), None, Some(5), None],
            },
            Step::Offline { node: 0 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 32,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 4,
            },
            Step::Crash {
                node: 0,
                gap_secs: 6,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 48,
            },
            Step::Symlink {
                node: 7,
                path: 10,
                target: 9,
            },
            Step::Delete { node: 0, path: 7 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(2), Some(5), Some(2), Some(2), None],
            },
            Step::Chmod { node: 4, path: 0 },
            Step::Heal { a: 7, b: 6 },
            Step::Rename {
                node: 5,
                from: 11,
                to: 6,
            },
            Step::Rename {
                node: 2,
                from: 5,
                to: 10,
            },
            Step::Online { node: 5 },
            Step::Modify {
                node: 0,
                path: 2,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 10,
                to: 3,
            },
            Step::Offline { node: 6 },
            Step::Settle { secs: 2 },
            Step::Create {
                node: 5,
                path: 11,
                content: 2,
            },
            Step::Online { node: 4 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 5,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(4), Some(1)],
            },
            Step::Delete { node: 3, path: 11 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 76,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 36,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 54,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 6,
                path: 0,
                content: 5,
            },
            Step::Touch { node: 4, path: 3 },
            Step::Create {
                node: 1,
                path: 10,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 9,
                content: 2,
            },
            Step::Delete { node: 3, path: 6 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 5,
            },
            Step::Partition { a: 0, b: 5 },
            Step::Delete { node: 2, path: 0 },
            Step::Offline { node: 1 },
            Step::Create {
                node: 7,
                path: 5,
                content: 5,
            },
            Step::Delete { node: 6, path: 8 },
            Step::Crash {
                node: 4,
                gap_secs: 9,
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 5,
            },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 3,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 49,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(4), Some(5), None, Some(4)],
            },
            Step::Online { node: 1 },
        ],
    );
}

/// Class A, the case a paused-only wait misses. The node was offline, so
/// its window did not tick; its user's deny ran on the unpaused folder,
/// and when the node came back the first tick paused on three local
/// changes with the deny's bumps in the pending batch. The user's revert
/// discarded them and the quarantine they had consumed (I1). revert now
/// returns the held item (§8.3). Shrunk before that; fails without it.
#[test]
fn a_revert_that_discards_a_denys_bumps_returns_its_held_item() {
    passes(
        17187,
        &[
            Step::Partition { a: 4, b: 3 },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 1,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 35,
            },
            Step::Delete { node: 0, path: 3 },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Delete { node: 4, path: 9 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Modify {
                node: 2,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 1,
                content: 4,
            },
            Step::Delete { node: 5, path: 10 },
            Step::Online { node: 4 },
            Step::Partition { a: 1, b: 1 },
            Step::Delete { node: 7, path: 0 },
            Step::Modify {
                node: 4,
                path: 8,
                content: 5,
            },
            Step::Delete { node: 7, path: 11 },
            Step::Online { node: 4 },
            Step::Tier {
                a: 5,
                b: 7,
                tier: 0,
            },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 0,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 4,
            },
            Step::Partition { a: 1, b: 3 },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(2), Some(4)],
            },
            Step::Settle { secs: 9 },
            Step::Touch { node: 5, path: 9 },
            Step::Settle { secs: 14 },
            Step::Create {
                node: 4,
                path: 10,
                content: 5,
            },
            Step::Partition { a: 6, b: 2 },
            Step::Heal { a: 4, b: 1 },
            Step::Settle { secs: 14 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Rename {
                node: 3,
                from: 0,
                to: 6,
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 4,
            },
            Step::Touch { node: 4, path: 6 },
            Step::Chmod { node: 0, path: 3 },
            Step::Touch { node: 1, path: 0 },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Delete { node: 0, path: 3 },
            Step::Partition { a: 2, b: 4 },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Rename {
                node: 1,
                from: 8,
                to: 1,
            },
            Step::Delete { node: 6, path: 4 },
            Step::Delete { node: 2, path: 5 },
            Step::Modify {
                node: 7,
                path: 1,
                content: 4,
            },
            Step::Crash {
                node: 4,
                gap_secs: 75,
            },
            Step::Touch { node: 6, path: 4 },
            Step::Touch { node: 4, path: 2 },
            Step::Offline { node: 1 },
            Step::Create {
                node: 6,
                path: 2,
                content: 1,
            },
            Step::Partition { a: 2, b: 3 },
            Step::Offline { node: 1 },
            Step::Rmdir { node: 3, dir: 0 },
            Step::Create {
                node: 6,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 5,
            },
            Step::Settle { secs: 14 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, Some(3), Some(2), Some(1), Some(4)],
            },
            Step::Modify {
                node: 7,
                path: 2,
                content: 3,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Modify {
                node: 6,
                path: 4,
                content: 3,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), None, Some(2), Some(3), None, None, Some(1)],
            },
            Step::Modify {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Chmod { node: 0, path: 4 },
            Step::Create {
                node: 6,
                path: 5,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 2,
                content: 4,
            },
            Step::Crash {
                node: 3,
                gap_secs: 75,
            },
            Step::Delete { node: 7, path: 10 },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 38,
                },
                delay_secs: 16,
            },
            Step::Symlink {
                node: 6,
                path: 5,
                target: 6,
            },
            Step::Touch { node: 2, path: 3 },
            Step::Offline { node: 7 },
            Step::Offline { node: 7 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 25,
            },
            Step::Partition { a: 5, b: 0 },
            Step::Delete { node: 7, path: 8 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 2,
            },
            Step::Online { node: 5 },
            Step::Partition { a: 1, b: 7 },
            Step::Heal { a: 0, b: 0 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(1), Some(2), Some(1)],
            },
            Step::Delete { node: 4, path: 1 },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Settle { secs: 1 },
            Step::Heal { a: 5, b: 0 },
            Step::Offline { node: 1 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 49,
            },
            Step::Create {
                node: 7,
                path: 7,
                content: 2,
            },
            Step::MassModify {
                node: 3,
                fraction: 73,
                content: 2,
            },
            Step::Rename {
                node: 0,
                from: 1,
                to: 7,
            },
            Step::Rmdir { node: 7, dir: 2 },
            Step::Online { node: 1 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 9,
            },
        ],
    );
}

/// Class B as the 20,000-seed run found it, shrunk before the fixes for
/// §8.3's settled folder. The restart scan, the in-flight occupant rule and
/// the I4 ledger each change its course, so it passes with any one of the
/// new waits disabled; kept as a plain regression.
#[test]
fn a_deny_after_revert_over_an_unobserved_occupant() {
    passes(
        5629,
        &[
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 43,
            },
            Step::Settle { secs: 24 },
            Step::Create {
                node: 5,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 4,
            },
            Step::MassDelete {
                node: 3,
                fraction: 91,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 49,
            },
            Step::Delete { node: 5, path: 10 },
            Step::Delete { node: 0, path: 2 },
            Step::Create {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Rmdir { node: 0, dir: 1 },
            Step::Offline { node: 0 },
            Step::Tier {
                a: 3,
                b: 6,
                tier: 1,
            },
            Step::Modify {
                node: 4,
                path: 7,
                content: 3,
            },
            Step::Partition { a: 6, b: 4 },
            Step::Create {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Delete { node: 4, path: 11 },
            Step::Modify {
                node: 4,
                path: 5,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 3,
                to: 7,
            },
            Step::Delete { node: 3, path: 11 },
            Step::Modify {
                node: 0,
                path: 8,
                content: 1,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Partition { a: 3, b: 4 },
            Step::Settle { secs: 16 },
            Step::Modify {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Online { node: 1 },
            Step::MassDelete {
                node: 7,
                fraction: 52,
            },
            Step::Create {
                node: 6,
                path: 4,
                content: 2,
            },
            Step::Settle { secs: 7 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 1,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(5), None, Some(5), Some(1), Some(4)],
            },
            Step::Modify {
                node: 6,
                path: 5,
                content: 5,
            },
            Step::Settle { secs: 12 },
            Step::Modify {
                node: 2,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Online { node: 0 },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(4), Some(4), Some(5), Some(1), Some(4)],
            },
            Step::Settle { secs: 15 },
            Step::Touch { node: 0, path: 1 },
            Step::MassModify {
                node: 4,
                fraction: 69,
                content: 5,
            },
            Step::Crash {
                node: 1,
                gap_secs: 17,
            },
            Step::Offline { node: 5 },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(2), Some(4), Some(5), Some(4)],
            },
            Step::Rmdir { node: 3, dir: 2 },
            Step::MassModify {
                node: 7,
                fraction: 88,
                content: 5,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), Some(2), Some(1), Some(2), None, Some(2), Some(1)],
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 3,
            },
            Step::Chmod { node: 3, path: 4 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 1,
            },
            Step::Tier {
                a: 5,
                b: 7,
                tier: 0,
            },
            Step::Delete { node: 0, path: 4 },
            Step::Chmod { node: 3, path: 8 },
            Step::Crash {
                node: 1,
                gap_secs: 17,
            },
            Step::Settle { secs: 29 },
            Step::MassModify {
                node: 0,
                fraction: 98,
                content: 3,
            },
            Step::Offline { node: 6 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 26,
            },
            Step::Settle { secs: 1 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 49,
            },
            Step::Create {
                node: 4,
                path: 7,
                content: 1,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Delete { node: 0, path: 7 },
            Step::Online { node: 0 },
            Step::Create {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Symlink {
                node: 3,
                path: 1,
                target: 10,
            },
            Step::Offline { node: 4 },
            Step::Tier {
                a: 2,
                b: 7,
                tier: 2,
            },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 1,
            },
            Step::Delete { node: 7, path: 8 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 7,
            },
            Step::Modify {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 4,
            },
            Step::Crash {
                node: 6,
                gap_secs: 5,
            },
            Step::Settle { secs: 5 },
            Step::Offline { node: 7 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 10,
                content: 4,
            },
            Step::Heal { a: 4, b: 5 },
            Step::Create {
                node: 1,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 5,
            },
            Step::Settle { secs: 33 },
            Step::Crash {
                node: 7,
                gap_secs: 90,
            },
            Step::Delete { node: 7, path: 10 },
            Step::Create {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Settle { secs: 11 },
            Step::Partition { a: 7, b: 3 },
            Step::Tier {
                a: 2,
                b: 7,
                tier: 0,
            },
            Step::Modify {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 2,
                },
                delay_secs: 31,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 1,
            },
            Step::Online { node: 6 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(5), Some(3), Some(3), Some(3)],
            },
            Step::MassDelete {
                node: 5,
                fraction: 65,
            },
            Step::Settle { secs: 13 },
            Step::Settle { secs: 21 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Partition { a: 0, b: 7 },
            Step::Offline { node: 5 },
            Step::Create {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::Tier {
                a: 6,
                b: 0,
                tier: 1,
            },
            Step::Delete { node: 2, path: 4 },
            Step::Partition { a: 0, b: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 5,
            },
            Step::Rmdir { node: 1, dir: 1 },
            Step::MassDelete {
                node: 5,
                fraction: 89,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 47,
            },
            Step::MassModify {
                node: 7,
                fraction: 69,
                content: 1,
            },
            Step::Touch { node: 5, path: 11 },
            Step::Tier {
                a: 5,
                b: 3,
                tier: 2,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 3,
            },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Online { node: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 46,
            },
            Step::Symlink {
                node: 1,
                path: 0,
                target: 9,
            },
            Step::Delete { node: 7, path: 3 },
        ],
    );
}

/// Class C as the run found it: a conflict commit's report lost to a
/// crash on a paused node that then reverts, its copy scanned later as a
/// new add. A plain regression, like 5629.
#[test]
fn a_lost_conflict_commit_on_a_node_that_then_reverts() {
    passes(
        17549,
        &[
            Step::Partition { a: 1, b: 6 },
            Step::Rename {
                node: 2,
                from: 11,
                to: 1,
            },
            Step::Heal { a: 2, b: 2 },
            Step::Rename {
                node: 3,
                from: 0,
                to: 11,
            },
            Step::Tier {
                a: 1,
                b: 4,
                tier: 2,
            },
            Step::Heal { a: 1, b: 5 },
            Step::Create {
                node: 4,
                path: 7,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 2,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::MassModify {
                node: 6,
                fraction: 62,
                content: 1,
            },
            Step::Offline { node: 7 },
            Step::Chmod { node: 5, path: 8 },
            Step::Modify {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::Online { node: 7 },
            Step::Online { node: 0 },
            Step::Create {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::Heal { a: 5, b: 3 },
            Step::MassModify {
                node: 4,
                fraction: 88,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 3,
            },
            Step::Settle { secs: 37 },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 23,
            },
            Step::Delete { node: 7, path: 8 },
            Step::Chmod { node: 1, path: 7 },
            Step::Heal { a: 0, b: 6 },
            Step::Modify {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Settle { secs: 19 },
            Step::Rmdir { node: 1, dir: 2 },
            Step::Chmod { node: 4, path: 10 },
            Step::MassModify {
                node: 6,
                fraction: 86,
                content: 4,
            },
            Step::Rename {
                node: 3,
                from: 9,
                to: 6,
            },
            Step::Chmod { node: 7, path: 0 },
            Step::Online { node: 6 },
            Step::Settle { secs: 16 },
            Step::Delete { node: 1, path: 2 },
            Step::Heal { a: 1, b: 0 },
            Step::Crash {
                node: 6,
                gap_secs: 93,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 5,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Delete { node: 2, path: 10 },
            Step::Settle { secs: 33 },
            Step::MassModify {
                node: 6,
                fraction: 96,
                content: 4,
            },
            Step::Crash {
                node: 0,
                gap_secs: 64,
            },
            Step::Settle { secs: 9 },
            Step::Modify {
                node: 1,
                path: 8,
                content: 2,
            },
            Step::Offline { node: 6 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Tier {
                a: 7,
                b: 5,
                tier: 1,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 39,
            },
            Step::Rename {
                node: 3,
                from: 3,
                to: 0,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Delete { node: 4, path: 8 },
            Step::Modify {
                node: 7,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 5,
            },
            Step::Crash {
                node: 0,
                gap_secs: 106,
            },
            Step::Create {
                node: 7,
                path: 3,
                content: 2,
            },
            Step::Settle { secs: 36 },
            Step::Partition { a: 6, b: 0 },
            Step::Chmod { node: 5, path: 4 },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(4)],
            },
            Step::Settle { secs: 18 },
            Step::Partition { a: 7, b: 4 },
            Step::Modify {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::Settle { secs: 7 },
            Step::Rmdir { node: 4, dir: 2 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(4), Some(5), None, Some(3)],
            },
            Step::Heal { a: 2, b: 5 },
            Step::MassDelete {
                node: 2,
                fraction: 71,
            },
            Step::Delete { node: 6, path: 8 },
            Step::Settle { secs: 4 },
            Step::Delete { node: 6, path: 5 },
            Step::Create {
                node: 6,
                path: 0,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 2,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(5), None],
            },
            Step::Symlink {
                node: 4,
                path: 11,
                target: 9,
            },
            Step::MassDelete {
                node: 5,
                fraction: 73,
            },
            Step::Partition { a: 4, b: 0 },
            Step::Delete { node: 3, path: 10 },
            Step::Partition { a: 6, b: 5 },
            Step::Crash {
                node: 5,
                gap_secs: 63,
            },
            Step::Offline { node: 4 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 1,
            },
            Step::Settle { secs: 6 },
            Step::Chmod { node: 4, path: 2 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(2), Some(5), Some(1), Some(3)],
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 42,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 2,
            },
            Step::MassDelete {
                node: 3,
                fraction: 72,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 1,
            },
            Step::Touch { node: 7, path: 4 },
            Step::Crash {
                node: 3,
                gap_secs: 87,
            },
            Step::Partition { a: 2, b: 0 },
            Step::Crash {
                node: 6,
                gap_secs: 85,
            },
            Step::Touch { node: 1, path: 6 },
            Step::Heal { a: 2, b: 5 },
            Step::Delete { node: 4, path: 4 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 10,
                },
                delay_secs: 25,
            },
            Step::Rename {
                node: 4,
                from: 9,
                to: 5,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 2,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Delete { node: 1, path: 3 },
            Step::Modify {
                node: 0,
                path: 6,
                content: 1,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 32,
            },
            Step::Partition { a: 6, b: 3 },
            Step::Chmod { node: 2, path: 3 },
            Step::Online { node: 0 },
            Step::Create {
                node: 5,
                path: 11,
                content: 1,
            },
            Step::Touch { node: 3, path: 1 },
            Step::Modify {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Create {
                node: 0,
                path: 11,
                content: 3,
            },
            Step::Heal { a: 0, b: 4 },
            Step::Chmod { node: 0, path: 5 },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 5,
            },
            Step::Settle { secs: 30 },
            Step::Create {
                node: 3,
                path: 7,
                content: 1,
            },
            Step::Settle { secs: 28 },
            Step::Partition { a: 4, b: 6 },
            Step::Modify {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 20,
            },
            Step::Partition { a: 5, b: 1 },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 2,
            },
            Step::Chmod { node: 6, path: 8 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(2), Some(2), Some(2), None, Some(2), Some(4), Some(2)],
            },
            Step::Offline { node: 3 },
            Step::MassModify {
                node: 3,
                fraction: 52,
                content: 2,
            },
            Step::Modify {
                node: 4,
                path: 6,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 5,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Heal { a: 7, b: 6 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 5,
                    hold_pct: 10,
                },
                delay_secs: 37,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Modify {
                node: 5,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 1,
                content: 2,
            },
            Step::Partition { a: 6, b: 6 },
            Step::Create {
                node: 3,
                path: 8,
                content: 4,
            },
            Step::Online { node: 4 },
            Step::Crash {
                node: 1,
                gap_secs: 71,
            },
            Step::Touch { node: 1, path: 8 },
            Step::Touch { node: 4, path: 5 },
            Step::Online { node: 2 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 11,
            },
            Step::Online { node: 3 },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Heal { a: 4, b: 7 },
            Step::Delete { node: 2, path: 4 },
            Step::Rename {
                node: 6,
                from: 11,
                to: 4,
            },
            Step::Mkdir { node: 1, dir: 0 },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 39,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 13,
            },
            Step::Touch { node: 1, path: 0 },
            Step::Modify {
                node: 7,
                path: 7,
                content: 3,
            },
            Step::Crash {
                node: 4,
                gap_secs: 38,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(2), Some(4)],
            },
            Step::Heal { a: 4, b: 2 },
            Step::Symlink {
                node: 6,
                path: 0,
                target: 9,
            },
            Step::Settle { secs: 6 },
            Step::Modify {
                node: 3,
                path: 1,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 6,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 4,
                content: 4,
            },
            Step::Delete { node: 2, path: 2 },
            Step::Chmod { node: 3, path: 0 },
            Step::Tier {
                a: 0,
                b: 4,
                tier: 1,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 59,
                },
                delay_secs: 44,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 1,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 4,
            },
            Step::Heal { a: 3, b: 6 },
            Step::Settle { secs: 9 },
            Step::Settle { secs: 4 },
            Step::Partition { a: 4, b: 2 },
            Step::Online { node: 1 },
            Step::Partition { a: 7, b: 1 },
            Step::Create {
                node: 5,
                path: 11,
                content: 2,
            },
            Step::Touch { node: 3, path: 1 },
            Step::Create {
                node: 6,
                path: 7,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 10,
                content: 1,
            },
            Step::Settle { secs: 4 },
            Step::Settle { secs: 17 },
            Step::Delete { node: 0, path: 8 },
            Step::Chmod { node: 0, path: 5 },
            Step::Chmod { node: 2, path: 11 },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![None, Some(1), None, Some(4), Some(2), None],
            },
            Step::Tier {
                a: 0,
                b: 0,
                tier: 0,
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Online { node: 1 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Modify {
                node: 7,
                path: 2,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 3,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), None],
            },
            Step::Partition { a: 5, b: 3 },
            Step::Chmod { node: 7, path: 9 },
            Step::Heal { a: 0, b: 1 },
            Step::Symlink {
                node: 6,
                path: 5,
                target: 1,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(2), Some(3), None, None, Some(4), Some(5)],
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::Settle { secs: 13 },
            Step::Modify {
                node: 6,
                path: 3,
                content: 5,
            },
            Step::Online { node: 4 },
            Step::Modify {
                node: 0,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 1,
            },
            Step::Chmod { node: 3, path: 10 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(1), Some(2), Some(5), Some(2), None, Some(5)],
            },
            Step::Touch { node: 1, path: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 50,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(1), None, Some(4), Some(3)],
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::MassDelete {
                node: 6,
                fraction: 84,
            },
            Step::Heal { a: 1, b: 5 },
            Step::Chmod { node: 3, path: 9 },
            Step::Online { node: 4 },
            Step::Offline { node: 4 },
            Step::Settle { secs: 35 },
            Step::Create {
                node: 5,
                path: 8,
                content: 1,
            },
            Step::Rmdir { node: 5, dir: 1 },
            Step::Delete { node: 0, path: 5 },
            Step::Touch { node: 2, path: 8 },
            Step::Touch { node: 2, path: 4 },
            Step::Symlink {
                node: 1,
                path: 0,
                target: 1,
            },
            Step::MassDelete {
                node: 0,
                fraction: 97,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Tier {
                a: 7,
                b: 7,
                tier: 0,
            },
            Step::Settle { secs: 30 },
            Step::Crash {
                node: 1,
                gap_secs: 63,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 1,
            },
            Step::Delete { node: 7, path: 10 },
            Step::MassDelete {
                node: 2,
                fraction: 91,
            },
            Step::Modify {
                node: 6,
                path: 4,
                content: 4,
            },
            Step::Online { node: 4 },
            Step::Online { node: 1 },
            Step::Settle { secs: 37 },
            Step::Create {
                node: 2,
                path: 6,
                content: 4,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![None, Some(4), None, Some(1), Some(2), None, Some(2)],
            },
            Step::Settle { secs: 38 },
            Step::Delete { node: 1, path: 7 },
            Step::Create {
                node: 5,
                path: 2,
                content: 1,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), Some(3), None, None, Some(5)],
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(5)],
            },
            Step::Create {
                node: 0,
                path: 4,
                content: 1,
            },
            Step::Create {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 52,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 1,
            },
            Step::MassModify {
                node: 4,
                fraction: 50,
                content: 2,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Create {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 6,
                path: 3,
                content: 1,
            },
            Step::Chmod { node: 1, path: 4 },
            Step::Create {
                node: 5,
                path: 6,
                content: 1,
            },
            Step::Settle { secs: 18 },
            Step::Heal { a: 3, b: 2 },
            Step::Offline { node: 2 },
            Step::Symlink {
                node: 3,
                path: 5,
                target: 2,
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(3), Some(5), None, Some(5)],
            },
            Step::Delete { node: 4, path: 1 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(1), None, Some(5), Some(5), Some(1), None],
            },
            Step::Partition { a: 0, b: 5 },
            Step::Tier {
                a: 0,
                b: 3,
                tier: 2,
            },
            Step::Heal { a: 0, b: 3 },
            Step::Delete { node: 0, path: 11 },
            Step::Partition { a: 7, b: 5 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 3,
            },
            Step::Crash {
                node: 1,
                gap_secs: 55,
            },
            Step::Offline { node: 2 },
            Step::Modify {
                node: 7,
                path: 5,
                content: 2,
            },
            Step::Crash {
                node: 6,
                gap_secs: 38,
            },
            Step::Offline { node: 4 },
            Step::Crash {
                node: 7,
                gap_secs: 7,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 19,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 5,
            },
        ],
    );
}

/// The same shape as 17549 on another node and path.
#[test]
fn another_lost_conflict_commit_on_a_node_that_then_reverts() {
    passes(
        13885,
        &[
            Step::Crash {
                node: 0,
                gap_secs: 98,
            },
            Step::Chmod { node: 3, path: 7 },
            Step::Modify {
                node: 7,
                path: 1,
                content: 5,
            },
            Step::Partition { a: 2, b: 1 },
            Step::Settle { secs: 1 },
            Step::MassModify {
                node: 1,
                fraction: 96,
                content: 5,
            },
            Step::Crash {
                node: 2,
                gap_secs: 104,
            },
            Step::Chmod { node: 1, path: 0 },
            Step::Heal { a: 2, b: 2 },
            Step::Create {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::Settle { secs: 24 },
            Step::Partition { a: 3, b: 4 },
            Step::Create {
                node: 1,
                path: 11,
                content: 3,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 35,
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Chmod { node: 0, path: 10 },
            Step::Heal { a: 6, b: 1 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 2,
                content: 2,
            },
            Step::MassDelete {
                node: 2,
                fraction: 60,
            },
            Step::Chmod { node: 3, path: 7 },
            Step::Create {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Delete { node: 2, path: 7 },
            Step::Create {
                node: 0,
                path: 2,
                content: 5,
            },
            Step::Settle { secs: 32 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 1,
            },
            Step::Heal { a: 6, b: 1 },
            Step::Create {
                node: 4,
                path: 11,
                content: 4,
            },
            Step::Touch { node: 2, path: 0 },
            Step::Touch { node: 4, path: 7 },
            Step::Delete { node: 1, path: 9 },
            Step::Chmod { node: 3, path: 10 },
            Step::MassModify {
                node: 2,
                fraction: 81,
                content: 3,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 57,
                },
                delay_secs: 4,
            },
            Step::Tier {
                a: 3,
                b: 5,
                tier: 2,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Offline { node: 0 },
            Step::Modify {
                node: 1,
                path: 10,
                content: 1,
            },
            Step::Delete { node: 0, path: 1 },
            Step::Create {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Offline { node: 0 },
            Step::Settle { secs: 2 },
            Step::Modify {
                node: 0,
                path: 10,
                content: 3,
            },
            Step::Delete { node: 2, path: 9 },
            Step::Crash {
                node: 3,
                gap_secs: 90,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 1,
            },
            Step::Delete { node: 7, path: 0 },
            Step::MassDelete {
                node: 2,
                fraction: 98,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(5), Some(2), None, Some(1)],
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 34,
                },
                delay_secs: 15,
            },
            Step::Online { node: 2 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 24,
            },
            Step::Delete { node: 4, path: 0 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 9,
                content: 5,
            },
            Step::Chmod { node: 5, path: 4 },
            Step::Modify {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 3,
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 7,
                content: 1,
            },
            Step::Crash {
                node: 0,
                gap_secs: 92,
            },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Create {
                node: 7,
                path: 2,
                content: 4,
            },
            Step::Rename {
                node: 3,
                from: 0,
                to: 0,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 1,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 22,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Chmod { node: 6, path: 2 },
            Step::Partition { a: 6, b: 4 },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 2,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 1,
            },
            Step::Partition { a: 6, b: 7 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 29,
            },
            Step::Offline { node: 3 },
            Step::Tier {
                a: 7,
                b: 5,
                tier: 0,
            },
            Step::Settle { secs: 1 },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Create {
                node: 3,
                path: 10,
                content: 1,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Rename {
                node: 5,
                from: 7,
                to: 8,
            },
            Step::Crash {
                node: 1,
                gap_secs: 85,
            },
            Step::Symlink {
                node: 2,
                path: 9,
                target: 10,
            },
            Step::Touch { node: 4, path: 8 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Offline { node: 7 },
            Step::Chmod { node: 2, path: 7 },
            Step::Everywhere {
                path: 0,
                contents: vec![None, None, None],
            },
            Step::Delete { node: 6, path: 6 },
            Step::Tier {
                a: 0,
                b: 5,
                tier: 0,
            },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 1,
            },
            Step::Crash {
                node: 5,
                gap_secs: 57,
            },
            Step::MassDelete {
                node: 6,
                fraction: 85,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(1), None, None, Some(2)],
            },
            Step::Heal { a: 4, b: 5 },
            Step::Chmod { node: 2, path: 9 },
            Step::Heal { a: 4, b: 6 },
            Step::Chmod { node: 7, path: 8 },
            Step::Create {
                node: 5,
                path: 8,
                content: 5,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Create {
                node: 4,
                path: 9,
                content: 2,
            },
            Step::Settle { secs: 5 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Tier {
                a: 1,
                b: 6,
                tier: 0,
            },
            Step::Settle { secs: 36 },
            Step::Partition { a: 0, b: 1 },
            Step::Rmdir { node: 5, dir: 1 },
            Step::Partition { a: 5, b: 1 },
            Step::Create {
                node: 2,
                path: 2,
                content: 1,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 10,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Chmod { node: 5, path: 6 },
            Step::Heal { a: 6, b: 2 },
            Step::Touch { node: 0, path: 9 },
            Step::Heal { a: 3, b: 3 },
            Step::Delete { node: 2, path: 9 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 1,
            },
            Step::Create {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 11,
                content: 3,
            },
            Step::Partition { a: 6, b: 2 },
            Step::Rename {
                node: 1,
                from: 11,
                to: 2,
            },
            Step::Chmod { node: 1, path: 2 },
            Step::Modify {
                node: 2,
                path: 11,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 2,
            },
            Step::Rmdir { node: 7, dir: 1 },
            Step::Partition { a: 5, b: 7 },
            Step::Create {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Online { node: 6 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), Some(2), Some(2), Some(2), None, Some(5), Some(3)],
            },
            Step::Touch { node: 4, path: 9 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(5), Some(5), None, Some(2)],
            },
            Step::Tier {
                a: 0,
                b: 6,
                tier: 0,
            },
            Step::Touch { node: 7, path: 5 },
            Step::Rename {
                node: 6,
                from: 8,
                to: 0,
            },
            Step::Delete { node: 6, path: 3 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Delete { node: 3, path: 8 },
            Step::Rename {
                node: 4,
                from: 10,
                to: 10,
            },
            Step::Crash {
                node: 3,
                gap_secs: 111,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 38,
            },
            Step::Chmod { node: 5, path: 8 },
            Step::MassModify {
                node: 3,
                fraction: 98,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Heal { a: 7, b: 1 },
            Step::Create {
                node: 4,
                path: 3,
                content: 5,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 44,
            },
            Step::Chmod { node: 0, path: 7 },
            Step::Tier {
                a: 0,
                b: 1,
                tier: 0,
            },
            Step::Online { node: 3 },
            Step::Crash {
                node: 0,
                gap_secs: 28,
            },
            Step::Chmod { node: 2, path: 4 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Create {
                node: 3,
                path: 6,
                content: 5,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 9,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Touch { node: 3, path: 3 },
            Step::Partition { a: 6, b: 3 },
            Step::Create {
                node: 0,
                path: 0,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 6,
            },
            Step::Online { node: 2 },
            Step::Delete { node: 2, path: 10 },
            Step::Heal { a: 1, b: 2 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Create {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Partition { a: 6, b: 6 },
            Step::Partition { a: 4, b: 2 },
            Step::Touch { node: 1, path: 3 },
            Step::Create {
                node: 7,
                path: 1,
                content: 2,
            },
            Step::Offline { node: 3 },
            Step::Symlink {
                node: 0,
                path: 9,
                target: 9,
            },
            Step::Symlink {
                node: 1,
                path: 11,
                target: 3,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 11,
                content: 5,
            },
            Step::Settle { secs: 39 },
            Step::Heal { a: 6, b: 2 },
            Step::Create {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Settle { secs: 18 },
            Step::Modify {
                node: 5,
                path: 10,
                content: 5,
            },
            Step::Rename {
                node: 2,
                from: 2,
                to: 8,
            },
            Step::MassDelete {
                node: 7,
                fraction: 52,
            },
            Step::Delete { node: 7, path: 9 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 1,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Delete { node: 0, path: 0 },
            Step::MassDelete {
                node: 6,
                fraction: 77,
            },
            Step::Modify {
                node: 3,
                path: 8,
                content: 4,
            },
            Step::Touch { node: 7, path: 6 },
            Step::Modify {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Heal { a: 7, b: 0 },
            Step::Modify {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 4,
            },
            Step::Chmod { node: 4, path: 11 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 5,
                    hold_pct: 13,
                },
                delay_secs: 40,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 3,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Heal { a: 0, b: 2 },
            Step::Partition { a: 6, b: 6 },
            Step::Crash {
                node: 5,
                gap_secs: 30,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 29,
            },
            Step::Settle { secs: 28 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Touch { node: 4, path: 10 },
            Step::Create {
                node: 5,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Create {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 2,
            },
            Step::Delete { node: 5, path: 5 },
            Step::Create {
                node: 3,
                path: 10,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 5,
                path: 11,
                content: 3,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Online { node: 6 },
            Step::Modify {
                node: 2,
                path: 3,
                content: 5,
            },
            Step::Online { node: 6 },
            Step::Heal { a: 5, b: 3 },
            Step::Delete { node: 6, path: 9 },
            Step::Settle { secs: 9 },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Everywhere {
                path: 3,
                contents: vec![
                    Some(4),
                    Some(2),
                    Some(4),
                    Some(5),
                    Some(1),
                    Some(5),
                    Some(1),
                ],
            },
            Step::Chmod { node: 2, path: 4 },
            Step::Online { node: 1 },
            Step::Delete { node: 3, path: 5 },
            Step::Create {
                node: 4,
                path: 3,
                content: 2,
            },
            Step::Delete { node: 0, path: 9 },
            Step::Create {
                node: 2,
                path: 2,
                content: 2,
            },
            Step::MassModify {
                node: 6,
                fraction: 50,
                content: 1,
            },
            Step::Delete { node: 7, path: 10 },
            Step::Delete { node: 0, path: 1 },
            Step::Offline { node: 7 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Tier {
                a: 5,
                b: 5,
                tier: 2,
            },
            Step::Partition { a: 1, b: 5 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 2,
            },
            Step::Rename {
                node: 3,
                from: 4,
                to: 2,
            },
            Step::Delete { node: 2, path: 11 },
            Step::Heal { a: 7, b: 7 },
            Step::Everywhere {
                path: 4,
                contents: vec![None, None, None, None],
            },
            Step::Online { node: 4 },
            Step::Touch { node: 2, path: 3 },
            Step::Chmod { node: 3, path: 2 },
            Step::Modify {
                node: 7,
                path: 0,
                content: 5,
            },
            Step::Heal { a: 6, b: 7 },
            Step::Create {
                node: 4,
                path: 11,
                content: 3,
            },
            Step::Delete { node: 3, path: 7 },
            Step::Chmod { node: 0, path: 0 },
            Step::Everywhere {
                path: 4,
                contents: vec![None, None],
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 5,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Modify {
                node: 6,
                path: 2,
                content: 1,
            },
            Step::Delete { node: 0, path: 2 },
            Step::Touch { node: 5, path: 0 },
            Step::Rename {
                node: 3,
                from: 0,
                to: 7,
            },
            Step::Delete { node: 4, path: 11 },
            Step::Partition { a: 0, b: 5 },
            Step::Settle { secs: 32 },
            Step::Delete { node: 3, path: 1 },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(1), Some(3), None],
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Symlink {
                node: 1,
                path: 7,
                target: 11,
            },
            Step::Symlink {
                node: 4,
                path: 2,
                target: 3,
            },
            Step::Touch { node: 0, path: 1 },
            Step::Rename {
                node: 0,
                from: 6,
                to: 1,
            },
            Step::Tier {
                a: 3,
                b: 4,
                tier: 0,
            },
            Step::Partition { a: 0, b: 2 },
            Step::Delete { node: 6, path: 6 },
            Step::Offline { node: 6 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(5), Some(4)],
            },
            Step::Offline { node: 4 },
            Step::Heal { a: 4, b: 1 },
            Step::Modify {
                node: 6,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 11,
            },
            Step::Delete { node: 1, path: 0 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 1,
            },
            Step::Crash {
                node: 7,
                gap_secs: 15,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 1,
            },
            Step::Rename {
                node: 2,
                from: 3,
                to: 2,
            },
            Step::Settle { secs: 24 },
            Step::MassModify {
                node: 3,
                fraction: 99,
                content: 3,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(4), Some(1), None, Some(2), Some(4), Some(2), Some(5)],
            },
            Step::Touch { node: 7, path: 5 },
            Step::Delete { node: 0, path: 4 },
        ],
    );
}

/// Seed 90057 of the first 100,000-seed run, with fetch corruption on,
/// shrunk to 23 steps: a want gave up after hash mismatches from two
/// sources and stayed given up while those sources reconnected, so the
/// file never arrived (§7.5).
#[test]
fn a_want_that_gave_up_is_wanted_again() {
    passes_with(
        90057,
        &corrupting(),
        &[
            Step::Delete { node: 6, path: 7 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 11,
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 5,
            },
            Step::Delete { node: 5, path: 1 },
            Step::MassModify {
                node: 1,
                fraction: 86,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 4,
            },
            Step::Touch { node: 4, path: 3 },
            Step::Symlink {
                node: 2,
                path: 0,
                target: 2,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 3,
            },
            Step::Rename {
                node: 4,
                from: 2,
                to: 5,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 40,
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Partition { a: 0, b: 4 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(5), Some(2), Some(3), None],
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 1,
            },
            Step::MassDelete {
                node: 7,
                fraction: 55,
            },
            Step::Modify {
                node: 0,
                path: 0,
                content: 3,
            },
            Step::Rename {
                node: 7,
                from: 4,
                to: 5,
            },
            Step::Delete { node: 6, path: 1 },
            Step::Partition { a: 0, b: 5 },
            Step::Create {
                node: 6,
                path: 0,
                content: 2,
            },
            Step::Symlink {
                node: 6,
                path: 10,
                target: 9,
            },
        ],
    );
}

/// Seed 97943, fetch corruption on, 387 steps: one corrupted transfer
/// excluded the only source of a want for good, and the want waited without
/// a source to the end of the run (§7.5).
#[test]
fn a_source_excluded_after_one_mismatch_is_asked_again() {
    passes_with(
        97943,
        &corrupting(),
        &[
            Step::Modify {
                node: 4,
                path: 8,
                content: 1,
            },
            Step::MassModify {
                node: 1,
                fraction: 53,
                content: 4,
            },
            Step::Settle { secs: 28 },
            Step::MassDelete {
                node: 0,
                fraction: 51,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 1,
            },
            Step::Settle { secs: 4 },
            Step::Mkdir { node: 6, dir: 2 },
            Step::Offline { node: 0 },
            Step::Create {
                node: 3,
                path: 4,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 5,
                content: 3,
            },
            Step::Tier {
                a: 1,
                b: 5,
                tier: 0,
            },
            Step::Settle { secs: 29 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Settle { secs: 6 },
            Step::Rename {
                node: 7,
                from: 11,
                to: 9,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 4,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 23,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Settle { secs: 22 },
            Step::Tier {
                a: 1,
                b: 3,
                tier: 2,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 4,
            },
            Step::Delete { node: 7, path: 4 },
            Step::Modify {
                node: 5,
                path: 9,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 10,
                content: 5,
            },
            Step::Rename {
                node: 4,
                from: 1,
                to: 5,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 4,
            },
            Step::Symlink {
                node: 2,
                path: 2,
                target: 11,
            },
            Step::Settle { secs: 37 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 5,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Settle { secs: 24 },
            Step::Delete { node: 6, path: 11 },
            Step::Touch { node: 7, path: 5 },
            Step::Delete { node: 1, path: 1 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(1), Some(4), None, Some(2), Some(1), Some(4), Some(2)],
            },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), None, Some(3), None, None, None],
            },
            Step::Heal { a: 7, b: 3 },
            Step::Chmod { node: 5, path: 4 },
            Step::Delete { node: 6, path: 10 },
            Step::Heal { a: 0, b: 6 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 1,
            },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 1,
            },
            Step::Partition { a: 7, b: 0 },
            Step::Create {
                node: 3,
                path: 3,
                content: 4,
            },
            Step::Crash {
                node: 1,
                gap_secs: 56,
            },
            Step::Partition { a: 2, b: 7 },
            Step::Create {
                node: 1,
                path: 11,
                content: 5,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![
                    Some(2),
                    Some(1),
                    Some(2),
                    Some(3),
                    Some(1),
                    Some(4),
                    Some(1),
                ],
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(3), Some(4), Some(3), None, Some(1)],
            },
            Step::Online { node: 7 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(3), Some(4), None, Some(4), Some(3), Some(5)],
            },
            Step::Chmod { node: 4, path: 8 },
            Step::Settle { secs: 5 },
            Step::Partition { a: 0, b: 3 },
            Step::Crash {
                node: 5,
                gap_secs: 111,
            },
            Step::Modify {
                node: 7,
                path: 7,
                content: 3,
            },
            Step::Rmdir { node: 7, dir: 2 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 59,
            },
            Step::Delete { node: 5, path: 3 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Online { node: 6 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 27,
            },
            Step::Create {
                node: 4,
                path: 7,
                content: 5,
            },
            Step::Rename {
                node: 7,
                from: 1,
                to: 6,
            },
            Step::Heal { a: 3, b: 5 },
            Step::Create {
                node: 4,
                path: 3,
                content: 2,
            },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 1,
            },
            Step::Settle { secs: 32 },
            Step::Chmod { node: 7, path: 8 },
            Step::Delete { node: 3, path: 0 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Crash {
                node: 1,
                gap_secs: 80,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Create {
                node: 4,
                path: 7,
                content: 5,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Heal { a: 4, b: 5 },
            Step::Online { node: 2 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::Crash {
                node: 4,
                gap_secs: 91,
            },
            Step::Delete { node: 2, path: 6 },
            Step::Rename {
                node: 1,
                from: 0,
                to: 5,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 48,
            },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 10,
            },
            Step::Chmod { node: 4, path: 4 },
            Step::Partition { a: 3, b: 7 },
            Step::Rename {
                node: 2,
                from: 5,
                to: 2,
            },
            Step::Heal { a: 5, b: 3 },
            Step::Settle { secs: 25 },
            Step::Delete { node: 3, path: 5 },
            Step::Heal { a: 0, b: 5 },
            Step::Online { node: 6 },
            Step::Partition { a: 0, b: 4 },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Delete { node: 0, path: 7 },
            Step::Create {
                node: 5,
                path: 11,
                content: 1,
            },
            Step::Heal { a: 6, b: 2 },
            Step::Delete { node: 2, path: 1 },
            Step::Heal { a: 4, b: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 6,
            },
            Step::Partition { a: 7, b: 0 },
            Step::Delete { node: 4, path: 6 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(3), Some(3), Some(4), None, Some(3), None],
            },
            Step::Delete { node: 7, path: 5 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 52,
            },
            Step::Heal { a: 6, b: 3 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 39,
            },
            Step::Online { node: 1 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), Some(4), Some(2), Some(4)],
            },
            Step::Delete { node: 2, path: 3 },
            Step::Delete { node: 5, path: 1 },
            Step::Online { node: 4 },
            Step::Delete { node: 6, path: 7 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 2,
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 5,
            },
            Step::Crash {
                node: 0,
                gap_secs: 66,
            },
            Step::Heal { a: 7, b: 7 },
            Step::Settle { secs: 32 },
            Step::Create {
                node: 2,
                path: 3,
                content: 4,
            },
            Step::Heal { a: 0, b: 4 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 1,
            },
            Step::Touch { node: 5, path: 4 },
            Step::Partition { a: 4, b: 1 },
            Step::Heal { a: 3, b: 1 },
            Step::Create {
                node: 7,
                path: 4,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 1,
                content: 5,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 21,
            },
            Step::Rmdir { node: 1, dir: 0 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 1,
            },
            Step::Touch { node: 3, path: 1 },
            Step::Create {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Delete { node: 6, path: 2 },
            Step::Offline { node: 4 },
            Step::Delete { node: 5, path: 3 },
            Step::Touch { node: 0, path: 5 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, None, Some(2), Some(5), Some(2), Some(2), None],
            },
            Step::Tier {
                a: 1,
                b: 0,
                tier: 2,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), Some(2), Some(2), None, Some(2)],
            },
            Step::Create {
                node: 3,
                path: 4,
                content: 3,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 56,
                },
                delay_secs: 19,
            },
            Step::Delete { node: 2, path: 10 },
            Step::Symlink {
                node: 6,
                path: 9,
                target: 1,
            },
            Step::Create {
                node: 5,
                path: 4,
                content: 4,
            },
            Step::MassDelete {
                node: 6,
                fraction: 91,
            },
            Step::Partition { a: 7, b: 1 },
            Step::Partition { a: 1, b: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 9,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 4,
            },
            Step::Create {
                node: 5,
                path: 3,
                content: 5,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 59,
            },
            Step::Crash {
                node: 1,
                gap_secs: 32,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Rename {
                node: 7,
                from: 6,
                to: 1,
            },
            Step::Partition { a: 4, b: 7 },
            Step::Heal { a: 6, b: 1 },
            Step::Rmdir { node: 7, dir: 0 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 36,
            },
            Step::Heal { a: 7, b: 4 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(3), Some(3), None],
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 19,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Delete { node: 4, path: 2 },
            Step::Modify {
                node: 4,
                path: 0,
                content: 4,
            },
            Step::Heal { a: 2, b: 6 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 5,
            },
            Step::Heal { a: 2, b: 6 },
            Step::MassModify {
                node: 4,
                fraction: 98,
                content: 5,
            },
            Step::Chmod { node: 3, path: 9 },
            Step::Modify {
                node: 3,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Delete { node: 0, path: 0 },
            Step::Chmod { node: 2, path: 6 },
            Step::Delete { node: 5, path: 9 },
            Step::Create {
                node: 4,
                path: 9,
                content: 5,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![None, Some(1), Some(3), None, Some(5), Some(1)],
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 0,
            },
            Step::Online { node: 0 },
            Step::Chmod { node: 3, path: 7 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Heal { a: 3, b: 1 },
            Step::Online { node: 6 },
            Step::Delete { node: 0, path: 10 },
            Step::Crash {
                node: 1,
                gap_secs: 95,
            },
            Step::Settle { secs: 33 },
            Step::Create {
                node: 1,
                path: 7,
                content: 1,
            },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Create {
                node: 6,
                path: 9,
                content: 1,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), Some(1), None],
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), Some(4), Some(2), Some(2)],
            },
            Step::Settle { secs: 3 },
            Step::Rename {
                node: 1,
                from: 1,
                to: 2,
            },
            Step::Create {
                node: 7,
                path: 5,
                content: 1,
            },
            Step::Delete { node: 3, path: 3 },
            Step::Delete { node: 3, path: 2 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 57,
            },
            Step::MassDelete {
                node: 2,
                fraction: 67,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Partition { a: 4, b: 1 },
            Step::MassModify {
                node: 5,
                fraction: 59,
                content: 1,
            },
            Step::Settle { secs: 14 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 4,
            },
            Step::Rename {
                node: 0,
                from: 3,
                to: 4,
            },
            Step::Create {
                node: 2,
                path: 6,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Heal { a: 4, b: 7 },
            Step::MassModify {
                node: 6,
                fraction: 93,
                content: 5,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 56,
                },
                delay_secs: 27,
            },
            Step::Heal { a: 1, b: 6 },
            Step::Create {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Partition { a: 3, b: 3 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 5,
            },
            Step::Crash {
                node: 7,
                gap_secs: 70,
            },
            Step::Chmod { node: 4, path: 0 },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Crash {
                node: 2,
                gap_secs: 67,
            },
            Step::Crash {
                node: 0,
                gap_secs: 50,
            },
            Step::Settle { secs: 31 },
            Step::Create {
                node: 3,
                path: 6,
                content: 2,
            },
            Step::MassDelete {
                node: 1,
                fraction: 85,
            },
            Step::Chmod { node: 1, path: 11 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 54,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(4), Some(4), Some(1), Some(5), None, None],
            },
            Step::Heal { a: 7, b: 4 },
            Step::Create {
                node: 6,
                path: 0,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Offline { node: 5 },
            Step::Create {
                node: 4,
                path: 2,
                content: 1,
            },
            Step::Create {
                node: 0,
                path: 1,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(2), None, Some(2), Some(4), Some(1)],
            },
            Step::MassModify {
                node: 3,
                fraction: 92,
                content: 3,
            },
            Step::MassModify {
                node: 3,
                fraction: 66,
                content: 3,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 22,
            },
            Step::Create {
                node: 4,
                path: 11,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 25,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Create {
                node: 4,
                path: 6,
                content: 3,
            },
            Step::Rename {
                node: 3,
                from: 11,
                to: 4,
            },
            Step::Tier {
                a: 0,
                b: 3,
                tier: 1,
            },
            Step::Crash {
                node: 6,
                gap_secs: 57,
            },
            Step::Settle { secs: 7 },
            Step::Settle { secs: 18 },
            Step::Create {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Settle { secs: 1 },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 7,
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 1,
            },
            Step::Heal { a: 4, b: 2 },
            Step::Chmod { node: 4, path: 10 },
            Step::Settle { secs: 17 },
            Step::Create {
                node: 7,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Tier {
                a: 2,
                b: 1,
                tier: 2,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 55,
            },
            Step::Tier {
                a: 2,
                b: 4,
                tier: 2,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 5,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 1,
            },
            Step::Partition { a: 4, b: 3 },
            Step::Touch { node: 6, path: 2 },
            Step::Create {
                node: 0,
                path: 1,
                content: 3,
            },
            Step::Delete { node: 6, path: 1 },
            Step::MassModify {
                node: 1,
                fraction: 61,
                content: 1,
            },
            Step::Partition { a: 0, b: 1 },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Touch { node: 0, path: 5 },
            Step::Modify {
                node: 7,
                path: 10,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Touch { node: 6, path: 2 },
            Step::Delete { node: 5, path: 4 },
            Step::Heal { a: 1, b: 1 },
            Step::Delete { node: 7, path: 7 },
            Step::Modify {
                node: 1,
                path: 7,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Chmod { node: 0, path: 0 },
            Step::Delete { node: 0, path: 10 },
            Step::Delete { node: 1, path: 7 },
            Step::Settle { secs: 8 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(4), None, None, Some(5), None, None],
            },
            Step::Heal { a: 7, b: 2 },
            Step::Settle { secs: 36 },
            Step::Tier {
                a: 7,
                b: 4,
                tier: 0,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 41,
                },
                delay_secs: 56,
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Everywhere {
                path: 10,
                contents: vec![None, None],
            },
            Step::Touch { node: 1, path: 3 },
            Step::Partition { a: 1, b: 7 },
            Step::Online { node: 5 },
            Step::Create {
                node: 0,
                path: 0,
                content: 5,
            },
            Step::Rename {
                node: 0,
                from: 0,
                to: 6,
            },
            Step::Touch { node: 1, path: 9 },
            Step::Settle { secs: 9 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 0,
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Symlink {
                node: 1,
                path: 8,
                target: 3,
            },
            Step::Partition { a: 4, b: 3 },
            Step::Settle { secs: 29 },
            Step::Settle { secs: 37 },
            Step::Create {
                node: 4,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 4,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Settle { secs: 8 },
            Step::Settle { secs: 12 },
            Step::Partition { a: 3, b: 7 },
            Step::Settle { secs: 4 },
            Step::Create {
                node: 5,
                path: 8,
                content: 5,
            },
            Step::Partition { a: 4, b: 7 },
            Step::Rename {
                node: 6,
                from: 9,
                to: 1,
            },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 3,
            },
            Step::Delete { node: 4, path: 4 },
            Step::Rename {
                node: 1,
                from: 2,
                to: 5,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(2), Some(3), Some(3), Some(1), None, None, Some(2)],
            },
            Step::Offline { node: 7 },
            Step::Create {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::Touch { node: 2, path: 9 },
            Step::Modify {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 16,
            },
            Step::MassDelete {
                node: 2,
                fraction: 52,
            },
            Step::Delete { node: 7, path: 0 },
            Step::Heal { a: 3, b: 2 },
            Step::Delete { node: 5, path: 6 },
            Step::Settle { secs: 23 },
            Step::Settle { secs: 8 },
            Step::Create {
                node: 6,
                path: 7,
                content: 4,
            },
            Step::Modify {
                node: 6,
                path: 10,
                content: 3,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Create {
                node: 0,
                path: 3,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 10,
                content: 3,
            },
            Step::Delete { node: 5, path: 11 },
            Step::Heal { a: 1, b: 4 },
            Step::Chmod { node: 0, path: 7 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 59,
            },
            Step::Partition { a: 7, b: 7 },
            Step::Crash {
                node: 4,
                gap_secs: 43,
            },
            Step::Create {
                node: 4,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Delete { node: 5, path: 1 },
            Step::Modify {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Settle { secs: 38 },
            Step::Rmdir { node: 6, dir: 2 },
            Step::Online { node: 4 },
            Step::Delete { node: 5, path: 4 },
            Step::Tier {
                a: 1,
                b: 2,
                tier: 2,
            },
            Step::Chmod { node: 0, path: 8 },
            Step::MassDelete {
                node: 5,
                fraction: 60,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 4,
            },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Create {
                node: 5,
                path: 8,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 5,
            },
            Step::Touch { node: 0, path: 3 },
            Step::Modify {
                node: 7,
                path: 11,
                content: 3,
            },
            Step::Touch { node: 0, path: 10 },
            Step::Settle { secs: 9 },
            Step::Heal { a: 7, b: 0 },
            Step::Create {
                node: 7,
                path: 6,
                content: 5,
            },
            Step::Settle { secs: 18 },
            Step::Delete { node: 3, path: 3 },
            Step::MassModify {
                node: 7,
                fraction: 54,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 5,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 0,
                content: 1,
            },
            Step::MassModify {
                node: 3,
                fraction: 55,
                content: 2,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Partition { a: 2, b: 2 },
            Step::Modify {
                node: 3,
                path: 6,
                content: 3,
            },
            Step::Rename {
                node: 3,
                from: 9,
                to: 1,
            },
            Step::Tier {
                a: 4,
                b: 7,
                tier: 2,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 4,
            },
            Step::Touch { node: 4, path: 11 },
        ],
    );
}

/// Seed 95175, fetch corruption on, 16 steps: after a revert, the only
/// source of a want served one corrupted transfer and was never asked again
/// (§7.5). Shrunk, it needs the expiry as well as draft 31's events.
#[test]
fn a_mismatch_after_a_revert_does_not_strand_the_file() {
    passes_with(
        95175,
        &corrupting(),
        &[
            Step::Everywhere {
                path: 3,
                contents: vec![Some(3), None, None, None, Some(4), Some(5), Some(3)],
            },
            Step::Modify {
                node: 2,
                path: 11,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 10,
                },
                delay_secs: 38,
            },
            Step::Online { node: 1 },
            Step::Settle { secs: 3 },
            Step::Rename {
                node: 5,
                from: 8,
                to: 4,
            },
            Step::Settle { secs: 29 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 14,
            },
            Step::Touch { node: 2, path: 9 },
            Step::Touch { node: 1, path: 4 },
            Step::Heal { a: 0, b: 2 },
            Step::Rename {
                node: 0,
                from: 0,
                to: 4,
            },
            Step::Heal { a: 6, b: 2 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Touch { node: 6, path: 2 },
            Step::Tier {
                a: 2,
                b: 3,
                tier: 0,
            },
        ],
    );
}

/// Seed 94206, fetch corruption on, 87 steps: the only source of a want
/// served one corrupted transfer, then stayed connected and never announced
/// the path again, and nothing changed at the path, so none of draft 31's
/// events released it. Only the expiry does (§7.5, draft 32); this fails
/// with the expiry disabled.
#[test]
fn an_exclusion_expires_for_a_file() {
    passes_with(
        94206,
        &corrupting(),
        &[
            Step::Heal { a: 0, b: 2 },
            Step::Modify {
                node: 2,
                path: 5,
                content: 1,
            },
            Step::Offline { node: 1 },
            Step::MassDelete {
                node: 2,
                fraction: 56,
            },
            Step::Crash {
                node: 1,
                gap_secs: 106,
            },
            Step::Settle { secs: 37 },
            Step::Crash {
                node: 7,
                gap_secs: 36,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 2,
            },
            Step::Chmod { node: 6, path: 2 },
            Step::Online { node: 7 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 1,
                path: 10,
                content: 2,
            },
            Step::Modify {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Tier {
                a: 5,
                b: 4,
                tier: 0,
            },
            Step::Modify {
                node: 5,
                path: 8,
                content: 2,
            },
            Step::MassDelete {
                node: 7,
                fraction: 77,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 59,
            },
            Step::Settle { secs: 15 },
            Step::Create {
                node: 4,
                path: 1,
                content: 5,
            },
            Step::Symlink {
                node: 0,
                path: 11,
                target: 4,
            },
            Step::Crash {
                node: 0,
                gap_secs: 53,
            },
            Step::Partition { a: 1, b: 6 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Touch { node: 1, path: 9 },
            Step::Tier {
                a: 3,
                b: 7,
                tier: 1,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Delete { node: 7, path: 9 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 7,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 1,
                content: 5,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(1), None],
            },
            Step::Online { node: 2 },
            Step::Delete { node: 3, path: 1 },
            Step::Create {
                node: 3,
                path: 2,
                content: 3,
            },
            Step::Partition { a: 7, b: 6 },
            Step::Create {
                node: 2,
                path: 6,
                content: 5,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 52,
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 5,
            },
            Step::Crash {
                node: 1,
                gap_secs: 39,
            },
            Step::Modify {
                node: 4,
                path: 4,
                content: 3,
            },
            Step::Tier {
                a: 7,
                b: 7,
                tier: 1,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 1,
            },
            Step::Rename {
                node: 6,
                from: 4,
                to: 8,
            },
            Step::Heal { a: 5, b: 6 },
            Step::Modify {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 3,
                path: 7,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 6,
                content: 1,
            },
            Step::Offline { node: 2 },
            Step::Modify {
                node: 0,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 1,
            },
            Step::Rename {
                node: 1,
                from: 1,
                to: 11,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 2,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(1), None, Some(3), Some(1)],
            },
            Step::Delete { node: 5, path: 5 },
            Step::MassModify {
                node: 7,
                fraction: 63,
                content: 3,
            },
            Step::Rename {
                node: 7,
                from: 4,
                to: 9,
            },
            Step::Delete { node: 3, path: 10 },
            Step::Partition { a: 3, b: 1 },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Online { node: 4 },
            Step::Online { node: 5 },
            Step::Modify {
                node: 6,
                path: 8,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 2,
            },
            Step::Delete { node: 5, path: 9 },
            Step::Crash {
                node: 5,
                gap_secs: 58,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Modify {
                node: 4,
                path: 2,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 4,
            },
            Step::Offline { node: 4 },
            Step::Modify {
                node: 5,
                path: 3,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 5,
            },
            Step::Touch { node: 5, path: 11 },
            Step::Modify {
                node: 1,
                path: 6,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Touch { node: 1, path: 1 },
            Step::Tier {
                a: 7,
                b: 1,
                tier: 2,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::Crash {
                node: 5,
                gap_secs: 100,
            },
            Step::Heal { a: 3, b: 2 },
            Step::Rename {
                node: 1,
                from: 7,
                to: 2,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Symlink {
                node: 7,
                path: 4,
                target: 0,
            },
            Step::Chmod { node: 6, path: 7 },
            Step::Online { node: 2 },
        ],
    );
}

/// Seed 97955, fetch corruption on, 254 steps: the only source of a want
/// served one corrupted transfer, then stayed connected and never announced
/// the path again, and nothing changed at the path, so none of draft 31's
/// events released it. Only the expiry does (§7.5, draft 32); this fails
/// with the expiry disabled.
#[test]
fn an_exclusion_expires_for_a_file_in_a_directory() {
    passes_with(
        97955,
        &corrupting(),
        &[
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), Some(5)],
            },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(4)],
            },
            Step::Partition { a: 6, b: 6 },
            Step::MassModify {
                node: 2,
                fraction: 82,
                content: 1,
            },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Rmdir { node: 4, dir: 1 },
            Step::Crash {
                node: 0,
                gap_secs: 112,
            },
            Step::Delete { node: 6, path: 10 },
            Step::Delete { node: 1, path: 3 },
            Step::Settle { secs: 4 },
            Step::Create {
                node: 6,
                path: 9,
                content: 5,
            },
            Step::Crash {
                node: 2,
                gap_secs: 76,
            },
            Step::Heal { a: 5, b: 6 },
            Step::MassModify {
                node: 1,
                fraction: 51,
                content: 4,
            },
            Step::Rename {
                node: 7,
                from: 4,
                to: 11,
            },
            Step::Create {
                node: 7,
                path: 2,
                content: 2,
            },
            Step::Create {
                node: 6,
                path: 7,
                content: 4,
            },
            Step::Delete { node: 2, path: 3 },
            Step::Create {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 8,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 4,
            },
            Step::Partition { a: 1, b: 1 },
            Step::Touch { node: 6, path: 1 },
            Step::Settle { secs: 1 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Tier {
                a: 6,
                b: 3,
                tier: 2,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 2,
            },
            Step::Settle { secs: 18 },
            Step::Settle { secs: 28 },
            Step::Modify {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Online { node: 2 },
            Step::Delete { node: 1, path: 6 },
            Step::Create {
                node: 2,
                path: 0,
                content: 2,
            },
            Step::Rmdir { node: 2, dir: 1 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 31,
            },
            Step::Delete { node: 2, path: 10 },
            Step::Chmod { node: 0, path: 9 },
            Step::Heal { a: 6, b: 2 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 2,
            },
            Step::Settle { secs: 36 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 4,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 31,
            },
            Step::Heal { a: 2, b: 6 },
            Step::Modify {
                node: 3,
                path: 6,
                content: 4,
            },
            Step::Online { node: 4 },
            Step::Delete { node: 2, path: 5 },
            Step::Heal { a: 0, b: 3 },
            Step::MassDelete {
                node: 3,
                fraction: 65,
            },
            Step::Online { node: 1 },
            Step::Modify {
                node: 5,
                path: 7,
                content: 5,
            },
            Step::Rename {
                node: 3,
                from: 9,
                to: 4,
            },
            Step::Settle { secs: 18 },
            Step::Modify {
                node: 5,
                path: 8,
                content: 5,
            },
            Step::Mkdir { node: 6, dir: 0 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 36,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Rename {
                node: 1,
                from: 3,
                to: 11,
            },
            Step::Settle { secs: 2 },
            Step::Create {
                node: 4,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 5,
            },
            Step::Crash {
                node: 2,
                gap_secs: 39,
            },
            Step::Settle { secs: 7 },
            Step::Modify {
                node: 3,
                path: 1,
                content: 1,
            },
            Step::Tier {
                a: 6,
                b: 7,
                tier: 2,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::DenyAll,
                delay_secs: 29,
            },
            Step::Touch { node: 7, path: 2 },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(3), Some(3), None, None, Some(5), None, Some(4)],
            },
            Step::Chmod { node: 3, path: 3 },
            Step::Heal { a: 6, b: 1 },
            Step::Delete { node: 4, path: 1 },
            Step::MassModify {
                node: 4,
                fraction: 66,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 2,
            },
            Step::MassModify {
                node: 2,
                fraction: 64,
                content: 5,
            },
            Step::Delete { node: 5, path: 2 },
            Step::Settle { secs: 21 },
            Step::Tier {
                a: 3,
                b: 5,
                tier: 2,
            },
            Step::Crash {
                node: 2,
                gap_secs: 69,
            },
            Step::Partition { a: 1, b: 1 },
            Step::Modify {
                node: 1,
                path: 4,
                content: 4,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 29,
            },
            Step::Partition { a: 2, b: 7 },
            Step::Delete { node: 1, path: 9 },
            Step::Modify {
                node: 0,
                path: 6,
                content: 1,
            },
            Step::Tier {
                a: 4,
                b: 7,
                tier: 0,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Modify {
                node: 4,
                path: 6,
                content: 1,
            },
            Step::Offline { node: 6 },
            Step::Delete { node: 6, path: 11 },
            Step::Create {
                node: 2,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 10,
                content: 3,
            },
            Step::Online { node: 3 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(3), Some(3), Some(5)],
            },
            Step::Modify {
                node: 7,
                path: 5,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 64,
            },
            Step::Online { node: 7 },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Rename {
                node: 3,
                from: 9,
                to: 7,
            },
            Step::Rename {
                node: 6,
                from: 0,
                to: 1,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 28,
            },
            Step::Partition { a: 1, b: 1 },
            Step::Delete { node: 2, path: 1 },
            Step::Modify {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Settle { secs: 29 },
            Step::Create {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Touch { node: 2, path: 8 },
            Step::Tier {
                a: 4,
                b: 5,
                tier: 1,
            },
            Step::Create {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 2,
            },
            Step::Delete { node: 7, path: 7 },
            Step::Create {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Delete { node: 6, path: 9 },
            Step::Crash {
                node: 6,
                gap_secs: 53,
            },
            Step::Chmod { node: 0, path: 5 },
            Step::Settle { secs: 5 },
            Step::Create {
                node: 6,
                path: 7,
                content: 5,
            },
            Step::Heal { a: 5, b: 3 },
            Step::Crash {
                node: 5,
                gap_secs: 102,
            },
            Step::Modify {
                node: 3,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Crash {
                node: 5,
                gap_secs: 14,
            },
            Step::Modify {
                node: 4,
                path: 9,
                content: 3,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(3), None, Some(5), Some(5), Some(3)],
            },
            Step::Heal { a: 3, b: 2 },
            Step::Delete { node: 3, path: 0 },
            Step::Crash {
                node: 1,
                gap_secs: 36,
            },
            Step::Create {
                node: 6,
                path: 9,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Settle { secs: 32 },
            Step::Touch { node: 6, path: 6 },
            Step::Create {
                node: 0,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Heal { a: 7, b: 2 },
            Step::Online { node: 7 },
            Step::Rename {
                node: 7,
                from: 8,
                to: 11,
            },
            Step::Settle { secs: 30 },
            Step::Crash {
                node: 3,
                gap_secs: 40,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Modify {
                node: 6,
                path: 4,
                content: 3,
            },
            Step::Chmod { node: 4, path: 3 },
            Step::Delete { node: 0, path: 3 },
            Step::Tier {
                a: 3,
                b: 1,
                tier: 2,
            },
            Step::Delete { node: 3, path: 3 },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 17,
            },
            Step::Delete { node: 7, path: 4 },
            Step::Heal { a: 6, b: 6 },
            Step::Heal { a: 1, b: 0 },
            Step::Create {
                node: 4,
                path: 6,
                content: 5,
            },
            Step::Heal { a: 2, b: 6 },
            Step::Tier {
                a: 1,
                b: 6,
                tier: 2,
            },
            Step::Rename {
                node: 3,
                from: 11,
                to: 9,
            },
            Step::Crash {
                node: 2,
                gap_secs: 46,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Partition { a: 1, b: 3 },
            Step::Delete { node: 6, path: 6 },
            Step::Settle { secs: 37 },
            Step::Settle { secs: 38 },
            Step::Modify {
                node: 2,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 6,
                content: 5,
            },
            Step::Heal { a: 4, b: 2 },
            Step::Create {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 22,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![None, None, Some(5), Some(4), Some(3), None],
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(5), Some(3)],
            },
            Step::Heal { a: 6, b: 2 },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Touch { node: 7, path: 4 },
            Step::Create {
                node: 0,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 0,
                path: 10,
                content: 2,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 1,
            },
            Step::Tier {
                a: 2,
                b: 4,
                tier: 2,
            },
            Step::Settle { secs: 9 },
            Step::Settle { secs: 38 },
            Step::Create {
                node: 5,
                path: 6,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 35,
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Delete { node: 1, path: 4 },
            Step::Create {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Chmod { node: 4, path: 7 },
            Step::Touch { node: 2, path: 9 },
            Step::Crash {
                node: 0,
                gap_secs: 64,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Rename {
                node: 6,
                from: 0,
                to: 5,
            },
            Step::Online { node: 6 },
            Step::Modify {
                node: 7,
                path: 11,
                content: 2,
            },
            Step::Settle { secs: 18 },
            Step::Delete { node: 2, path: 7 },
            Step::Online { node: 6 },
            Step::Delete { node: 2, path: 8 },
            Step::Heal { a: 2, b: 5 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 3,
            },
            Step::Delete { node: 5, path: 5 },
            Step::Create {
                node: 1,
                path: 4,
                content: 5,
            },
            Step::Delete { node: 6, path: 11 },
            Step::Create {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Crash {
                node: 4,
                gap_secs: 49,
            },
            Step::Delete { node: 5, path: 6 },
            Step::Chmod { node: 4, path: 8 },
            Step::Rename {
                node: 0,
                from: 11,
                to: 6,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 3,
            },
            Step::Rename {
                node: 0,
                from: 4,
                to: 7,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(1), Some(5)],
            },
            Step::Touch { node: 0, path: 8 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 21,
            },
            Step::Create {
                node: 2,
                path: 10,
                content: 4,
            },
            Step::Crash {
                node: 0,
                gap_secs: 68,
            },
            Step::Modify {
                node: 1,
                path: 0,
                content: 5,
            },
            Step::Settle { secs: 10 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 4,
                },
                delay_secs: 28,
            },
            Step::Settle { secs: 4 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Chmod { node: 2, path: 9 },
            Step::Rename {
                node: 6,
                from: 8,
                to: 3,
            },
            Step::Create {
                node: 6,
                path: 6,
                content: 2,
            },
            Step::Delete { node: 5, path: 6 },
            Step::Online { node: 4 },
            Step::Create {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 5,
            },
            Step::Heal { a: 1, b: 3 },
            Step::Symlink {
                node: 2,
                path: 11,
                target: 10,
            },
            Step::Modify {
                node: 0,
                path: 11,
                content: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 24,
            },
            Step::Delete { node: 0, path: 7 },
            Step::Touch { node: 4, path: 1 },
            Step::Create {
                node: 5,
                path: 9,
                content: 1,
            },
            Step::Tier {
                a: 2,
                b: 5,
                tier: 1,
            },
            Step::Partition { a: 6, b: 2 },
            Step::Create {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Crash {
                node: 3,
                gap_secs: 97,
            },
            Step::Create {
                node: 6,
                path: 4,
                content: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 52,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 33,
            },
            Step::Online { node: 3 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 2,
            },
            Step::Settle { secs: 14 },
            Step::Create {
                node: 6,
                path: 5,
                content: 3,
            },
            Step::MassModify {
                node: 5,
                fraction: 54,
                content: 2,
            },
        ],
    );
}

/// Seed 98377, fetch corruption on, 285 steps: the only source of a want
/// served one corrupted transfer, then stayed connected and never announced
/// the path again, and nothing changed at the path, so none of draft 31's
/// events released it. Only the expiry does (§7.5, draft 32); this fails
/// with the expiry disabled.
#[test]
fn an_exclusion_expires_for_a_symlink_conflict_copy() {
    passes_with(
        98377,
        &corrupting(),
        &[
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(1), Some(3)],
            },
            Step::Settle { secs: 13 },
            Step::Create {
                node: 7,
                path: 11,
                content: 2,
            },
            Step::Settle { secs: 18 },
            Step::Modify {
                node: 6,
                path: 6,
                content: 4,
            },
            Step::Online { node: 7 },
            Step::Tier {
                a: 6,
                b: 0,
                tier: 0,
            },
            Step::Modify {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Rmdir { node: 3, dir: 0 },
            Step::Symlink {
                node: 3,
                path: 5,
                target: 0,
            },
            Step::Modify {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 11,
                content: 2,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 4,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 2,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 1,
            },
            Step::Partition { a: 3, b: 1 },
            Step::Chmod { node: 1, path: 6 },
            Step::Create {
                node: 2,
                path: 10,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 5,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(1), Some(4), None],
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 51,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 5,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 3,
            },
            Step::Partition { a: 1, b: 7 },
            Step::Modify {
                node: 4,
                path: 5,
                content: 2,
            },
            Step::Partition { a: 0, b: 4 },
            Step::Rename {
                node: 5,
                from: 8,
                to: 10,
            },
            Step::MassModify {
                node: 4,
                fraction: 51,
                content: 3,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 18,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 7,
                content: 3,
            },
            Step::Crash {
                node: 1,
                gap_secs: 54,
            },
            Step::Delete { node: 4, path: 8 },
            Step::MassDelete {
                node: 7,
                fraction: 73,
            },
            Step::Create {
                node: 2,
                path: 0,
                content: 3,
            },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Partition { a: 0, b: 2 },
            Step::Create {
                node: 7,
                path: 1,
                content: 5,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(5), Some(3), Some(5), None, Some(3), Some(2), Some(5)],
            },
            Step::Delete { node: 7, path: 4 },
            Step::Create {
                node: 2,
                path: 4,
                content: 3,
            },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Online { node: 1 },
            Step::Create {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Settle { secs: 1 },
            Step::Delete { node: 4, path: 3 },
            Step::Create {
                node: 7,
                path: 2,
                content: 1,
            },
            Step::Crash {
                node: 2,
                gap_secs: 112,
            },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 1,
            },
            Step::Online { node: 6 },
            Step::Delete { node: 7, path: 9 },
            Step::Touch { node: 3, path: 2 },
            Step::Offline { node: 5 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 56,
            },
            Step::Touch { node: 3, path: 3 },
            Step::Create {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Touch { node: 6, path: 10 },
            Step::Modify {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 4,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 54,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 4,
            },
            Step::Rmdir { node: 1, dir: 1 },
            Step::Delete { node: 4, path: 9 },
            Step::Modify {
                node: 2,
                path: 2,
                content: 4,
            },
            Step::Online { node: 4 },
            Step::Create {
                node: 7,
                path: 10,
                content: 1,
            },
            Step::Touch { node: 3, path: 10 },
            Step::MassModify {
                node: 3,
                fraction: 76,
                content: 1,
            },
            Step::Online { node: 7 },
            Step::Mkdir { node: 4, dir: 0 },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 11,
            },
            Step::Heal { a: 1, b: 6 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 5,
            },
            Step::Online { node: 6 },
            Step::Chmod { node: 0, path: 11 },
            Step::Rename {
                node: 6,
                from: 9,
                to: 3,
            },
            Step::Delete { node: 5, path: 6 },
            Step::Rename {
                node: 1,
                from: 6,
                to: 2,
            },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 2,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(1), Some(1), Some(1), Some(3), Some(3), Some(2)],
            },
            Step::Modify {
                node: 2,
                path: 8,
                content: 1,
            },
            Step::Tier {
                a: 5,
                b: 6,
                tier: 1,
            },
            Step::Chmod { node: 6, path: 3 },
            Step::Touch { node: 7, path: 7 },
            Step::Partition { a: 3, b: 0 },
            Step::Delete { node: 1, path: 2 },
            Step::Modify {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 4,
                path: 0,
                content: 1,
            },
            Step::Crash {
                node: 3,
                gap_secs: 114,
            },
            Step::Settle { secs: 9 },
            Step::Modify {
                node: 7,
                path: 5,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 3,
                content: 5,
            },
            Step::Offline { node: 3 },
            Step::Delete { node: 6, path: 3 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(1), Some(3), Some(4)],
            },
            Step::Modify {
                node: 5,
                path: 0,
                content: 2,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![None, None, None, Some(2)],
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 13,
            },
            Step::MassDelete {
                node: 4,
                fraction: 56,
            },
            Step::Heal { a: 0, b: 5 },
            Step::Symlink {
                node: 0,
                path: 7,
                target: 10,
            },
            Step::MassModify {
                node: 2,
                fraction: 97,
                content: 1,
            },
            Step::Rename {
                node: 3,
                from: 9,
                to: 3,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(1), Some(3), Some(3), Some(1), None, None, Some(3)],
            },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Heal { a: 2, b: 6 },
            Step::Create {
                node: 0,
                path: 11,
                content: 2,
            },
            Step::Settle { secs: 35 },
            Step::Settle { secs: 32 },
            Step::Modify {
                node: 5,
                path: 4,
                content: 4,
            },
            Step::Heal { a: 1, b: 7 },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Create {
                node: 6,
                path: 8,
                content: 5,
            },
            Step::Heal { a: 4, b: 4 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Settle { secs: 2 },
            Step::Create {
                node: 7,
                path: 10,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 4,
            },
            Step::Delete { node: 0, path: 11 },
            Step::Online { node: 2 },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Delete { node: 1, path: 10 },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 27,
            },
            Step::Online { node: 2 },
            Step::Delete { node: 7, path: 6 },
            Step::Create {
                node: 7,
                path: 9,
                content: 1,
            },
            Step::Heal { a: 1, b: 0 },
            Step::Partition { a: 0, b: 1 },
            Step::Everywhere {
                path: 4,
                contents: vec![None, Some(2)],
            },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(4), Some(3), Some(4), None, Some(5), None],
            },
            Step::Partition { a: 2, b: 7 },
            Step::Create {
                node: 3,
                path: 6,
                content: 2,
            },
            Step::Rmdir { node: 0, dir: 1 },
            Step::Crash {
                node: 6,
                gap_secs: 20,
            },
            Step::Settle { secs: 14 },
            Step::Modify {
                node: 2,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Heal { a: 7, b: 6 },
            Step::MassDelete {
                node: 0,
                fraction: 78,
            },
            Step::Heal { a: 2, b: 4 },
            Step::Chmod { node: 3, path: 9 },
            Step::Create {
                node: 0,
                path: 1,
                content: 5,
            },
            Step::Offline { node: 5 },
            Step::Modify {
                node: 1,
                path: 3,
                content: 2,
            },
            Step::Heal { a: 6, b: 6 },
            Step::Online { node: 4 },
            Step::Create {
                node: 7,
                path: 0,
                content: 2,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Create {
                node: 1,
                path: 10,
                content: 5,
            },
            Step::Heal { a: 1, b: 6 },
            Step::Symlink {
                node: 4,
                path: 7,
                target: 8,
            },
            Step::Offline { node: 2 },
            Step::Create {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 21,
                },
                delay_secs: 58,
            },
            Step::Touch { node: 4, path: 9 },
            Step::Symlink {
                node: 5,
                path: 2,
                target: 10,
            },
            Step::Delete { node: 1, path: 5 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 47,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 8,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 58,
            },
            Step::Offline { node: 3 },
            Step::Create {
                node: 6,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 1,
            },
            Step::Rename {
                node: 4,
                from: 0,
                to: 1,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 1,
            },
            Step::Delete { node: 7, path: 1 },
            Step::Delete { node: 4, path: 11 },
            Step::Create {
                node: 6,
                path: 8,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 8,
                content: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(5)],
            },
            Step::Partition { a: 5, b: 0 },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 4,
                },
                delay_secs: 41,
            },
            Step::Partition { a: 3, b: 2 },
            Step::Modify {
                node: 0,
                path: 6,
                content: 4,
            },
            Step::Touch { node: 1, path: 2 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 2,
            },
            Step::Rename {
                node: 4,
                from: 6,
                to: 6,
            },
            Step::Touch { node: 0, path: 4 },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), None, Some(3), Some(3), Some(1), Some(3)],
            },
            Step::Mkdir { node: 7, dir: 0 },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 30,
                },
                delay_secs: 57,
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 2,
            },
            Step::Create {
                node: 7,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 5,
                path: 8,
                content: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(1), Some(3), Some(3), None, Some(4), Some(5)],
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 25,
            },
            Step::Chmod { node: 6, path: 2 },
            Step::Chmod { node: 2, path: 10 },
            Step::Symlink {
                node: 6,
                path: 4,
                target: 3,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 1,
            },
            Step::Settle { secs: 10 },
            Step::Heal { a: 6, b: 1 },
            Step::Crash {
                node: 0,
                gap_secs: 44,
            },
            Step::Rename {
                node: 6,
                from: 9,
                to: 10,
            },
            Step::Modify {
                node: 6,
                path: 8,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 7,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 1,
            },
            Step::Rmdir { node: 5, dir: 2 },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 52,
            },
            Step::Delete { node: 3, path: 7 },
            Step::Touch { node: 0, path: 5 },
            Step::Heal { a: 7, b: 4 },
            Step::Create {
                node: 1,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 54,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 1,
            },
            Step::Mkdir { node: 3, dir: 2 },
            Step::MassModify {
                node: 5,
                fraction: 92,
                content: 3,
            },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Tier {
                a: 1,
                b: 7,
                tier: 1,
            },
            Step::Symlink {
                node: 1,
                path: 10,
                target: 6,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 2,
            },
            Step::Crash {
                node: 0,
                gap_secs: 6,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 2,
            },
            Step::Delete { node: 7, path: 0 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 3,
            },
            Step::Everywhere {
                path: 1,
                contents: vec![Some(3), Some(5), Some(3), Some(1), Some(1)],
            },
            Step::Delete { node: 7, path: 6 },
            Step::Create {
                node: 4,
                path: 0,
                content: 3,
            },
            Step::Settle { secs: 27 },
            Step::Modify {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Delete { node: 0, path: 6 },
            Step::Delete { node: 0, path: 11 },
            Step::Settle { secs: 8 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 19,
            },
            Step::Modify {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Chmod { node: 3, path: 1 },
            Step::Offline { node: 6 },
            Step::Mkdir { node: 7, dir: 0 },
            Step::MassModify {
                node: 1,
                fraction: 77,
                content: 2,
            },
            Step::Delete { node: 2, path: 6 },
            Step::Tier {
                a: 5,
                b: 4,
                tier: 1,
            },
            Step::Chmod { node: 1, path: 4 },
            Step::Create {
                node: 1,
                path: 10,
                content: 3,
            },
            Step::Rename {
                node: 5,
                from: 6,
                to: 4,
            },
            Step::Touch { node: 6, path: 10 },
            Step::Delete { node: 2, path: 3 },
            Step::Delete { node: 3, path: 3 },
            Step::Modify {
                node: 6,
                path: 1,
                content: 3,
            },
            Step::Rename {
                node: 1,
                from: 1,
                to: 10,
            },
            Step::Tier {
                a: 7,
                b: 2,
                tier: 1,
            },
            Step::Heal { a: 6, b: 7 },
            Step::Settle { secs: 30 },
            Step::Create {
                node: 3,
                path: 11,
                content: 5,
            },
            Step::Chmod { node: 5, path: 6 },
            Step::Heal { a: 2, b: 2 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Create {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Online { node: 1 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 0,
            },
            Step::Rename {
                node: 6,
                from: 3,
                to: 9,
            },
            Step::Touch { node: 6, path: 5 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Delete { node: 3, path: 7 },
            Step::Settle { secs: 14 },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Create {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::MassModify {
                node: 5,
                fraction: 57,
                content: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(1), Some(5), None, Some(5)],
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 44,
            },
            Step::Create {
                node: 2,
                path: 11,
                content: 1,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 34,
                },
                delay_secs: 3,
            },
            Step::Create {
                node: 5,
                path: 11,
                content: 4,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 53,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Online { node: 7 },
            Step::Delete { node: 2, path: 1 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 17,
            },
            Step::Modify {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Create {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 0,
                path: 0,
                content: 4,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 14,
                },
                delay_secs: 5,
            },
            Step::Partition { a: 4, b: 6 },
            Step::Partition { a: 5, b: 1 },
            Step::Delete { node: 1, path: 4 },
            Step::Online { node: 6 },
        ],
    );
}

/// Seed 99246, fetch corruption on, 37 steps: the only source of a want
/// served one corrupted transfer, then stayed connected and never announced
/// the path again, and nothing changed at the path, so none of draft 31's
/// events released it. Only the expiry does (§7.5, draft 32); this fails
/// with the expiry disabled.
#[test]
fn an_exclusion_expires_for_another_file_in_a_directory() {
    passes_with(
        99246,
        &corrupting(),
        &[
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 12,
            },
            Step::Chmod { node: 3, path: 9 },
            Step::Touch { node: 5, path: 1 },
            Step::Heal { a: 0, b: 0 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 56,
                },
                delay_secs: 9,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Delete { node: 7, path: 5 },
            Step::Delete { node: 1, path: 10 },
            Step::Online { node: 1 },
            Step::Heal { a: 1, b: 5 },
            Step::Rename {
                node: 4,
                from: 8,
                to: 2,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(5), Some(4), None, Some(5), Some(1), Some(4), None],
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 40,
            },
            Step::Mkdir { node: 7, dir: 0 },
            Step::Delete { node: 0, path: 4 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Heal { a: 4, b: 0 },
            Step::User {
                node: 5,
                action: crate::UserAction::ApproveAll,
                delay_secs: 23,
            },
            Step::Online { node: 2 },
            Step::Create {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Delete { node: 1, path: 8 },
            Step::Settle { secs: 7 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 1,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 11,
            },
            Step::Delete { node: 2, path: 2 },
            Step::Partition { a: 4, b: 6 },
            Step::Online { node: 1 },
            Step::Partition { a: 7, b: 7 },
            Step::Create {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::Touch { node: 5, path: 2 },
            Step::Heal { a: 5, b: 7 },
            Step::Modify {
                node: 4,
                path: 5,
                content: 2,
            },
            Step::Partition { a: 3, b: 5 },
            Step::Heal { a: 4, b: 5 },
            Step::Create {
                node: 1,
                path: 6,
                content: 1,
            },
        ],
    );
}

/// Seed 24896 of the first 100,000-seed run, default knobs, all 400
/// steps: during the final drain one node crashed thirteen times, each
/// time within the batch window's quiet period, and wrote records 246 to
/// 406 in between. The window died with each crash, and after the last
/// restart the node wrote nothing more, so those records, a conflict copy
/// among them, were never announced (§11). No shorter list keeps the
/// failure: removing any step changes the crash sequence it depends on.
#[test]
fn a_restart_announces_what_a_crash_left_unannounced() {
    passes(
        24896,
        &[
            Step::Everywhere {
                path: 8,
                contents: vec![Some(5), Some(4), Some(5)],
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 42,
            },
            Step::Symlink {
                node: 7,
                path: 9,
                target: 1,
            },
            Step::Delete { node: 4, path: 0 },
            Step::Chmod { node: 0, path: 9 },
            Step::Delete { node: 6, path: 5 },
            Step::Delete { node: 6, path: 4 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Heal { a: 6, b: 2 },
            Step::Modify {
                node: 5,
                path: 1,
                content: 3,
            },
            Step::Heal { a: 2, b: 7 },
            Step::Delete { node: 5, path: 7 },
            Step::Modify {
                node: 4,
                path: 9,
                content: 2,
            },
            Step::Create {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Offline { node: 2 },
            Step::Touch { node: 4, path: 1 },
            Step::Modify {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 25,
            },
            Step::Delete { node: 0, path: 2 },
            Step::Modify {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 2,
                content: 3,
            },
            Step::Touch { node: 2, path: 5 },
            Step::Heal { a: 1, b: 3 },
            Step::Delete { node: 5, path: 0 },
            Step::Delete { node: 2, path: 4 },
            Step::Create {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 5,
            },
            Step::Partition { a: 1, b: 7 },
            Step::Partition { a: 0, b: 3 },
            Step::Modify {
                node: 6,
                path: 4,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 3,
                content: 1,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(5), Some(4), None],
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(4), Some(5)],
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Revert,
                delay_secs: 18,
            },
            Step::Delete { node: 7, path: 4 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(3), Some(5), Some(5)],
            },
            Step::Chmod { node: 3, path: 7 },
            Step::Online { node: 6 },
            Step::Rename {
                node: 3,
                from: 6,
                to: 11,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Partition { a: 7, b: 4 },
            Step::Partition { a: 4, b: 5 },
            Step::Heal { a: 7, b: 1 },
            Step::Modify {
                node: 2,
                path: 1,
                content: 4,
            },
            Step::Settle { secs: 9 },
            Step::Offline { node: 1 },
            Step::Symlink {
                node: 6,
                path: 6,
                target: 1,
            },
            Step::Settle { secs: 2 },
            Step::Online { node: 4 },
            Step::Crash {
                node: 6,
                gap_secs: 26,
            },
            Step::Crash {
                node: 2,
                gap_secs: 44,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 3,
            },
            Step::Delete { node: 1, path: 9 },
            Step::Modify {
                node: 0,
                path: 2,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 2,
            },
            Step::Chmod { node: 4, path: 4 },
            Step::Modify {
                node: 6,
                path: 2,
                content: 4,
            },
            Step::Settle { secs: 17 },
            Step::Create {
                node: 4,
                path: 10,
                content: 4,
            },
            Step::Settle { secs: 37 },
            Step::Modify {
                node: 0,
                path: 9,
                content: 4,
            },
            Step::Settle { secs: 8 },
            Step::Touch { node: 6, path: 1 },
            Step::Delete { node: 6, path: 7 },
            Step::Touch { node: 7, path: 6 },
            Step::Online { node: 1 },
            Step::Create {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::Online { node: 5 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), Some(3), Some(1)],
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Tier {
                a: 3,
                b: 2,
                tier: 0,
            },
            Step::Offline { node: 7 },
            Step::Rename {
                node: 3,
                from: 8,
                to: 2,
            },
            Step::Partition { a: 6, b: 6 },
            Step::MassDelete {
                node: 6,
                fraction: 51,
            },
            Step::Rename {
                node: 1,
                from: 6,
                to: 2,
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 5,
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::Settle { secs: 31 },
            Step::Partition { a: 6, b: 1 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(3), Some(5), Some(3), None, None, None, Some(3)],
            },
            Step::Online { node: 1 },
            Step::Touch { node: 7, path: 1 },
            Step::Settle { secs: 39 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Heal { a: 7, b: 3 },
            Step::Settle { secs: 31 },
            Step::Delete { node: 2, path: 5 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 4,
            },
            Step::Settle { secs: 21 },
            Step::Create {
                node: 7,
                path: 2,
                content: 4,
            },
            Step::Partition { a: 6, b: 0 },
            Step::Rename {
                node: 5,
                from: 5,
                to: 1,
            },
            Step::Heal { a: 4, b: 4 },
            Step::Partition { a: 4, b: 4 },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Settle { secs: 31 },
            Step::Create {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Partition { a: 6, b: 1 },
            Step::Rmdir { node: 7, dir: 0 },
            Step::Settle { secs: 28 },
            Step::Modify {
                node: 3,
                path: 7,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Delete { node: 0, path: 4 },
            Step::Settle { secs: 31 },
            Step::Settle { secs: 13 },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Settle { secs: 2 },
            Step::Offline { node: 6 },
            Step::Mkdir { node: 5, dir: 2 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Partition { a: 7, b: 2 },
            Step::Create {
                node: 3,
                path: 0,
                content: 4,
            },
            Step::Delete { node: 2, path: 4 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 2,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(3), Some(3)],
            },
            Step::Settle { secs: 32 },
            Step::Touch { node: 0, path: 10 },
            Step::Touch { node: 6, path: 4 },
            Step::Crash {
                node: 6,
                gap_secs: 16,
            },
            Step::Partition { a: 5, b: 1 },
            Step::Create {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::MassDelete {
                node: 3,
                fraction: 51,
            },
            Step::Create {
                node: 1,
                path: 3,
                content: 1,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Chmod { node: 4, path: 5 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(1), Some(3), Some(2)],
            },
            Step::Rename {
                node: 5,
                from: 11,
                to: 11,
            },
            Step::Rename {
                node: 6,
                from: 5,
                to: 5,
            },
            Step::Online { node: 6 },
            Step::Modify {
                node: 3,
                path: 5,
                content: 5,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(4), Some(4), Some(2), None],
            },
            Step::Delete { node: 0, path: 6 },
            Step::Create {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 2,
            },
            Step::Create {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 4,
            },
            Step::Partition { a: 6, b: 1 },
            Step::Modify {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 1,
            },
            Step::Settle { secs: 26 },
            Step::Tier {
                a: 3,
                b: 3,
                tier: 0,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(3)],
            },
            Step::Partition { a: 6, b: 0 },
            Step::Settle { secs: 19 },
            Step::Online { node: 2 },
            Step::Everywhere {
                path: 5,
                contents: vec![None, Some(5), Some(2), Some(1), Some(2), Some(5)],
            },
            Step::Offline { node: 5 },
            Step::Partition { a: 0, b: 1 },
            Step::Crash {
                node: 5,
                gap_secs: 58,
            },
            Step::Tier {
                a: 5,
                b: 4,
                tier: 1,
            },
            Step::Crash {
                node: 7,
                gap_secs: 74,
            },
            Step::Modify {
                node: 3,
                path: 6,
                content: 2,
            },
            Step::Crash {
                node: 2,
                gap_secs: 42,
            },
            Step::Crash {
                node: 0,
                gap_secs: 34,
            },
            Step::Online { node: 2 },
            Step::Settle { secs: 7 },
            Step::MassModify {
                node: 0,
                fraction: 77,
                content: 4,
            },
            Step::Create {
                node: 6,
                path: 1,
                content: 1,
            },
            Step::Rmdir { node: 4, dir: 2 },
            Step::Partition { a: 2, b: 0 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 26,
            },
            Step::MassDelete {
                node: 1,
                fraction: 70,
            },
            Step::Crash {
                node: 5,
                gap_secs: 110,
            },
            Step::Delete { node: 3, path: 4 },
            Step::Heal { a: 1, b: 6 },
            Step::Chmod { node: 0, path: 4 },
            Step::Online { node: 7 },
            Step::Modify {
                node: 4,
                path: 9,
                content: 5,
            },
            Step::Mkdir { node: 6, dir: 2 },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 12,
            },
            Step::Rename {
                node: 0,
                from: 10,
                to: 10,
            },
            Step::Modify {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Modify {
                node: 3,
                path: 11,
                content: 2,
            },
            Step::Touch { node: 1, path: 7 },
            Step::MassDelete {
                node: 0,
                fraction: 96,
            },
            Step::Online { node: 0 },
            Step::Delete { node: 6, path: 3 },
            Step::Create {
                node: 2,
                path: 11,
                content: 4,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(3), Some(2), Some(5), Some(1), Some(4), None],
            },
            Step::MassDelete {
                node: 0,
                fraction: 60,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::MassDelete {
                node: 7,
                fraction: 95,
            },
            Step::Delete { node: 0, path: 2 },
            Step::Online { node: 0 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 5,
            },
            Step::Delete { node: 5, path: 2 },
            Step::Delete { node: 2, path: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 10,
            },
            Step::Rename {
                node: 4,
                from: 7,
                to: 7,
            },
            Step::Chmod { node: 0, path: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::DenyAll,
                delay_secs: 48,
            },
            Step::Symlink {
                node: 2,
                path: 9,
                target: 1,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 1,
            },
            Step::Create {
                node: 2,
                path: 0,
                content: 1,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 43,
            },
            Step::Modify {
                node: 7,
                path: 7,
                content: 1,
            },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(3), Some(4), None, None, Some(5)],
            },
            Step::Delete { node: 0, path: 7 },
            Step::Create {
                node: 2,
                path: 1,
                content: 4,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 4,
            },
            Step::Rename {
                node: 3,
                from: 8,
                to: 8,
            },
            Step::Chmod { node: 0, path: 6 },
            Step::Everywhere {
                path: 3,
                contents: vec![None, None, Some(2), Some(2), Some(1), Some(5), None],
            },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 1,
            },
            Step::Modify {
                node: 5,
                path: 10,
                content: 1,
            },
            Step::MassDelete {
                node: 1,
                fraction: 56,
            },
            Step::Touch { node: 0, path: 11 },
            Step::Delete { node: 7, path: 2 },
            Step::Delete { node: 5, path: 7 },
            Step::Create {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Offline { node: 3 },
            Step::Delete { node: 4, path: 2 },
            Step::Delete { node: 3, path: 6 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Chmod { node: 5, path: 0 },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Touch { node: 0, path: 11 },
            Step::MassDelete {
                node: 6,
                fraction: 86,
            },
            Step::Partition { a: 7, b: 4 },
            Step::Modify {
                node: 0,
                path: 5,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 3,
                content: 2,
            },
            Step::Online { node: 1 },
            Step::Online { node: 4 },
            Step::Modify {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Settle { secs: 22 },
            Step::Rmdir { node: 5, dir: 0 },
            Step::Symlink {
                node: 0,
                path: 6,
                target: 4,
            },
            Step::Partition { a: 1, b: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 26,
            },
            Step::Chmod { node: 4, path: 9 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(2), Some(3), Some(2)],
            },
            Step::Delete { node: 1, path: 9 },
            Step::MassModify {
                node: 2,
                fraction: 60,
                content: 3,
            },
            Step::Settle { secs: 28 },
            Step::Create {
                node: 3,
                path: 2,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 7,
                content: 4,
            },
            Step::Mkdir { node: 7, dir: 2 },
            Step::Delete { node: 4, path: 4 },
            Step::Modify {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Online { node: 0 },
            Step::Heal { a: 5, b: 5 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 2,
            },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(2), None, None, Some(3), None, Some(5)],
            },
            Step::Tier {
                a: 4,
                b: 3,
                tier: 2,
            },
            Step::Crash {
                node: 0,
                gap_secs: 17,
            },
            Step::Settle { secs: 36 },
            Step::Delete { node: 3, path: 0 },
            Step::Modify {
                node: 4,
                path: 11,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 24,
            },
            Step::Heal { a: 1, b: 2 },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Chmod { node: 5, path: 1 },
            Step::Modify {
                node: 5,
                path: 6,
                content: 2,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 51,
            },
            Step::Chmod { node: 5, path: 10 },
            Step::Chmod { node: 1, path: 0 },
            Step::Partition { a: 6, b: 4 },
            Step::Settle { secs: 5 },
            Step::Rmdir { node: 0, dir: 2 },
            Step::Create {
                node: 1,
                path: 9,
                content: 2,
            },
            Step::Settle { secs: 16 },
            Step::Touch { node: 0, path: 6 },
            Step::Create {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 23 },
            Step::Heal { a: 0, b: 6 },
            Step::Create {
                node: 4,
                path: 7,
                content: 2,
            },
            Step::Heal { a: 6, b: 4 },
            Step::Create {
                node: 6,
                path: 8,
                content: 4,
            },
            Step::Tier {
                a: 3,
                b: 2,
                tier: 0,
            },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 3,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 51,
            },
            Step::Delete { node: 4, path: 10 },
            Step::Modify {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Rules {
                    hold_count: 0,
                    hold_pct: 54,
                },
                delay_secs: 28,
            },
            Step::Crash {
                node: 7,
                gap_secs: 92,
            },
            Step::Modify {
                node: 7,
                path: 7,
                content: 2,
            },
            Step::Online { node: 3 },
            Step::Heal { a: 1, b: 3 },
            Step::Modify {
                node: 5,
                path: 10,
                content: 1,
            },
            Step::Partition { a: 5, b: 1 },
            Step::Chmod { node: 4, path: 1 },
            Step::MassDelete {
                node: 1,
                fraction: 66,
            },
            Step::Offline { node: 7 },
            Step::Create {
                node: 4,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 1,
            },
            Step::Heal { a: 2, b: 7 },
            Step::Offline { node: 2 },
            Step::Settle { secs: 31 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 3,
            },
            Step::Settle { secs: 27 },
            Step::Partition { a: 4, b: 2 },
            Step::Create {
                node: 3,
                path: 1,
                content: 3,
            },
            Step::Rmdir { node: 3, dir: 0 },
            Step::Everywhere {
                path: 9,
                contents: vec![Some(5), Some(5)],
            },
            Step::Touch { node: 3, path: 2 },
            Step::Partition { a: 6, b: 1 },
            Step::Touch { node: 4, path: 6 },
            Step::Modify {
                node: 2,
                path: 10,
                content: 1,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::MassModify {
                node: 0,
                fraction: 97,
                content: 1,
            },
            Step::Delete { node: 1, path: 2 },
            Step::Create {
                node: 7,
                path: 2,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 9,
                content: 2,
            },
            Step::MassModify {
                node: 0,
                fraction: 92,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 9,
                content: 1,
            },
            Step::Mkdir { node: 1, dir: 0 },
            Step::Touch { node: 6, path: 5 },
            Step::Create {
                node: 7,
                path: 5,
                content: 1,
            },
            Step::Delete { node: 2, path: 11 },
            Step::Symlink {
                node: 5,
                path: 0,
                target: 11,
            },
            Step::Create {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![None, Some(5), Some(2), Some(3), Some(5), Some(5), Some(2)],
            },
            Step::Heal { a: 1, b: 5 },
            Step::Chmod { node: 7, path: 8 },
            Step::Partition { a: 2, b: 6 },
            Step::Create {
                node: 3,
                path: 0,
                content: 3,
            },
            Step::Crash {
                node: 5,
                gap_secs: 24,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Chmod { node: 6, path: 1 },
            Step::Partition { a: 1, b: 0 },
            Step::Crash {
                node: 1,
                gap_secs: 72,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 3,
            },
            Step::Modify {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 3,
                path: 8,
                content: 2,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![None, Some(2), Some(3), Some(5), Some(5), None],
            },
            Step::Settle { secs: 34 },
            Step::Create {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::Delete { node: 0, path: 9 },
            Step::Partition { a: 1, b: 1 },
            Step::Tier {
                a: 5,
                b: 2,
                tier: 0,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 5,
            },
            Step::Settle { secs: 35 },
            Step::Modify {
                node: 5,
                path: 7,
                content: 3,
            },
            Step::Symlink {
                node: 6,
                path: 1,
                target: 11,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 21,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 32,
                },
                delay_secs: 51,
            },
            Step::Create {
                node: 3,
                path: 11,
                content: 4,
            },
            Step::Offline { node: 5 },
            Step::Rmdir { node: 0, dir: 0 },
            Step::Symlink {
                node: 2,
                path: 8,
                target: 6,
            },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(3), Some(4), None, Some(1)],
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 1,
                content: 1,
            },
            Step::Touch { node: 7, path: 4 },
            Step::Chmod { node: 2, path: 11 },
            Step::Symlink {
                node: 1,
                path: 4,
                target: 11,
            },
            Step::Modify {
                node: 6,
                path: 1,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 7,
                content: 4,
            },
            Step::Chmod { node: 7, path: 0 },
            Step::Create {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Rename {
                node: 2,
                from: 7,
                to: 0,
            },
            Step::Create {
                node: 7,
                path: 9,
                content: 4,
            },
            Step::Delete { node: 7, path: 1 },
            Step::MassModify {
                node: 0,
                fraction: 94,
                content: 2,
            },
            Step::Modify {
                node: 6,
                path: 2,
                content: 2,
            },
            Step::Symlink {
                node: 3,
                path: 4,
                target: 7,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 1,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 5,
            },
            Step::Offline { node: 5 },
            Step::Settle { secs: 1 },
            Step::Create {
                node: 7,
                path: 1,
                content: 3,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![None, Some(4), Some(2), Some(2), None, Some(4)],
            },
            Step::Heal { a: 6, b: 6 },
            Step::Heal { a: 7, b: 1 },
            Step::Create {
                node: 0,
                path: 4,
                content: 4,
            },
            Step::MassDelete {
                node: 5,
                fraction: 91,
            },
            Step::Delete { node: 1, path: 10 },
            Step::Rename {
                node: 4,
                from: 4,
                to: 4,
            },
            Step::Modify {
                node: 4,
                path: 10,
                content: 2,
            },
            Step::Online { node: 3 },
            Step::Delete { node: 4, path: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 0,
            },
            Step::Touch { node: 0, path: 4 },
            Step::Tier {
                a: 4,
                b: 0,
                tier: 1,
            },
            Step::MassModify {
                node: 7,
                fraction: 89,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Tier {
                a: 2,
                b: 3,
                tier: 0,
            },
            Step::Partition { a: 1, b: 5 },
            Step::MassModify {
                node: 0,
                fraction: 98,
                content: 4,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 4,
            },
            Step::Offline { node: 0 },
            Step::Crash {
                node: 3,
                gap_secs: 65,
            },
            Step::Delete { node: 3, path: 10 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(1), Some(3), Some(2), Some(5), Some(2), Some(3), None],
            },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::Settle { secs: 26 },
            Step::Modify {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![None, Some(4), Some(3)],
            },
        ],
    );
}

/// Seed 66033 of the first 100,000-seed run, default knobs, shrunk to
/// 156 steps: revert kept a file whose pending change was only a touch
/// and ordered a reset to set its mtime back. While the reset was pending
/// the node answered NotAvailable for the file, its disk not yet matching
/// the record; the peer that asked then lost it as a source, and the landed
/// reset announced nothing, so the peer never asked again (§7.1, §8.3).
#[test]
fn a_peer_refused_during_a_reset_asks_again_when_it_lands() {
    passes(
        66033,
        &[
            Step::Delete { node: 4, path: 8 },
            Step::Heal { a: 2, b: 4 },
            Step::Rename {
                node: 2,
                from: 3,
                to: 4,
            },
            Step::Partition { a: 1, b: 4 },
            Step::Settle { secs: 16 },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Partition { a: 7, b: 5 },
            Step::Rmdir { node: 0, dir: 0 },
            Step::Offline { node: 2 },
            Step::Modify {
                node: 4,
                path: 10,
                content: 1,
            },
            Step::Tier {
                a: 7,
                b: 3,
                tier: 1,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 1,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::ApproveAll,
                delay_secs: 34,
            },
            Step::Delete { node: 5, path: 7 },
            Step::Offline { node: 4 },
            Step::Offline { node: 6 },
            Step::Delete { node: 7, path: 3 },
            Step::Create {
                node: 4,
                path: 10,
                content: 3,
            },
            Step::Delete { node: 2, path: 0 },
            Step::Delete { node: 4, path: 1 },
            Step::Modify {
                node: 2,
                path: 1,
                content: 3,
            },
            Step::Crash {
                node: 6,
                gap_secs: 116,
            },
            Step::Delete { node: 6, path: 6 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 6,
            },
            Step::Heal { a: 4, b: 6 },
            Step::Create {
                node: 5,
                path: 7,
                content: 5,
            },
            Step::Touch { node: 7, path: 8 },
            Step::Online { node: 1 },
            Step::Chmod { node: 5, path: 1 },
            Step::MassDelete {
                node: 4,
                fraction: 96,
            },
            Step::Partition { a: 5, b: 5 },
            Step::Heal { a: 3, b: 4 },
            Step::Chmod { node: 1, path: 1 },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 33,
            },
            Step::Online { node: 3 },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 12,
            },
            Step::Create {
                node: 5,
                path: 8,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 3,
            },
            Step::Modify {
                node: 3,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(3), None, Some(3), Some(5), None, Some(5)],
            },
            Step::Online { node: 7 },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 19,
            },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Create {
                node: 6,
                path: 3,
                content: 5,
            },
            Step::Modify {
                node: 7,
                path: 6,
                content: 5,
            },
            Step::Partition { a: 0, b: 0 },
            Step::Heal { a: 4, b: 1 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::MassDelete {
                node: 5,
                fraction: 59,
            },
            Step::Create {
                node: 7,
                path: 2,
                content: 3,
            },
            Step::Modify {
                node: 5,
                path: 7,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 3,
            },
            Step::Settle { secs: 28 },
            Step::Online { node: 4 },
            Step::Settle { secs: 35 },
            Step::Online { node: 7 },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 2,
            },
            Step::Chmod { node: 5, path: 3 },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 23,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 17,
            },
            Step::Create {
                node: 4,
                path: 10,
                content: 5,
            },
            Step::Create {
                node: 6,
                path: 9,
                content: 2,
            },
            Step::Heal { a: 2, b: 0 },
            Step::Modify {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Settle { secs: 19 },
            Step::Settle { secs: 25 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 4,
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::MassDelete {
                node: 0,
                fraction: 82,
            },
            Step::Delete { node: 6, path: 4 },
            Step::Modify {
                node: 1,
                path: 3,
                content: 1,
            },
            Step::Rmdir { node: 7, dir: 2 },
            Step::Touch { node: 7, path: 4 },
            Step::Rename {
                node: 6,
                from: 5,
                to: 3,
            },
            Step::Touch { node: 1, path: 8 },
            Step::MassDelete {
                node: 4,
                fraction: 63,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 43,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(3), None, Some(1), Some(2), Some(2), Some(4)],
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 4,
            },
            Step::Tier {
                a: 0,
                b: 5,
                tier: 2,
            },
            Step::Symlink {
                node: 2,
                path: 0,
                target: 0,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 4,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 37,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 15,
            },
            Step::Modify {
                node: 7,
                path: 8,
                content: 3,
            },
            Step::Settle { secs: 15 },
            Step::Create {
                node: 2,
                path: 8,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 2,
            },
            Step::Delete { node: 4, path: 5 },
            Step::Modify {
                node: 0,
                path: 0,
                content: 4,
            },
            Step::Chmod { node: 1, path: 11 },
            Step::MassDelete {
                node: 6,
                fraction: 73,
            },
            Step::Settle { secs: 9 },
            Step::Create {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Settle { secs: 19 },
            Step::MassModify {
                node: 6,
                fraction: 57,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 4,
                content: 2,
            },
            Step::Chmod { node: 3, path: 6 },
            Step::Settle { secs: 16 },
            Step::Online { node: 0 },
            Step::Offline { node: 6 },
            Step::Create {
                node: 5,
                path: 3,
                content: 4,
            },
            Step::MassDelete {
                node: 4,
                fraction: 65,
            },
            Step::Create {
                node: 7,
                path: 0,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 5,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 0,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 4,
            },
            Step::Settle { secs: 38 },
            Step::Tier {
                a: 5,
                b: 6,
                tier: 1,
            },
            Step::Online { node: 3 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 3,
            },
            Step::Create {
                node: 0,
                path: 2,
                content: 5,
            },
            Step::Create {
                node: 0,
                path: 1,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 6,
                content: 2,
            },
            Step::MassModify {
                node: 3,
                fraction: 63,
                content: 3,
            },
            Step::Delete { node: 0, path: 3 },
            Step::Tier {
                a: 1,
                b: 5,
                tier: 2,
            },
            Step::Delete { node: 3, path: 9 },
            Step::Offline { node: 1 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), Some(3), Some(5)],
            },
            Step::Modify {
                node: 5,
                path: 3,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 9,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 11,
                content: 3,
            },
            Step::Modify {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Heal { a: 7, b: 6 },
            Step::Heal { a: 5, b: 3 },
            Step::Chmod { node: 2, path: 0 },
            Step::Touch { node: 2, path: 9 },
            Step::Delete { node: 6, path: 5 },
            Step::Create {
                node: 3,
                path: 5,
                content: 3,
            },
            Step::Modify {
                node: 1,
                path: 11,
                content: 1,
            },
            Step::Delete { node: 7, path: 10 },
            Step::Create {
                node: 4,
                path: 2,
                content: 2,
            },
            Step::Partition { a: 2, b: 4 },
            Step::User {
                node: 4,
                action: crate::UserAction::ApproveAll,
                delay_secs: 33,
            },
            Step::Symlink {
                node: 0,
                path: 7,
                target: 1,
            },
            Step::Create {
                node: 4,
                path: 4,
                content: 1,
            },
            Step::Create {
                node: 3,
                path: 0,
                content: 2,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 2,
            },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 2,
            },
            Step::Delete { node: 5, path: 6 },
            Step::Everywhere {
                path: 10,
                contents: vec![None, Some(4), Some(4), Some(3), None, Some(5), Some(3)],
            },
            Step::Modify {
                node: 7,
                path: 9,
                content: 2,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Delete { node: 1, path: 8 },
            Step::Settle { secs: 35 },
            Step::Partition { a: 2, b: 3 },
            Step::Create {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 39,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 1,
            },
            Step::Partition { a: 1, b: 2 },
        ],
    );
}

/// Seed 74284, default knobs, shrunk to 104 steps. It passed on
/// the first run and failed once the restart window changed its course, the
/// same way as 66033: a peer asked for d1/f4 while a reset was pending, was
/// refused, and heard nothing when the reset landed (§7.1, §8.3).
#[test]
fn a_peer_refused_during_another_reset_asks_again() {
    passes(
        74284,
        &[
            Step::Touch { node: 5, path: 9 },
            Step::Create {
                node: 2,
                path: 7,
                content: 3,
            },
            Step::MassDelete {
                node: 0,
                fraction: 67,
            },
            Step::Create {
                node: 4,
                path: 3,
                content: 1,
            },
            Step::Touch { node: 1, path: 6 },
            Step::Crash {
                node: 1,
                gap_secs: 63,
            },
            Step::Settle { secs: 28 },
            Step::Touch { node: 1, path: 8 },
            Step::Rename {
                node: 6,
                from: 5,
                to: 3,
            },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Everywhere {
                path: 5,
                contents: vec![
                    Some(5),
                    Some(3),
                    Some(4),
                    Some(5),
                    Some(2),
                    Some(5),
                    Some(3),
                ],
            },
            Step::Create {
                node: 0,
                path: 3,
                content: 5,
            },
            Step::Create {
                node: 3,
                path: 9,
                content: 2,
            },
            Step::Crash {
                node: 6,
                gap_secs: 67,
            },
            Step::Offline { node: 3 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 4,
            },
            Step::Partition { a: 2, b: 0 },
            Step::Crash {
                node: 7,
                gap_secs: 41,
            },
            Step::Offline { node: 7 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Rename {
                node: 2,
                from: 6,
                to: 6,
            },
            Step::MassModify {
                node: 4,
                fraction: 96,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 3,
            },
            Step::Crash {
                node: 3,
                gap_secs: 108,
            },
            Step::Heal { a: 7, b: 1 },
            Step::Modify {
                node: 6,
                path: 10,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 6,
                content: 5,
            },
            Step::Chmod { node: 1, path: 4 },
            Step::Delete { node: 2, path: 5 },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 17,
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Settle { secs: 3 },
            Step::Create {
                node: 7,
                path: 7,
                content: 1,
            },
            Step::Delete { node: 2, path: 1 },
            Step::Modify {
                node: 2,
                path: 5,
                content: 2,
            },
            Step::Partition { a: 2, b: 0 },
            Step::Heal { a: 5, b: 0 },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Rename {
                node: 4,
                from: 3,
                to: 6,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 3,
            },
            Step::Chmod { node: 3, path: 5 },
            Step::Rename {
                node: 2,
                from: 6,
                to: 3,
            },
            Step::Online { node: 6 },
            Step::Delete { node: 0, path: 2 },
            Step::Modify {
                node: 7,
                path: 11,
                content: 2,
            },
            Step::Heal { a: 3, b: 6 },
            Step::Touch { node: 5, path: 2 },
            Step::Everywhere {
                path: 4,
                contents: vec![Some(2), None],
            },
            Step::Modify {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Settle { secs: 10 },
            Step::Chmod { node: 2, path: 7 },
            Step::Modify {
                node: 4,
                path: 0,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 2,
                content: 3,
            },
            Step::Touch { node: 2, path: 6 },
            Step::Heal { a: 4, b: 2 },
            Step::Delete { node: 7, path: 9 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 7,
            },
            Step::Crash {
                node: 2,
                gap_secs: 16,
            },
            Step::Online { node: 0 },
            Step::MassDelete {
                node: 2,
                fraction: 82,
            },
            Step::Rename {
                node: 2,
                from: 8,
                to: 9,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Touch { node: 4, path: 10 },
            Step::Crash {
                node: 7,
                gap_secs: 60,
            },
            Step::Offline { node: 6 },
            Step::Touch { node: 4, path: 1 },
            Step::MassDelete {
                node: 5,
                fraction: 72,
            },
            Step::Settle { secs: 30 },
            Step::Delete { node: 3, path: 10 },
            Step::Chmod { node: 6, path: 3 },
            Step::Mkdir { node: 4, dir: 2 },
            Step::Modify {
                node: 7,
                path: 3,
                content: 5,
            },
            Step::Tier {
                a: 6,
                b: 4,
                tier: 2,
            },
            Step::MassModify {
                node: 7,
                fraction: 96,
                content: 4,
            },
            Step::Crash {
                node: 5,
                gap_secs: 20,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 3,
            },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Create {
                node: 6,
                path: 10,
                content: 2,
            },
            Step::Online { node: 5 },
            Step::Create {
                node: 2,
                path: 4,
                content: 4,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Modify {
                node: 4,
                path: 5,
                content: 4,
            },
            Step::Create {
                node: 5,
                path: 5,
                content: 3,
            },
            Step::Settle { secs: 28 },
            Step::Create {
                node: 1,
                path: 6,
                content: 3,
            },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Touch { node: 5, path: 3 },
            Step::Modify {
                node: 0,
                path: 6,
                content: 2,
            },
            Step::Heal { a: 4, b: 1 },
            Step::Crash {
                node: 6,
                gap_secs: 31,
            },
            Step::Delete { node: 2, path: 7 },
            Step::Create {
                node: 6,
                path: 11,
                content: 5,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 30,
            },
            Step::Rename {
                node: 1,
                from: 9,
                to: 4,
            },
            Step::Delete { node: 2, path: 10 },
            Step::MassDelete {
                node: 4,
                fraction: 57,
            },
            Step::Create {
                node: 2,
                path: 4,
                content: 2,
            },
            Step::Chmod { node: 1, path: 3 },
            Step::Delete { node: 2, path: 6 },
            Step::Modify {
                node: 6,
                path: 7,
                content: 2,
            },
            Step::Settle { secs: 39 },
            Step::Create {
                node: 1,
                path: 11,
                content: 2,
            },
            Step::Online { node: 2 },
        ],
    );
}

/// Seed 33742 of the first 100,000-seed run, default knobs: 380 of its
/// 400 steps. The full seed no longer fails once the first two fixes change
/// its course, so this list was found among its subsets by a search for
/// one that fails without the commit rule and passes with it, then shrunk.
/// It fails the way the first run did: a newer version replaced a want
/// whose commit was in flight, the commit's report was dropped, the landed
/// file came back at the next scan as a local change, and that change
/// re-issued a vector a revert had discarded (§7.5).
#[test]
fn a_version_arriving_during_a_commit_does_not_drop_its_report() {
    passes(
        33742,
        &[
            Step::Modify {
                node: 4,
                path: 10,
                content: 2,
            },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 23,
            },
            Step::Modify {
                node: 2,
                path: 1,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 4,
            },
            Step::MassModify {
                node: 1,
                fraction: 64,
                content: 3,
            },
            Step::Crash {
                node: 5,
                gap_secs: 47,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(3), Some(1), Some(4), None, Some(3), Some(5)],
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::Partition { a: 6, b: 5 },
            Step::Modify {
                node: 7,
                path: 5,
                content: 2,
            },
            Step::Create {
                node: 1,
                path: 7,
                content: 4,
            },
            Step::User {
                node: 5,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 50,
                },
                delay_secs: 6,
            },
            Step::Modify {
                node: 1,
                path: 1,
                content: 1,
            },
            Step::Rename {
                node: 0,
                from: 4,
                to: 11,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(5), Some(4), Some(3), None, Some(5), Some(1), Some(1)],
            },
            Step::Touch { node: 6, path: 3 },
            Step::Modify {
                node: 3,
                path: 6,
                content: 2,
            },
            Step::Mkdir { node: 1, dir: 1 },
            Step::Heal { a: 6, b: 1 },
            Step::Symlink {
                node: 1,
                path: 8,
                target: 6,
            },
            Step::Tier {
                a: 6,
                b: 3,
                tier: 0,
            },
            Step::Partition { a: 3, b: 4 },
            Step::Everywhere {
                path: 0,
                contents: vec![None, Some(3), None],
            },
            Step::Partition { a: 5, b: 1 },
            Step::Create {
                node: 0,
                path: 7,
                content: 5,
            },
            Step::Create {
                node: 2,
                path: 9,
                content: 3,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(4), None, None, None, Some(4)],
            },
            Step::Modify {
                node: 2,
                path: 5,
                content: 3,
            },
            Step::Partition { a: 0, b: 5 },
            Step::Rename {
                node: 0,
                from: 10,
                to: 4,
            },
            Step::Tier {
                a: 4,
                b: 1,
                tier: 2,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(5), Some(4), Some(4), Some(2), Some(2), Some(1)],
            },
            Step::Offline { node: 6 },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 44,
            },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Mkdir { node: 2, dir: 1 },
            Step::MassModify {
                node: 0,
                fraction: 99,
                content: 5,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 19,
            },
            Step::Online { node: 7 },
            Step::Modify {
                node: 4,
                path: 6,
                content: 5,
            },
            Step::Settle { secs: 38 },
            Step::Partition { a: 0, b: 4 },
            Step::MassDelete {
                node: 0,
                fraction: 81,
            },
            Step::MassModify {
                node: 1,
                fraction: 90,
                content: 3,
            },
            Step::Create {
                node: 1,
                path: 8,
                content: 3,
            },
            Step::Settle { secs: 18 },
            Step::Delete { node: 6, path: 3 },
            Step::Chmod { node: 1, path: 3 },
            Step::Modify {
                node: 5,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 7,
                content: 1,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 4,
            },
            Step::Heal { a: 5, b: 5 },
            Step::Heal { a: 4, b: 0 },
            Step::Chmod { node: 2, path: 10 },
            Step::Heal { a: 1, b: 7 },
            Step::Modify {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 1,
            },
            Step::Crash {
                node: 0,
                gap_secs: 87,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 27,
            },
            Step::Offline { node: 2 },
            Step::Modify {
                node: 1,
                path: 9,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 0,
                content: 1,
            },
            Step::Heal { a: 2, b: 0 },
            Step::Mkdir { node: 6, dir: 1 },
            Step::Rmdir { node: 7, dir: 0 },
            Step::Delete { node: 5, path: 4 },
            Step::Delete { node: 0, path: 10 },
            Step::Everywhere {
                path: 7,
                contents: vec![None, Some(3)],
            },
            Step::Modify {
                node: 1,
                path: 2,
                content: 2,
            },
            Step::Crash {
                node: 4,
                gap_secs: 59,
            },
            Step::Chmod { node: 3, path: 4 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Symlink {
                node: 7,
                path: 10,
                target: 1,
            },
            Step::Partition { a: 1, b: 5 },
            Step::Create {
                node: 2,
                path: 0,
                content: 4,
            },
            Step::Symlink {
                node: 6,
                path: 6,
                target: 10,
            },
            Step::Symlink {
                node: 1,
                path: 10,
                target: 1,
            },
            Step::Modify {
                node: 2,
                path: 6,
                content: 5,
            },
            Step::MassDelete {
                node: 2,
                fraction: 60,
            },
            Step::Modify {
                node: 5,
                path: 7,
                content: 2,
            },
            Step::Symlink {
                node: 3,
                path: 8,
                target: 2,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 27,
                },
                delay_secs: 59,
            },
            Step::Rename {
                node: 0,
                from: 6,
                to: 10,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 2,
            },
            Step::Symlink {
                node: 7,
                path: 11,
                target: 4,
            },
            Step::Online { node: 2 },
            Step::Delete { node: 3, path: 9 },
            Step::Partition { a: 3, b: 6 },
            Step::Modify {
                node: 5,
                path: 9,
                content: 1,
            },
            Step::MassDelete {
                node: 5,
                fraction: 81,
            },
            Step::Create {
                node: 2,
                path: 3,
                content: 4,
            },
            Step::Rmdir { node: 3, dir: 2 },
            Step::Delete { node: 5, path: 7 },
            Step::Delete { node: 0, path: 10 },
            Step::Offline { node: 7 },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Partition { a: 7, b: 6 },
            Step::Partition { a: 5, b: 7 },
            Step::Delete { node: 7, path: 2 },
            Step::Everywhere {
                path: 8,
                contents: vec![Some(2), Some(5)],
            },
            Step::Settle { secs: 27 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(5), Some(3), None],
            },
            Step::Rename {
                node: 3,
                from: 1,
                to: 11,
            },
            Step::Create {
                node: 5,
                path: 1,
                content: 2,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(5), Some(1), None],
            },
            Step::User {
                node: 2,
                action: crate::UserAction::Revert,
                delay_secs: 55,
            },
            Step::Modify {
                node: 0,
                path: 3,
                content: 4,
            },
            Step::Delete { node: 5, path: 9 },
            Step::Symlink {
                node: 0,
                path: 1,
                target: 9,
            },
            Step::Rmdir { node: 4, dir: 1 },
            Step::Settle { secs: 1 },
            Step::Heal { a: 4, b: 7 },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 28,
            },
            Step::Tier {
                a: 0,
                b: 1,
                tier: 1,
            },
            Step::Offline { node: 3 },
            Step::Delete { node: 2, path: 11 },
            Step::Create {
                node: 7,
                path: 0,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 2,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 3,
            },
            Step::Mkdir { node: 4, dir: 1 },
            Step::Touch { node: 4, path: 3 },
            Step::Rename {
                node: 2,
                from: 11,
                to: 1,
            },
            Step::Online { node: 4 },
            Step::Delete { node: 0, path: 5 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 5,
            },
            Step::Settle { secs: 20 },
            Step::Create {
                node: 1,
                path: 0,
                content: 4,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 42,
            },
            Step::Delete { node: 2, path: 11 },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 1,
                    hold_pct: 50,
                },
                delay_secs: 26,
            },
            Step::Modify {
                node: 5,
                path: 7,
                content: 1,
            },
            Step::Create {
                node: 4,
                path: 9,
                content: 3,
            },
            Step::Offline { node: 7 },
            Step::Settle { secs: 32 },
            Step::Chmod { node: 5, path: 6 },
            Step::Offline { node: 6 },
            Step::Partition { a: 5, b: 7 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 5,
            },
            Step::Modify {
                node: 5,
                path: 8,
                content: 1,
            },
            Step::Delete { node: 6, path: 5 },
            Step::Create {
                node: 2,
                path: 5,
                content: 4,
            },
            Step::Online { node: 5 },
            Step::Heal { a: 7, b: 0 },
            Step::Tier {
                a: 5,
                b: 7,
                tier: 1,
            },
            Step::Tier {
                a: 0,
                b: 1,
                tier: 2,
            },
            Step::Touch { node: 0, path: 7 },
            Step::Delete { node: 2, path: 4 },
            Step::User {
                node: 4,
                action: crate::UserAction::Revert,
                delay_secs: 52,
            },
            Step::Everywhere {
                path: 11,
                contents: vec![None, None, Some(2), None, Some(2), Some(2), Some(3)],
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Settle { secs: 18 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Touch { node: 4, path: 0 },
            Step::Offline { node: 4 },
            Step::Modify {
                node: 4,
                path: 8,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 1,
                content: 1,
            },
            Step::Tier {
                a: 0,
                b: 7,
                tier: 1,
            },
            Step::Offline { node: 6 },
            Step::Touch { node: 3, path: 2 },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Delete { node: 4, path: 4 },
            Step::Delete { node: 3, path: 4 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 18,
            },
            Step::Tier {
                a: 4,
                b: 6,
                tier: 1,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 4,
            },
            Step::Partition { a: 5, b: 2 },
            Step::MassModify {
                node: 0,
                fraction: 74,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 39,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![Some(3), Some(5), None, Some(5), Some(5)],
            },
            Step::Create {
                node: 6,
                path: 3,
                content: 1,
            },
            Step::Modify {
                node: 1,
                path: 3,
                content: 2,
            },
            Step::Partition { a: 4, b: 0 },
            Step::Partition { a: 3, b: 4 },
            Step::Partition { a: 5, b: 0 },
            Step::Modify {
                node: 5,
                path: 5,
                content: 4,
            },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(4), Some(5), Some(5), None, Some(3)],
            },
            Step::Partition { a: 6, b: 4 },
            Step::Everywhere {
                path: 0,
                contents: vec![Some(5), Some(3), Some(5), None, Some(2), None, None],
            },
            Step::Create {
                node: 6,
                path: 5,
                content: 5,
            },
            Step::Chmod { node: 1, path: 11 },
            Step::Tier {
                a: 7,
                b: 0,
                tier: 1,
            },
            Step::Delete { node: 7, path: 10 },
            Step::Modify {
                node: 6,
                path: 8,
                content: 4,
            },
            Step::Partition { a: 5, b: 2 },
            Step::Rename {
                node: 2,
                from: 3,
                to: 1,
            },
            Step::Crash {
                node: 1,
                gap_secs: 72,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Chmod { node: 6, path: 8 },
            Step::Crash {
                node: 2,
                gap_secs: 3,
            },
            Step::Offline { node: 5 },
            Step::Partition { a: 5, b: 6 },
            Step::Mkdir { node: 0, dir: 2 },
            Step::Rename {
                node: 1,
                from: 0,
                to: 4,
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 5,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Rules {
                    hold_count: 2,
                    hold_pct: 0,
                },
                delay_secs: 14,
            },
            Step::Create {
                node: 4,
                path: 1,
                content: 5,
            },
            Step::Settle { secs: 32 },
            Step::Modify {
                node: 1,
                path: 2,
                content: 3,
            },
            Step::Create {
                node: 6,
                path: 4,
                content: 5,
            },
            Step::Tier {
                a: 1,
                b: 2,
                tier: 2,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::ApproveAll,
                delay_secs: 32,
            },
            Step::MassModify {
                node: 4,
                fraction: 89,
                content: 2,
            },
            Step::Settle { secs: 14 },
            Step::Partition { a: 5, b: 1 },
            Step::Settle { secs: 21 },
            Step::Create {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Delete { node: 1, path: 5 },
            Step::Partition { a: 1, b: 1 },
            Step::Settle { secs: 7 },
            Step::User {
                node: 3,
                action: crate::UserAction::ApproveAll,
                delay_secs: 0,
            },
            Step::Heal { a: 5, b: 1 },
            Step::Delete { node: 3, path: 2 },
            Step::MassDelete {
                node: 2,
                fraction: 62,
            },
            Step::Chmod { node: 5, path: 3 },
            Step::Create {
                node: 0,
                path: 1,
                content: 4,
            },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Delete { node: 3, path: 3 },
            Step::Delete { node: 2, path: 0 },
            Step::Create {
                node: 0,
                path: 4,
                content: 1,
            },
            Step::Mkdir { node: 0, dir: 0 },
            Step::Settle { secs: 18 },
            Step::Crash {
                node: 3,
                gap_secs: 101,
            },
            Step::Modify {
                node: 6,
                path: 3,
                content: 4,
            },
            Step::Online { node: 5 },
            Step::Modify {
                node: 3,
                path: 1,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![None, Some(1), Some(5), Some(3)],
            },
            Step::Partition { a: 3, b: 4 },
            Step::Crash {
                node: 4,
                gap_secs: 106,
            },
            Step::Settle { secs: 30 },
            Step::Rmdir { node: 2, dir: 1 },
            Step::Crash {
                node: 6,
                gap_secs: 119,
            },
            Step::Online { node: 0 },
            Step::Heal { a: 6, b: 5 },
            Step::Partition { a: 3, b: 4 },
            Step::Mkdir { node: 7, dir: 1 },
            Step::Create {
                node: 4,
                path: 5,
                content: 4,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::ApproveAll,
                delay_secs: 33,
            },
            Step::Modify {
                node: 5,
                path: 2,
                content: 2,
            },
            Step::Delete { node: 0, path: 2 },
            Step::Create {
                node: 5,
                path: 11,
                content: 2,
            },
            Step::Rmdir { node: 1, dir: 1 },
            Step::Create {
                node: 3,
                path: 10,
                content: 3,
            },
            Step::Everywhere {
                path: 7,
                contents: vec![Some(3), Some(5), Some(4), Some(5), Some(4), Some(4)],
            },
            Step::Modify {
                node: 4,
                path: 1,
                content: 5,
            },
            Step::Modify {
                node: 0,
                path: 9,
                content: 2,
            },
            Step::Rmdir { node: 6, dir: 2 },
            Step::Create {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Chmod { node: 2, path: 3 },
            Step::Modify {
                node: 1,
                path: 6,
                content: 1,
            },
            Step::Create {
                node: 5,
                path: 2,
                content: 3,
            },
            Step::Tier {
                a: 0,
                b: 0,
                tier: 0,
            },
            Step::Create {
                node: 7,
                path: 1,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 8,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 6,
                content: 2,
            },
            Step::Modify {
                node: 2,
                path: 10,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::Settle { secs: 21 },
            Step::Symlink {
                node: 5,
                path: 9,
                target: 3,
            },
            Step::Create {
                node: 7,
                path: 4,
                content: 1,
            },
            Step::Delete { node: 1, path: 2 },
            Step::Online { node: 6 },
            Step::Crash {
                node: 3,
                gap_secs: 78,
            },
            Step::Heal { a: 5, b: 1 },
            Step::Everywhere {
                path: 11,
                contents: vec![Some(3), Some(3), None, Some(1), None, Some(3), Some(4)],
            },
            Step::Delete { node: 0, path: 6 },
            Step::Online { node: 3 },
            Step::Touch { node: 1, path: 10 },
            Step::Touch { node: 4, path: 9 },
            Step::MassDelete {
                node: 3,
                fraction: 93,
            },
            Step::Delete { node: 0, path: 9 },
            Step::Create {
                node: 4,
                path: 9,
                content: 1,
            },
            Step::Touch { node: 4, path: 9 },
            Step::Create {
                node: 5,
                path: 10,
                content: 4,
            },
            Step::Symlink {
                node: 1,
                path: 2,
                target: 4,
            },
            Step::Modify {
                node: 5,
                path: 9,
                content: 4,
            },
            Step::Symlink {
                node: 1,
                path: 9,
                target: 6,
            },
            Step::Crash {
                node: 7,
                gap_secs: 114,
            },
            Step::Modify {
                node: 0,
                path: 7,
                content: 1,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(1), Some(5), Some(5), Some(2), Some(3), Some(5)],
            },
            Step::Rename {
                node: 1,
                from: 10,
                to: 1,
            },
            Step::Delete { node: 7, path: 2 },
            Step::Crash {
                node: 1,
                gap_secs: 13,
            },
            Step::Modify {
                node: 6,
                path: 9,
                content: 5,
            },
            Step::Rename {
                node: 7,
                from: 4,
                to: 5,
            },
            Step::Settle { secs: 19 },
            Step::Chmod { node: 6, path: 3 },
            Step::Chmod { node: 1, path: 5 },
            Step::Symlink {
                node: 1,
                path: 2,
                target: 4,
            },
            Step::Partition { a: 0, b: 6 },
            Step::Rename {
                node: 7,
                from: 10,
                to: 8,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 48,
            },
            Step::Rename {
                node: 6,
                from: 10,
                to: 4,
            },
            Step::Partition { a: 3, b: 3 },
            Step::Settle { secs: 23 },
            Step::Partition { a: 2, b: 2 },
            Step::Mkdir { node: 6, dir: 0 },
            Step::Partition { a: 6, b: 5 },
            Step::Heal { a: 4, b: 7 },
            Step::Everywhere {
                path: 9,
                contents: vec![None, Some(4), Some(4), Some(5), Some(3), None, Some(2)],
            },
            Step::Delete { node: 4, path: 5 },
            Step::Online { node: 2 },
            Step::Crash {
                node: 2,
                gap_secs: 41,
            },
            Step::Touch { node: 2, path: 6 },
            Step::Delete { node: 1, path: 0 },
            Step::Create {
                node: 5,
                path: 1,
                content: 4,
            },
            Step::Delete { node: 1, path: 3 },
            Step::Chmod { node: 4, path: 10 },
            Step::Create {
                node: 7,
                path: 10,
                content: 4,
            },
            Step::Delete { node: 1, path: 10 },
            Step::Modify {
                node: 5,
                path: 10,
                content: 3,
            },
            Step::Touch { node: 0, path: 0 },
            Step::Mkdir { node: 0, dir: 1 },
            Step::Modify {
                node: 2,
                path: 6,
                content: 4,
            },
            Step::Create {
                node: 5,
                path: 0,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 4,
            },
            Step::Modify {
                node: 7,
                path: 11,
                content: 4,
            },
            Step::Create {
                node: 4,
                path: 0,
                content: 5,
            },
            Step::Symlink {
                node: 5,
                path: 9,
                target: 1,
            },
            Step::Create {
                node: 0,
                path: 0,
                content: 5,
            },
            Step::MassModify {
                node: 5,
                fraction: 53,
                content: 1,
            },
            Step::Partition { a: 3, b: 5 },
            Step::Touch { node: 0, path: 8 },
            Step::MassDelete {
                node: 2,
                fraction: 56,
            },
            Step::Partition { a: 4, b: 2 },
            Step::Create {
                node: 4,
                path: 7,
                content: 1,
            },
            Step::Partition { a: 6, b: 6 },
            Step::Everywhere {
                path: 10,
                contents: vec![Some(1), Some(2), Some(1), Some(3)],
            },
            Step::Tier {
                a: 2,
                b: 4,
                tier: 2,
            },
            Step::Rmdir { node: 0, dir: 2 },
            Step::Heal { a: 3, b: 1 },
            Step::Create {
                node: 1,
                path: 7,
                content: 5,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 3,
            },
            Step::Mkdir { node: 7, dir: 1 },
            Step::MassDelete {
                node: 5,
                fraction: 95,
            },
            Step::Everywhere {
                path: 3,
                contents: vec![None, Some(2), Some(2), Some(1), Some(1)],
            },
            Step::Mkdir { node: 2, dir: 2 },
            Step::MassModify {
                node: 1,
                fraction: 51,
                content: 3,
            },
            Step::Mkdir { node: 5, dir: 1 },
            Step::Rename {
                node: 3,
                from: 3,
                to: 11,
            },
            Step::Delete { node: 4, path: 9 },
            Step::Delete { node: 3, path: 5 },
            Step::Settle { secs: 27 },
            Step::Heal { a: 0, b: 3 },
            Step::User {
                node: 3,
                action: crate::UserAction::Revert,
                delay_secs: 45,
            },
            Step::Settle { secs: 26 },
            Step::User {
                node: 2,
                action: crate::UserAction::ApproveAll,
                delay_secs: 44,
            },
            Step::Create {
                node: 1,
                path: 5,
                content: 1,
            },
            Step::Modify {
                node: 6,
                path: 11,
                content: 3,
            },
            Step::Create {
                node: 7,
                path: 5,
                content: 2,
            },
            Step::Rename {
                node: 3,
                from: 1,
                to: 9,
            },
            Step::Heal { a: 1, b: 5 },
            Step::Crash {
                node: 3,
                gap_secs: 95,
            },
            Step::Create {
                node: 5,
                path: 8,
                content: 2,
            },
            Step::Settle { secs: 22 },
            Step::Create {
                node: 0,
                path: 3,
                content: 1,
            },
            Step::Create {
                node: 3,
                path: 1,
                content: 4,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::DenyAll,
                delay_secs: 52,
            },
            Step::Settle { secs: 14 },
            Step::Create {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Modify {
                node: 3,
                path: 1,
                content: 5,
            },
            Step::Mkdir { node: 3, dir: 2 },
            Step::Delete { node: 7, path: 9 },
            Step::Create {
                node: 4,
                path: 8,
                content: 1,
            },
            Step::Rename {
                node: 0,
                from: 0,
                to: 7,
            },
            Step::Settle { secs: 13 },
            Step::Rename {
                node: 7,
                from: 9,
                to: 1,
            },
            Step::User {
                node: 6,
                action: crate::UserAction::DenyAll,
                delay_secs: 10,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![
                    Some(4),
                    Some(3),
                    Some(4),
                    Some(1),
                    Some(3),
                    Some(4),
                    Some(4),
                ],
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Rules {
                    hold_count: 4,
                    hold_pct: 2,
                },
                delay_secs: 41,
            },
            Step::Modify {
                node: 5,
                path: 4,
                content: 1,
            },
            Step::User {
                node: 4,
                action: crate::UserAction::DenyAll,
                delay_secs: 16,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 31,
            },
            Step::Partition { a: 6, b: 4 },
            Step::Modify {
                node: 0,
                path: 3,
                content: 3,
            },
            Step::Modify {
                node: 6,
                path: 6,
                content: 5,
            },
        ],
    );
}

/// Seed 10187, default knobs, shrunk to 121 steps. It passed on the first
/// run and failed once the restart window changed its course: across two
/// reverts one node issued the same vector at f0 twice, for symlinks with
/// different targets. Symlinks carry no mtime, so both losing versions have
/// the same copy name, and I4 kept only the first loser under the shared
/// vector (§14.1).
#[test]
fn two_losers_under_one_reissued_vector_both_count() {
    passes(
        10187,
        &[
            Step::Offline { node: 6 },
            Step::Modify {
                node: 1,
                path: 5,
                content: 2,
            },
            Step::Rename {
                node: 5,
                from: 2,
                to: 7,
            },
            Step::Create {
                node: 2,
                path: 1,
                content: 2,
            },
            Step::Partition { a: 6, b: 3 },
            Step::Create {
                node: 2,
                path: 9,
                content: 4,
            },
            Step::Online { node: 6 },
            Step::Heal { a: 7, b: 7 },
            Step::Delete { node: 0, path: 5 },
            Step::Rmdir { node: 2, dir: 0 },
            Step::Create {
                node: 4,
                path: 11,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 9,
                content: 3,
            },
            Step::Modify {
                node: 4,
                path: 7,
                content: 2,
            },
            Step::MassModify {
                node: 5,
                fraction: 76,
                content: 2,
            },
            Step::Touch { node: 0, path: 10 },
            Step::Partition { a: 1, b: 1 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 3,
            },
            Step::Settle { secs: 23 },
            Step::Crash {
                node: 3,
                gap_secs: 19,
            },
            Step::MassModify {
                node: 0,
                fraction: 63,
                content: 2,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::DenyAll,
                delay_secs: 12,
            },
            Step::Heal { a: 5, b: 6 },
            Step::Heal { a: 6, b: 7 },
            Step::Mkdir { node: 1, dir: 2 },
            Step::Create {
                node: 6,
                path: 4,
                content: 5,
            },
            Step::Modify {
                node: 1,
                path: 10,
                content: 4,
            },
            Step::Modify {
                node: 0,
                path: 10,
                content: 1,
            },
            Step::Delete { node: 2, path: 0 },
            Step::Delete { node: 4, path: 5 },
            Step::Create {
                node: 3,
                path: 0,
                content: 5,
            },
            Step::Tier {
                a: 7,
                b: 6,
                tier: 2,
            },
            Step::Delete { node: 4, path: 2 },
            Step::Create {
                node: 1,
                path: 8,
                content: 3,
            },
            Step::Rmdir { node: 7, dir: 1 },
            Step::MassModify {
                node: 1,
                fraction: 52,
                content: 3,
            },
            Step::Settle { secs: 37 },
            Step::Partition { a: 3, b: 7 },
            Step::Offline { node: 6 },
            Step::Modify {
                node: 3,
                path: 9,
                content: 4,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 5,
            },
            Step::MassModify {
                node: 6,
                fraction: 75,
                content: 4,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(1), Some(1), Some(4), Some(5), Some(1)],
            },
            Step::Crash {
                node: 1,
                gap_secs: 97,
            },
            Step::Create {
                node: 2,
                path: 10,
                content: 2,
            },
            Step::Modify {
                node: 5,
                path: 6,
                content: 5,
            },
            Step::Partition { a: 1, b: 6 },
            Step::Modify {
                node: 7,
                path: 6,
                content: 1,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::Revert,
                delay_secs: 21,
            },
            Step::Create {
                node: 4,
                path: 11,
                content: 1,
            },
            Step::Crash {
                node: 0,
                gap_secs: 49,
            },
            Step::Partition { a: 4, b: 5 },
            Step::Touch { node: 5, path: 9 },
            Step::Mkdir { node: 5, dir: 0 },
            Step::Touch { node: 4, path: 0 },
            Step::Delete { node: 7, path: 1 },
            Step::Modify {
                node: 5,
                path: 2,
                content: 4,
            },
            Step::Tier {
                a: 3,
                b: 0,
                tier: 1,
            },
            Step::Touch { node: 7, path: 11 },
            Step::Create {
                node: 7,
                path: 9,
                content: 3,
            },
            Step::Offline { node: 1 },
            Step::Everywhere {
                path: 6,
                contents: vec![Some(4), Some(3), Some(5), Some(3)],
            },
            Step::Partition { a: 0, b: 1 },
            Step::Modify {
                node: 6,
                path: 9,
                content: 2,
            },
            Step::Online { node: 6 },
            Step::Everywhere {
                path: 11,
                contents: vec![None, Some(3), Some(3), Some(4), Some(4)],
            },
            Step::Heal { a: 5, b: 3 },
            Step::Create {
                node: 4,
                path: 5,
                content: 5,
            },
            Step::Tier {
                a: 0,
                b: 1,
                tier: 0,
            },
            Step::Modify {
                node: 4,
                path: 11,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 2,
                content: 1,
            },
            Step::Heal { a: 0, b: 4 },
            Step::Offline { node: 3 },
            Step::Create {
                node: 1,
                path: 1,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 7,
                content: 4,
            },
            Step::MassModify {
                node: 7,
                fraction: 52,
                content: 1,
            },
            Step::Create {
                node: 6,
                path: 9,
                content: 3,
            },
            Step::Modify {
                node: 7,
                path: 1,
                content: 5,
            },
            Step::Delete { node: 4, path: 10 },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
            Step::Modify {
                node: 1,
                path: 7,
                content: 1,
            },
            Step::User {
                node: 1,
                action: crate::UserAction::DenyAll,
                delay_secs: 7,
            },
            Step::Rmdir { node: 4, dir: 0 },
            Step::Partition { a: 2, b: 0 },
            Step::User {
                node: 1,
                action: crate::UserAction::Rules {
                    hold_count: 3,
                    hold_pct: 8,
                },
                delay_secs: 52,
            },
            Step::Chmod { node: 1, path: 1 },
            Step::Heal { a: 7, b: 4 },
            Step::Partition { a: 3, b: 0 },
            Step::Touch { node: 7, path: 7 },
            Step::Create {
                node: 1,
                path: 3,
                content: 4,
            },
            Step::Create {
                node: 0,
                path: 8,
                content: 4,
            },
            Step::Heal { a: 5, b: 1 },
            Step::Rename {
                node: 6,
                from: 5,
                to: 11,
            },
            Step::Modify {
                node: 3,
                path: 4,
                content: 1,
            },
            Step::Settle { secs: 8 },
            Step::Heal { a: 6, b: 1 },
            Step::Mkdir { node: 2, dir: 1 },
            Step::Create {
                node: 0,
                path: 8,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 4,
            },
            Step::Touch { node: 5, path: 6 },
            Step::Touch { node: 4, path: 3 },
            Step::Partition { a: 6, b: 4 },
            Step::Chmod { node: 0, path: 0 },
            Step::Delete { node: 4, path: 2 },
            Step::Create {
                node: 2,
                path: 5,
                content: 1,
            },
            Step::Tier {
                a: 6,
                b: 6,
                tier: 2,
            },
            Step::Online { node: 4 },
            Step::Mkdir { node: 5, dir: 2 },
            Step::Modify {
                node: 5,
                path: 11,
                content: 5,
            },
            Step::Delete { node: 6, path: 0 },
            Step::MassDelete {
                node: 3,
                fraction: 82,
            },
            Step::Online { node: 2 },
            Step::Crash {
                node: 4,
                gap_secs: 77,
            },
            Step::Settle { secs: 31 },
            Step::Symlink {
                node: 6,
                path: 0,
                target: 3,
            },
            Step::User {
                node: 0,
                action: crate::UserAction::Revert,
                delay_secs: 34,
            },
            Step::Settle { secs: 9 },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(1), Some(4), Some(3)],
            },
            Step::User {
                node: 4,
                action: crate::UserAction::Rules {
                    hold_count: 5,
                    hold_pct: 34,
                },
                delay_secs: 15,
            },
            Step::Modify {
                node: 6,
                path: 0,
                content: 3,
            },
            Step::Symlink {
                node: 6,
                path: 0,
                target: 0,
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 1,
            },
        ],
    );
}

/// Seed 15791, default knobs, shrunk to 96 steps: the same shape as
/// 10187 at d2/f5, a vector issued twice across a revert for two symlink
/// targets under one copy name (§14.1).
#[test]
fn two_symlink_losers_under_one_reissued_vector_both_count() {
    passes(
        15791,
        &[
            Step::Modify {
                node: 2,
                path: 1,
                content: 1,
            },
            Step::Create {
                node: 7,
                path: 10,
                content: 2,
            },
            Step::Settle { secs: 35 },
            Step::Partition { a: 3, b: 7 },
            Step::Rename {
                node: 3,
                from: 10,
                to: 8,
            },
            Step::Crash {
                node: 1,
                gap_secs: 117,
            },
            Step::Modify {
                node: 2,
                path: 7,
                content: 4,
            },
            Step::Create {
                node: 1,
                path: 9,
                content: 5,
            },
            Step::Create {
                node: 7,
                path: 7,
                content: 2,
            },
            Step::Heal { a: 2, b: 5 },
            Step::Tier {
                a: 2,
                b: 3,
                tier: 2,
            },
            Step::Heal { a: 7, b: 2 },
            Step::Create {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Tier {
                a: 5,
                b: 1,
                tier: 2,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(5), Some(4), Some(3), Some(1), Some(4), Some(4), None],
            },
            Step::Modify {
                node: 3,
                path: 10,
                content: 1,
            },
            Step::Everywhere {
                path: 5,
                contents: vec![Some(1), Some(4), Some(2), Some(4), Some(4)],
            },
            Step::Everywhere {
                path: 6,
                contents: vec![None, Some(3), Some(4), Some(4), Some(3), Some(1), Some(1)],
            },
            Step::Heal { a: 5, b: 0 },
            Step::Heal { a: 4, b: 7 },
            Step::MassDelete {
                node: 4,
                fraction: 81,
            },
            Step::Rename {
                node: 3,
                from: 0,
                to: 6,
            },
            Step::Create {
                node: 3,
                path: 11,
                content: 1,
            },
            Step::Delete { node: 7, path: 3 },
            Step::Modify {
                node: 3,
                path: 2,
                content: 1,
            },
            Step::Delete { node: 6, path: 11 },
            Step::Partition { a: 3, b: 5 },
            Step::Partition { a: 0, b: 3 },
            Step::Modify {
                node: 1,
                path: 10,
                content: 1,
            },
            Step::Chmod { node: 1, path: 8 },
            Step::Delete { node: 3, path: 3 },
            Step::Tier {
                a: 2,
                b: 5,
                tier: 2,
            },
            Step::Create {
                node: 0,
                path: 5,
                content: 5,
            },
            Step::Modify {
                node: 2,
                path: 2,
                content: 4,
            },
            Step::Mkdir { node: 2, dir: 0 },
            Step::Create {
                node: 6,
                path: 6,
                content: 2,
            },
            Step::Crash {
                node: 6,
                gap_secs: 94,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![None, Some(2)],
            },
            Step::Delete { node: 2, path: 8 },
            Step::MassDelete {
                node: 1,
                fraction: 80,
            },
            Step::Everywhere {
                path: 2,
                contents: vec![Some(4), Some(2), Some(1), Some(4)],
            },
            Step::Create {
                node: 7,
                path: 9,
                content: 5,
            },
            Step::Create {
                node: 5,
                path: 9,
                content: 5,
            },
            Step::Delete { node: 0, path: 0 },
            Step::Settle { secs: 37 },
            Step::Delete { node: 2, path: 5 },
            Step::Rename {
                node: 0,
                from: 3,
                to: 5,
            },
            Step::Everywhere {
                path: 8,
                contents: vec![None, None, None, None, Some(2), None],
            },
            Step::Mkdir { node: 3, dir: 1 },
            Step::Modify {
                node: 2,
                path: 11,
                content: 5,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Online { node: 5 },
            Step::Delete { node: 5, path: 1 },
            Step::Create {
                node: 3,
                path: 9,
                content: 5,
            },
            Step::Online { node: 2 },
            Step::Settle { secs: 4 },
            Step::Create {
                node: 5,
                path: 0,
                content: 4,
            },
            Step::MassDelete {
                node: 2,
                fraction: 75,
            },
            Step::Tier {
                a: 2,
                b: 3,
                tier: 2,
            },
            Step::Delete { node: 4, path: 8 },
            Step::MassModify {
                node: 2,
                fraction: 95,
                content: 4,
            },
            Step::Settle { secs: 27 },
            Step::User {
                node: 0,
                action: crate::UserAction::ApproveAll,
                delay_secs: 39,
            },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 31,
            },
            Step::Symlink {
                node: 1,
                path: 4,
                target: 7,
            },
            Step::Rmdir { node: 6, dir: 1 },
            Step::Modify {
                node: 4,
                path: 4,
                content: 5,
            },
            Step::Create {
                node: 1,
                path: 4,
                content: 2,
            },
            Step::User {
                node: 3,
                action: crate::UserAction::DenyAll,
                delay_secs: 59,
            },
            Step::Modify {
                node: 5,
                path: 9,
                content: 3,
            },
            Step::Create {
                node: 2,
                path: 8,
                content: 2,
            },
            Step::Modify {
                node: 3,
                path: 5,
                content: 1,
            },
            Step::Offline { node: 1 },
            Step::Create {
                node: 0,
                path: 3,
                content: 2,
            },
            Step::Rmdir { node: 6, dir: 0 },
            Step::Chmod { node: 7, path: 1 },
            Step::User {
                node: 6,
                action: crate::UserAction::Revert,
                delay_secs: 11,
            },
            Step::Create {
                node: 1,
                path: 4,
                content: 1,
            },
            Step::Modify {
                node: 4,
                path: 8,
                content: 4,
            },
            Step::MassModify {
                node: 3,
                fraction: 92,
                content: 3,
            },
            Step::Symlink {
                node: 1,
                path: 5,
                target: 1,
            },
            Step::Settle { secs: 6 },
            Step::Create {
                node: 3,
                path: 8,
                content: 5,
            },
            Step::Modify {
                node: 4,
                path: 5,
                content: 2,
            },
            Step::Online { node: 1 },
            Step::Settle { secs: 37 },
            Step::Heal { a: 3, b: 1 },
            Step::User {
                node: 7,
                action: crate::UserAction::Revert,
                delay_secs: 48,
            },
            Step::Offline { node: 3 },
            Step::Mkdir { node: 3, dir: 0 },
            Step::Create {
                node: 1,
                path: 5,
                content: 3,
            },
            Step::Create {
                node: 4,
                path: 5,
                content: 1,
            },
            Step::Create {
                node: 1,
                path: 0,
                content: 2,
            },
            Step::Symlink {
                node: 7,
                path: 5,
                target: 9,
            },
            Step::Modify {
                node: 3,
                path: 2,
                content: 5,
            },
        ],
    );
}
