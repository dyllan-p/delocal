...
>
> Changes from draft 29: the revert-keeps-content example corrected; a reset waiting for its report is a commit in flight (§8.3).
>
> Changes from draft 28: `revert` neither trashes nor refetches a path whose file already holds the announced content (§8.3).
>
> Changes from draft 27: a settled folder has no scan bracket open; `revert` undoes a deny it discards by returning the held item it consumed (§8.3).
>
...
>
> Changes from draft 22: the landing of a revert's refetch is a commit and is announced, so peers that were refused during the window ask again; the restoring flag belongs to the path until a refetch lands (§8.3).
>
> Changes from draft 21: a want keeps the entry as received and is re-classified from it whenever the record at its path is written (§7.5); revert re-derives the wants at every reverted path (§8.3).
>
> Changes from draft 20: a restoring want's commit expects the path to be absent (§8.3 step 2, §7.5 step 6).
>
...
>
> Changes from draft 17: the fast path compares a symlink's target (§7.3); a reverted path whose content no machine can serve resolves to a tombstone once every member has answered (§8.3).
>
> Changes from draft 16: the stamp rule generalised to every version an increment dominates, covering `deny` (§7.1).
>
...
**When `seq` advances.** A record gets a new `seq` only when it describes committed local state: after a local change is observed, after a fetched file has been renamed into place, after a deletion or a metadata-only change has been applied. **Every commit that lands advances `seq` and is announced**, whether it brought new content, set metadata back, or restored a record: a machine that answered `NotAvailable` while the commit was pending owes its peers that announcement, which is what puts it back among their sources (§7.5). Accepting a batch never advances `seq`. This is what makes "a member's index announces version X" mean "that member can serve X" (§7.5).
...
**Want-list.** Accepted entries go into a per-folder want-list, one want per path: the entry to end up with (`M` for a conflict), its apply mode, and its `sources`: the members that announced a version **with the wanted content** at that path, meaning the batch's source, the version's author, and any later batch carrying a version at that path with the same hash. Because fetches are by hash (below), whoever announced `W` is a source for `M`. A want also keeps **the entry as it was received**, beside the version it resolved to, for the same reason §8.2 quarantines raw entries rather than merges: whenever the record at a path is written, the want at that path is re-classified from its received entry against the record as it now stands. For an ordinary local change this gives the same result as re-classifying the resolved version (the new local version dominates the old one, so the merge is the same); for a record that `revert` discards (§8.3) it gives the right result instead of a dead one, since a conflict's `M` folds in the very version that no longer exists. The engine does not model every peer's index; a source that has moved on answers `NotAvailable` and is dropped from that want. **Selection**: among connected, non-excluded sources the best tier wins (`lan`, then `direct`, then `relay`), ties broken by smaller node ID so two machines with the same view pick the same source; a source whose tier limit (§6.5) the file exceeds is skipped. No candidate → the want is *without source*; every candidate skipped → *deferred*, naming the least demanding tier that would allow it. Both are re-evaluated whenever a peer connects, disconnects or changes tier. At most `max_fetches_per_peer` (default 4) and `max_fetches_per_folder` (default 16) fetches run at once, held in `Rules`, so the engine, not the host, decides parallelism and a deadline means something. **Deadlines**: a fetch with no progress reported for 60 s returns the want to *wanted* and reselects, without excluding the source (the stall may have been ours); the host reports fetch progress at most every few seconds. A commit is different: it is a local operation on this machine's own disk, so it **holds its path until the host reports it or the process restarts**, and a commit not reported within 30 s raises a status warning but releases nothing. **At most one commit is in flight per path**: a version arriving at a path whose commit is in flight waits for that commit's report and is then classified against the record as the report leaves it. Replacing the want mid-commit would drop the report of a commit that may already have landed, and the landed file would reappear at the next scan as a spurious local change. Transfers resume from the temp file's offset, so a false stall costs a round trip, not the bytes. A source that serves content whose hash does not match is excluded from that want, and a second mismatch from a second source gives the want up. **Exclusions and give-ups are both temporary**, and the same events lift them: the excluded source reconnecting, any source announcing a version at the path with the wanted content, or the index at the path changing. They also **expire on their own, on a backoff**: one minute after the mismatch, doubling on each repeat for the same want and source, capped at one hour. The events cover the common cases quickly; the expiry covers a source that stays connected and never re-announces, which none of the events would catch. A corrupted transfer is usually transient, and a permanent exclusion of what happened to be the only source would strand the file forever; a persistently corrupt source costs at most one retry an hour. Persistent corruption at a source is that source's to find: it hashes what it serves as it streams it, and a mismatch against its own record is an observation of the path (§7.3), so the damaged file is rescanned and announced as what it now is rather than served again [Phase 3].
...
- A batch entry counts as a `mod` only if it changes content: its `hash` differs from its `prev_hash`, or its kind or exec bit differs from the record it replaces. Metadata-only changes (touches, §7.3) and metadata-only applies (§7.5) count as neither `mods` nor `dels` for H1 and contribute no bytes to H2, on the sender pre-check and the receiver alike. Tombstones the engine writes itself because no member holds the content (§8.3 step 4) are not counted by the sender pre-check: there is nothing anywhere for a peer to lose, and counting them would pause the folder again on the very deletions `revert` just failed to undo, forever. Receivers need no exemption: a receiver counts a tombstone only over a live record, and a record enters the index only on commit, so a machine that never fetched the content has nothing to count. Exec-bit-only changes count as `mods` and contribute no bytes on the receiver, which can see the record being replaced. The sender pre-check cannot (after coalescing it no longer holds the replaced record, and `hash == prev_hash`), so it classifies them as metadata-only; a mass `chmod` is not destructive, and the receiver check still counts it.
...
- A batch id names **one** review item. Anything held under an id that already has an item joins it, its versions added to the item's: entries re-admitted from the deferred set, and an item returned by `revert` (§8.3). Nothing ever replaces an item's versions; dropping one would lose a version the sender has been acknowledged for.
...
2. For every path in it: move the current local file (if any) to trash, and reset the index record to the last version that was actually announced, restoring that record exactly, `seq` included, so it is not re-announced. A path whose file already holds the announced record's content (same kind and hash; its pending change was only a touch, a chmod, or a deny's bump) is **not** trashed or refetched: the record is restored, the host sets the file's mtime and exec bit back to the record's with a guarded `SetMeta` (§7.5 step 6), and no restoring mark is set. When the file's mtime and exec bit already match the record (after a deny's bump alone, or for a directory or symlink) nothing is sent at all. Without this rule, checking out another branch (which changes content and pauses the folder) and then checking the first branch out again (which leaves only touches pending) would, on `revert`, refetch the whole folder to restore bytes already on disk. While such a reset waits for its report it is a **commit in flight** at the path, with the usual deadline and persistence: a version arriving there waits for it rather than replacing it (replacing it would commit against a file the reset has not yet adjusted, fail the guard, and turn the untouched file into a spurious local touch), and queued decisions treat it as in flight. When the reset lands it is announced like any other landing (§7.1): until then this machine answers `NotAvailable` for the file, since its disk does not yet match the record, and the announcement is what tells a refused peer to ask again. A path peers never saw has no announced version; its record is removed. From this moment until the refetch lands, the index deliberately disagrees with the disk: the record describes what is to be fetched, not what is there, because the engine itself has just moved the file to the trash (or the reverted batch had deleted it). The refetch's commit therefore **expects the path to be absent** (§7.5 step 6 with `expected = None`), not the restored record's shape; anything found there is a real local change and fails the guard as it should. Revert also re-derives every want at a reverted path from its received entry (§7.5): a want resolved against the discarded record, such as a conflict's `M`, describes a merge that never happened and can never be satisfied, while the received entry classified against the restored record (or against nothing) is an ordinary apply that a peer can serve, and any deferred version that dominates it then replaces it, and inherits the restoring mark. The mark belongs to the **path**, carried by whichever want or deferred entry holds it, and it records a fact: this machine's record there says what it announced, while its disk holds nothing. At a marked path this machine therefore **holds neither** version (§7.6), and everything else follows from that:
